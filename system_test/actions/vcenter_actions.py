...
class VMwareActions(object):
    ...
    def create_instances(self):
        ...
        for num in xrange(self.vc_inst_count):
            name = '{prefix}-{num}'.format(prefix=self.vc_inst_name_prefix,
                                           num=num)
            self._create_server(name=name, availability_zone=vcenter_az,
    ...
    def shutdown_primary(self):
        ...
        while ng_node['online'] and timeout:
            time.sleep(interval)
            timeout -= interval
            ng_node = self.env.fuel_web.get_nailgun_node_by_devops_node(
                self.primary_ctlr_ng)
        if timeout:
            raise AssertionError('Primary controller is still online after')
    ...
    def turn_on_primary(self):
        ...
        while not ng_node['online'] and timeout:
            time.sleep(interval)
            timeout -= interval
            ng_node = self.env.fuel_web.get_nailgun_node_by_devops_node(
                self.primary_ctlr_ng)
    ...
    def ping_instance_from_instance(self, source_floating_ip,
                                    destination_ip, primary, size, count=1):
    ...
    def check_vm_connect(self):
        ...
        for v_num in xrange(self.vc_inst_count):
            v_name = '{prefix}-{num}'.format(
                prefix=self.vc_inst_name_prefix, num=v_num)
            v_server = self.os_conn.nova.servers.find(name=v_name)

            v_ip = self.os_conn.get_nova_instance_ip(v_server, net_name=net)
            v_floating = self.os_conn.assign_floating_ip(v_server)

            logger.info("Floating address {0} associated with instance {1}"
                        .format(v_floating.ip, v_server.name))

            for n_num in xrange(self.nova_inst_count):
                n_name = '{prefix}-{num}'.format(
                    prefix=self.nova_inst_name_prefix, num=n_num)
                n_server = self.os_conn.nova.servers.find(name=n_name)

                n_ip = self.os_conn.get_nova_instance_ip(n_server,
                                                         net_name=net)
                n_floating = self.os_conn.assign_floating_ip(n_server)

                logger.info("Floating address {0} associated with instance {1}"
                            .format(n_floating.ip, n_server.name))

                self.ping_instance_from_instance(
                    n_floating.ip, v_ip, self.primary_ctlr_ng.name, 5)

                self.ping_instance_from_instance(
                    v_floating.ip, n_ip, self.primary_ctlr_ng.name, 5)
