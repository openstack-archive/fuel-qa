...
class VMwareActions(object):
    ...
    def update_vmware(self, vmware_attr):
        """Update VMware attributes."""
        logger.debug("Try to update cluster with next vmware_attributes "
                     "{0}".format(pretty_log(vmware_attr)))
    ...
    def config_vmware_vcenter(self, vc_user, vc_host, vc_az, vc_pwd):
        """Configure vCenter."""
        ...
        vc_values.update(vcenter_value)
        self.update_vmware(vmware_attr)

    def config_vmware_glance(self, host, user, pwd, dc, ds):
        """Enable and configure Glance for VMware."""
    ...
    def config_vmware_computes(self, clusters):
    ...
    def configure_vcenter(self):
        ...
        self.config_vmware_vcenter(vc_user=settings['user'],
        ...
        if glance['enabled']:
            self.config_vmware_glance(host=glance['host'],
                                      user=glance['user'],
                                      pwd=glance['pwd'],
                                      dc=glance['datacenter'],
                                      ds=glance['datastore'])

        self.config_vmware_computes(clusters=vmware_vcenter['nova-compute'])
    ...
    def configure_vcenter_incorrect(self):
        ...
        self.config_vmware_vcenter(vc_user='user',
                                   vc_host='8.8.8.8',
                                   vc_az='az',
                                   vc_pwd='pwd')
        ...
        glance = vmware_vcenter['glance']
        if glance['enabled']:
            self.config_vmware_glance(host='8.8.8.8',
                                      user='user',
                                      pwd='pwd',
                                      dc='dc',
                                      ds='!@#$%^&*()')
        ...
        self.config_vmware_computes(clusters=clusters)
    ...
    def stop_deploy(self):
        ...
        self.fuel_web.deploy_cluster_wait_progress(self.cluster_id,
                                                   progress=50)
