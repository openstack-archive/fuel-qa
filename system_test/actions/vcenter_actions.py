...
class VMwareActions(object):
    ...
    def create_instances(self):
        ...
        n_inst_count = 1  # amount of VMs to create on nova
        n_inst_name_prefix = 'nova-test'
        n_image = self.os_conn.get_image('TestVM')

        # Instances with vcenter availability zone
        for num in xrange(vc_inst_count):
            name = '{prefix}-{num}'.format(prefix=vc_inst_name_prefix, num=num)
            logger.info(
                'Started: create VM "{name}" with net="{net}", az="{az}", '
                'image="{image}"'.format(name=name, net=net['name'],
                                         az=vcenter_az, image='TestVM-VMDK')
            )
            srv = self.os_conn.create_server(
                name=name, net_id=net['id'], availability_zone=vcenter_az,
                image=vc_image, timeout=200, sec_groups=[sec_group]
            )
            logger.info('VM "{}" was created'.format(name))
            self.vms_to_ping.append(srv)

        # Instances with nova availability zone
        for num in xrange(n_inst_count):
            name = '{prefix}-{num}'.format(prefix=n_inst_name_prefix, num=num)
            logger.info(
                'Started: create VM "{name}" with net="{net}", az="{az}", '
                'image="{image}"'.format(name=name, net=net['name'],
                                         az=vcenter_az, image='TestVM')
            )
            srv = self.os_conn.create_server(
                name=name, net_id=net['id'], image=n_image,
    ...
    def check_vm_connect(self):
        ...
        for srv in self.vms_to_ping:
            ...
            private_ips[srv] = self.os_conn.get_nova_instance_ip(
                server, net_name='admin_internal_net')
        ...
        for vm in itertools.combinations(self.vms_to_ping, 2):
            ...
            self.ping_instance_from_instance(
                floating_ips[vm[0]], private_ips[vm[1]],
                self.primary_ctlr_ng.name)
