...
class VMwareActions(object):
    ...
    def create_and_attach_empty_volume(self):
        ...
        try:
            ...
            logger.debug("Attaching volume failed by timeout")
            status = os_conn.get_volume_details(vol)['status']
            assert_equal(status, "in-use", "Volume doesn't reach 'in-use' "
                                           "state, current state is {0}"
                                           "".format(status))
