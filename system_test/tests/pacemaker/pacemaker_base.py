...
class PacemakerBaseActions(actions_base.ActionsBase):
    ...
    @deferred_decorator([make_snapshot_if_step_fail])
    def _action_fill_root_check_and_stopping_resources(self):
    ...
    @deferred_decorator([make_snapshot_if_step_fail])
    def _action_resolve_space_check_starting_resources(self):
    ...
    @deferred_decorator([make_snapshot_if_step_fail])
    def _action_health_check_after_filling_root(self):
        """Number of failed OSTF test should be equale 1"""
        number_of_failed = self._action_health_check_repeatably(self,
                                                                test_name=None,
                                                                test_retries=3,
                                                                checks=True)
        assert_true(
            number_of_failed == self.ostf_tests_should_failed,
            "Number of failed ostf tests not equal {}".format(
                self.ostf_tests_should_failed))
