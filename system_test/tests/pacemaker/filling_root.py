...
class PacemakerFillRootPrimaryController(pacemaker_base.PacemakerBaseActions):
    ...
    actions_order = [
        ...
        '_action_health_check_after_filling_root',
        '_action_resolve_space_check_starting_resources',
        '_action_health_check_repeatably',
        '_action_health_check_ha',
    ]
    @deferred_decorator([make_snapshot_if_step_fail])
    def _action_health_check_after_filling_root(self):
        number_of_failed=self._action_health_check_repeatably(self, self,
                                        test_name=None,
                                        test_retries=3,
                                        checks=True)
        assert_true(number_of_failed == self.ostf_tests_should_failed, "Number of failed ostf tests not equal {}")
