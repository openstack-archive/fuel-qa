...
class FillRootBaseActions(actions_base.ActionsBase):
    ...
    def fill_root_below_rabbit_disk_free_limit(self):
        """filling root more to below rabbit disk free limit"""
    ...
    def check_stopping_resources(self):
        """check stopped pacemaker resources"""
    ...
    @deferred_decorator([make_snapshot_if_step_fail])
    @action
    def resolve_space_on_root(self):
    ...
    @deferred_decorator([make_snapshot_if_step_fail])
    @action
    def check_failed_ostf_ha(self):
        """check that ostf ha is failed"""
        self.ostf_tests_should_failed = 7

        self.health_check_ha
    ...
    def check_starting_resources(self):
        ...
        logger.info(
            "Waiting 150 seconds for changing pacemaker status of {}".format(
                self.primary_controller_fqdn))
        time.sleep(150)
