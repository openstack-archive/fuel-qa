...
class VMwareActions(ActionsBase):
    ...
    def configure_dvs_plugin(self):
        ...
        options = {'#1_vmware_dvs_net_maps/value': self.full_config['template']['cluster_template']['settings']['vmware_dvs']['dvswitch_name']}
    ...
    def configure_vcenter(self):
        ...
        comp_vmware_nodes = [n for n in nodes if set(roles) <= set(n['pending_roles'])]

        for i in range(len(clusters['clusters'])):
            cluster = clusters['clusters'][i]
            srv_name = clusters['srv_name'][i]
            datastore = clusters['datastore'][i]
            if clusters['target_node'][i] == 'compute-vmware':
                node = comp_vmware_nodes.pop()
                target_node = node['hostname']
            else:
                target_node = clusters['target_node'][i]
        ...
        if vmware_vcenter['glance']['enable']:
            vcenter_value["glance"]["vcenter_host"] = vmware_vcenter['glance']['host']
            vcenter_value["glance"]["vcenter_username"] = vmware_vcenter['glance']['user']
            vcenter_value["glance"]["vcenter_password"] = vmware_vcenter['glance']['pwd']
            vcenter_value["glance"]["datacenter"] = vmware_vcenter['glance']['datacenter']
            vcenter_value["glance"]["datastore"] = vmware_vcenter['glance']['datastore']
