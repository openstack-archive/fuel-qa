...
class ActionsBase(PrepareBase, HealthCheckActions, PluginsActions):
    ...
    def _add_node(self, nodes_list):
        ...
        for new in nodes_list:
            for one in xrange(new['count']):
                ...
                if test_settings.MULTIPLE_NETWORKS:
                    nodes[name] = [new['roles'], new['nodegroup']]
                    logger.info("Set roles {} "
                                "to node {} in "
                                "{} nodegroup".format(new['roles'],
                                                      name,
                                                      new['nodegroup']))
                else:
    ...
    def create_env_multirack(self):
        ...
        # TODO(mstrukov): temporary fix of
        # https://bugs.launchpad.net/fuel/+bug/1524116
        from fuelweb_test.helpers.utils import run_on_remote
        with self.env.d_env.get_admin_remote() as remote:
            run_on_remote(remote,
                          'dockerctl shell mcollective rpm -ihv '
                          'http://mirror.seed-cz1.fuel-infra.org/'
                          'mos-repos/centos/mos8.0-centos6-fuel/'
                          'os/x86_64/Packages/daemonize-1.7.3-1'
                          '.el6.mira2.x86_64.rpm')
