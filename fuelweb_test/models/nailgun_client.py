...
class NailgunClient(object):
    ...
    def upload_configuration(self, config, cluster_id, role=None,
        ...
        data = {'cluster_id': cluster_id, 'config': config}
        if role is not None:
            data['node_role'] = role
    ...
    @logwrap
    @json_parse
    def list_configuration(self, cluster_id, role=None, nodes=None):
        ...
        Returns:
        a decoded JSON response."""
        url = '/api/openstack-config/cluster={0}'.format(cluster_id)
        if role is not None:
            url += '&role={0}'.format(role)
        if nodes is not None:
            url += '&nodes=' + ','.join(nodes)
        return self.client.get(url)
