...
class FuelWebClient(object):
    ...
    def common_net_settings(self, network_configuration):
        ...
        public = IPNetwork(self.environment.d_env.get_network(name="public"))
        ...
        if not net_pools:
            if not BONDING:
                ...
                pub_subnets = list(IPNetwork(
                    self.environment.d_env.get_network(
                        name="public")).subnet(27))
                if "floating" == net_name:
    ...
    def net_settings(self, net_config, net_name, floating=False, jbond=False):
        if jbond:
            ...
            ip_network = IPNetwork(
                self.environment.d_env.get_network(name=net_name).ip_network)
    ...
    def get_floating_ranges(self, network_set=''):
        ...
        net = list(IPNetwork
                   (self.environment.d_env.get_network(name=net_name)))
