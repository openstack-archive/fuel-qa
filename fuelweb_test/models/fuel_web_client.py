...
class FuelWebClient(object):
    ...
    def update_node_networks(self, node_id, interfaces_dict,
        ...
        # fuelweb_admin is always on first interface unless the interface is not bonded
        if PREDICTABLE_INTERFACE_NAMES:
            iface_to_check = 'enp0s3'
        else:
            iface_to_check = 'eth0'
        if iface_to_check not in get_bond_ifaces():
            interfaces_dict[iface_to_check] = interfaces_dict.get(iface_to_check, [])
            if 'fuelweb_admin' not in interfaces_dict[iface_to_check]:
                interfaces_dict[iface_to_check].append('fuelweb_admin')
    ...
    def change_default_network_settings(self):
        ...
        for _release in self.client.get_releases():
            ...
            for net_provider in net_settings:
                ...
                if not PREDICTABLE_INTERFACE_NAMES and 'baremetal' in networks and devops_env.get_networks(
                        name='ironic'):
