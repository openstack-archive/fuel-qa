...
class FuelWebClient(object):
    ...
    def common_net_settings(self, network_configuration):
        ...
        public = IPNetwork(self.environment.d_env._get_network("public"))
        ...
        if not net_pools:
            if not BONDING:
                ...
                pub_subnets = list(IPNetwork(
                    self.environment.d_env._get_network("public")).subnet(27))
                if "floating" == net_name:
            ...
            if not BONDING:
                ...
                pub_subnets = list(IPNetwork(
                    self.environment.d_env._get_network(public_net)).subnet(27))
    ...
    def net_settings(self, net_config, net_name, floating=False, jbond=False):
        if jbond:
            ...
            ip_network = IPNetwork(self.environment.d_env._get_network(net_name))
    ...
    def get_floating_ranges(self, network_set=''):
        ...
        net = list(IPNetwork(self.environment.d_env._get_network(net_name)))
    ...
    def cold_restart_nodes(self, devops_nodes):
        ...
        for node in devops_nodes:
            wait(
                lambda: self.get_nailgun_node_by_devops_node(node)['online'],
                timeout=60 * 10)
            remote = self.environment.d_env.get_ssh_to_remote_by_name(
                node.name)
            try:
