...
class FuelWebClient(object):
    ...
    def create_cluster(self,
        ...
        if not cluster_id:
            ...
            if help_data.VCENTER_USE and vcenter_value is not None :
                logger.info('Configure vCenter')
                logger.info('##############################################')
                vmware_attributes = self.client.get_cluster_vmware_attributes(cluster_id)
                logger.info("Get next with next attributes {0}".format(vmware_attributes))
                logger.info('##############################################')
                vcenter_data = vmware_attributes['editable']
                vcenter_data['value'] = vcenter_value
                logger.info("Put next with next attributes {0}".format(vmware_attributes))
                logger.info('##############################################')
                self.client.update_cluster_vmware_attributes(cluster_id, vmware_attributes)
                logger.info('##############################################')
                logger.info('##############################################')
                vmware_attributes = self.client.get_cluster_vmware_attributes(cluster_id)
                logger.info("Get next with next attributes {0}".format(vmware_attributes))
