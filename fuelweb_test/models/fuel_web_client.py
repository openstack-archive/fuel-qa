        ...
        for node in devops_nodes:
            ...
            try:
                ...
                assert_true(
                    ...
                    'Node {0} do not become online'.format(node))
            node.destroy()
    ...
    def warm_start_nodes(self, devops_nodes):
        ...
        for node in devops_nodes:
            try:
                ...
                assert_true(
                    self.get_nailgun_node_by_devops_node(node)['online'],
                    'Node {0} do not become online in 60*10 sec'.format(node))
    ...
    def cold_restart_nodes(self, devops_nodes):
        ...
        for node in devops_nodes:
            ...
            try:
                ...
                assert_false(
                    self.get_nailgun_node_by_devops_node(node)['online'],
                    'Node {0} do not become offline in 600  sec'.format(node))
        ...
        for node in devops_nodes:
            try:
                ...
                assert_true(
                    self.get_nailgun_node_by_devops_node(node)['online'],
                    'Node do not become online in 600 s'.format(node))
    ...
    def wait_nodes_get_online_state(self, nodes, timeout=4 * 60):
        for node in nodes:
            ...
            try:
                ...
                assert_true(
                    self.get_nailgun_node_by_devops_node(node)['online'],
                    'Node {0} does not become online'.format(node))
