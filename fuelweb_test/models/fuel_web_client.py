...
class FuelWebClient(object):
    ...
    def create_cluster(self,
        ...
        if not cluster_id:
            ...
            for option in settings:
                section = False
                if option in ('sahara', 'murano', 'ceilometer', 'mongo', 'ironic'):
    ...
    def update_baremetal_ranges(self, network_configuration):
        ...
        nc["baremetal_ranges"] = [[str(baremetal_net.ip[52]),
                                   str(baremetal_net.ip[-2])]]
    ...
    def net_settings(self, net_config, net_name, floating=False, jbond=False):
        if jbond:
            ...
            if net_config['name'] == 'baremetal':
                baremetal_net = self.environment.d_env.get_network(
                                    name='ironic')
                net_config['gateway'] = str(baremetal_net.ip[51])
                ip_network = baremetal_net.ip_network
            else:
    ...
    def get_cluster_additional_components(self, cluster_id):
       components = {}
       attributes = self.client.get_cluster_attributes(cluster_id)
       for component,options in attributes['editable']['additional_components'].items():
           #exclude metadata
           if not component in 'metadata':
               components[component] = options['value']
       return components
