...
class FuelWebClient29(object):
    ...
    def get_devops_node_online_status(self, node):
    ...
    def warm_shutdown_nodes(self, devops_nodes):
        ...
        for node in devops_nodes:
            ...
            try:
                wait(lambda: not self.get_devops_node_online_status(node),
                     timeout=60 * 10)
            except TimeoutError:
                assert_false(
                    self.get_devops_node_online_status(node),
    ...
    def warm_start_nodes(self, devops_nodes):
        ...
        for node in devops_nodes:
            try:
                wait(lambda: self.get_devops_node_online_status(node),
                     timeout=60 * 10)
            except TimeoutError:
                assert_true(
                    self.get_devops_node_online_status(node),
