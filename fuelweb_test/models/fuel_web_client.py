    ...
    def wait_mysql_galera_is_up(self, node_names, timeout=60 * 4):
        def _get_galera_status(_remote):
            template_cmd = (
                ...
                " = '{}';\"")
            result = _remote.execute(template_cmd.format('wsrep_ready'))
            if result.exit_code != 0 or u'ON' not in result.stdout_str:
                return False
            result = _remote.execute(template_cmd.format('wsrep_connected'))
            return result.exit_code != 0 and u'ON' in result.stdout_str
