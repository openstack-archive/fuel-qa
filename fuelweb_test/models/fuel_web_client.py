    ...
    def wait_mysql_galera_is_up(self, node_names, timeout=60 * 4):
        def _get_galera_status(_remote):
            get_request = (
                ...
                " = '{}';\"").format
            result0 = _remote.execute(get_request('wsrep_ready'), verbose=True)
            # if result.exit_code != 0 or u'ON' not in result.stdout_str:
            #     return False
            result1 = _remote.execute(get_request('wsrep_connected'), verbose=True)
            return (result0.exit_code == result1.exit_code == 0) and u'ON' in result0.stdout_str and u'ON' in result1.stdout_str
