...
class CollectorClient(object):
    ...
    def get_action_logs_ids(self, master_node_uid):
        return [actions['id'] for actions in self.get_action_logs(master_node_uid)[0]]

    @logwrap
    def get_action_logs_count(self, master_node_uid):
        return len([actions['id'] for actions in self.get_action_logs(master_node_uid)][0])

    @logwrap
    def get_action_logs_additional_info_by_id(self, master_node_uid, id):
        return [actions['body']['additional_info']
                for actions in self.get_action_logs(master_node_uid)[0]
                if actions['id'] == id]
