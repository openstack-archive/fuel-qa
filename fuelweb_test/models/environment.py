    ...
    def change_default_network_settings(self):
        ...
        for _release in self.fuel_web.client.get_releases():
            ...
            for net_provider in net_settings:
                ...
                # TODO(vkhlyunev): verify this block
                if net_provider == 'baremetal':
                    net = list(IPNetwork(
                        self.d_env.get_network(name='ironic').ip_network))
                    half = len(net) / 2
                    net_settings[net_provider]["baremetal_ranges"] =\
                        [[str(net[half]), str(net[-3])]]
                    continue
                ...
                if net_provider == 'neutron':
                    pr = networks['private_tun']
                    pr['cidr'] = str(private_net)
                    net_settings[net_provider]['config'][
                        'internal_cidr'] = '{}/{}'.format(
                        private_net.network, private_net.prefixlen + 1)
                    net_settings[net_provider]['config'][
                        'internal_gateway'] = str(private_net.network + 1)

                pub = networks['public']
                pub['cidr'] = str(public_net)
                pub['gateway'] = str(public_net.network + 1)
                pub_range = list(IPNetwork(public_net))
                pub['ip_range'] = [str(pub_range[2]),
