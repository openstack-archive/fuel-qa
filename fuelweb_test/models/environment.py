...
class EnvironmentModel(object):
    ...
    @logwrap
    def.d_env.get_admin_remote(self, login=settings.SSH_CREDENTIALS['login'],
    ...
    def set_admin_ssh_password(self):
        try:
            ...
            remote = self.d_env.get_admin_remote(login='root', password='r00tme')
    ...
    def wait_bootstrap(self):
        ...
        result = self.d_env.get_admin_remote().execute("grep 'Fuel node deployment "
                                                 "complete' '%s'" % log_path
                                                 )['exit_code']
        if result != 0:
