...
class EnvironmentModel(object):
    ...
    # Execute yum updates
    # If updates installed, then `dockerctl destroy all; bootstrap_admin_node.sh;`
    def admin_install_updates(self):
        logger.info('Searching for updates and re-bootstrap slaves')
        admin_remote = self.d_env.get_admin_remote()
        ...
        for str_line in update_result:
            match_updated_packets_count = re.search("Upgrade(?:\s*)(\d+).*Package", str_line)
            if match_updated_packets_count:
                updates_count = match_updated_packets_count.group(1)
            match_complete_message = re.search("(Complete!)", str_line)
            match_no_updates = re.search("No Packages marked for Update", str_line)

        if (updates_count < 1 or match_no_updates) and not match_complete_message:
            logger.warning('No updates were found or update was incomplete.\n'
                           'No re-bootsrap were done')
            return
        logger.info('{0} packet(s) were updated'.format(updates_count))

        destroy_bootstrap_cmd = 'dockerctl destroy all; bootstrap_admin_node.sh;'
        destroy_bootstrap_result = admin_remote.execute(destroy_bootstrap_cmd)
        logger.debug('Result of "{1}" command on master node: '
                     '{0}'.format(destroy_bootstrap_result, destroy_bootstrap_cmd))
        assert_equal(int(destroy_bootstrap_result['exit_code']), 0,
                     'Packages update failed, '
                     'inspect logs for details')
