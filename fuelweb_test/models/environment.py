...
class EnvironmentModel(object):
    ...
    def setup_environment(self, custom=settings.CUSTOM_ENV,
        ...
        if settings.DISTRO_SYNC_STEP == 'setup_environment':
            self.do_distro_sync()
    ...
    def do_distro_sync(self):
        ...
        cmd = "yum install -y git && " \
              ...
              "git checkout origin/{} && " \
              "./install.sh".format(
                  settings.DISTRO_SYNC_URL,
                  settings.DISTRO_SYNC_BRANCH)
        logger.info("Installing fuel-distupgrade from git '{}'".format(
            cmd
        ))
        self.execute_remote_cmd(remote, cmd)
        cmd = '{} fuel-distupgrade prepare && ' \
              'fuel-distupgrade update'.format(
                  settings.DISTRO_SYNC_ENV_VARS)
