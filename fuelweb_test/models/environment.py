...
class EnvironmentModel(six.with_metaclass(SingletonMeta, object)):
    ...
    def admin_install_updates_mos_mu(self, cluster_id):
        ...
        # WORKAROUND: Set priority for repo
        # (Bug: https://bugs.launchpad.net/fuel/+bug/1641543)
        logger.warning('WORKAROUND: Set priority for repository')
        command = "sed -i 's/priority=1//g' /etc/yum.repos.d/temporary-0.repo"
        self.ssh_manager.check_call(
            ip=self.ssh_manager.admin_ip,
            command=command)

        logger.info('Perform a preparation playbook')
        ext_vars = '\'{{"env_id":{0}, "mos9_centos_repo":"{1}", ' \
                   '"mos9_GPG_KEY":"{2}"}}\''.format(
            cluster_id,
            settings.UPGRADE_CLUSTER_FROM_PROPOSED_RPM,
            settings.UPGRADE_CLUSTER_FROM_PROPOSED_RPM_KEY
        )
        command = '{0} ansible-playbook playbooks/mos9_prepare.yml -e ' \
        ...
        logger.info('Update Fuel node')
        update_command = \
            '{} ansible-playbook playbooks/update_fuel.yml'.format(mos_mu_path)
