...
class EnvironmentModel(object):
    ...
    def get_keys(self, node, custom=None, build_images=None, iso_connect_as='cdrom'):
        params = {
            'ks': 'cdrom:/ks.cfg' if iso_connect_as=='cdrom' else 'hd:LABEL="Mirantis_Fuel":/ks.cfg',
    ...
    def d_env(self):
        if self._virtual_environment is None:
            try:
                ...
                self._virtual_environment = Environment.describe_environment(boot_from=self._boot_from)
    ...
    def setup_environment(self, custom=settings.CUSTOM_ENV,
        ...
        admin.send_keys(self.get_keys(admin, custom=custom,
                        build_images=build_images, iso_connect_as=iso_connect_as))
        if custom:
