...
@test(groups=["ha", "classic_provisioning", "neutron_tun_ha"])
...
class TestHaNeutronScalability(TestBasic):
    ...
    def neutron_tun_scalability(self):
        ...
        def _check_swift(remote):
            with self.env.d_env.get_ssh_for_remote(devops_node.name) as remote:
                for i in range(5):
                    try:
                        checkers.check_swift_ring(remote)
                        break
...
@test(groups=["known_issues", "ha", "neutron_tun_ha"])
