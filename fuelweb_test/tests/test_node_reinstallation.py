...
class ReadyNodeReinstallation(TestBasic):
    ...
    def reinstall_single_regular_controller_node(self):
        ...
        # reinstalled node
        with self.env.d_env.get_ssh_to_remote(regular_ctrl['ip']) as remote:
    ...
    def reinstall_single_primary_controller_node(self):
        ...
        # Verify that Ceilometer API service is up and running on the
        # reinstalled node
        with self.env.d_env.get_ssh_to_remote(primary_ctrl_nailgun['ip']) \
                as remote:
            checkers.verify_service(remote, service_name='ceilometer-api')
    ...
    def full_cluster_reinstallation(self):
        ...
        for ctrl in ctrls_nailgun:
            with self.env.d_env.get_ssh_to_remote(ctrl['ip']) as remote:
                checkers.verify_service(remote, service_name='ceilometer-api')
...
class ErrorNodeReinstallation(TestBasic):
    ...
    def reinstall_failed_controller_deployment(self):
        ...
        Scenario:
            1. Revert the snapshot
            2. Start deployment; for one of controllers put inappropriate task
        ...
        # Verify that Ceilometer API service is up and running on the
        # reinstalled node
        with self.env.d_env.get_ssh_to_remote(ctrls_nailgun[-1]['ip']) \
                as remote:
            checkers.verify_service(remote, service_name='ceilometer-api')
