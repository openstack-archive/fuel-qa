...
class PartitionPreservation(TestBasic):
    ...
    def cinder_nova_partition_preservation(self):
        ...
        with self.fuel_web.get_ssh_for_nailgun_node(cmp_nailgun) as remote:
            devops_helpers.wait(
                lambda: "running" in remote.execute("service neutron-plugin-openvswitch-agent status")['stdout'][0],
                timeout=600)
