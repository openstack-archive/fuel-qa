...
@test(groups=["node_reinstallation"])
class NodeReinstallation(TestBasic):
    """NodeReinstallation."""  # TODO documentation

    def _check_ctrls(self, ctrls):
        """Check that controllers are operational after reinstallation."""
        # Verify that all controllers are in pacemaker cluster
        # and online
        self.fuel_web.assert_pacemaker(ctrls[0].name, ctrls, [])

        # Verify that all controllers are in rabbitmq cluster
        # and running
        rabbit_running_nodes = self.fuel_web.get_rabbit_running_nodes(
            ctrls[0].name)
        for node in ctrls:
            nailgun_node = self.fuel_web.get_nailgun_node_by_devops_node(node)
            assert_true(nailgun_node['hostname'] in rabbit_running_nodes,
                        "Rabbit node {0} is not in rabbit "
                        "status.".format(nailgun_node['hostname']))

        # Verify that all controllers are in Galera cluster and ready
        self.fuel_web.wait_mysql_galera_is_up([n.name for n in ctrls])
    ...
    def _reinstall_nodes(self, cluster_id, nodes=None):
        """Provision and deploy the given nodes."""
        task = self.fuel_web.client.provision_nodes(cluster_id, nodes)
        self.fuel_web.assert_task_success(task)
        task = self.fuel_web.client.deploy_nodes(cluster_id, nodes)
        self.fuel_web.assert_task_success(task)
    ...
    @test(depends_on=[node_reinstallation_env],
    ...
    def reinstall_single_regular_controller_node(self):
        ...
        Scenario:
            ...
            5. Run OSTF
            6. Verify that all controllers are operational:
                - they are in pacemaker cluster
                - they are in rabbitmq cluster
                - they are in galera cluster
            7. Verify that Ceilometer API service is up and running on the
               reinstalled node
            8. Verify that the hostname is not changed on reinstallation
               of the node
        ...
        self._reinstall_nodes(
            cluster_id, [str(non_primary_ctrl_nailgun['id'])])

        self.fuel_web.verify_network(cluster_id)
        self.fuel_web.run_ostf(cluster_id, test_sets=['ha', 'smoke', 'sanity'])

        # Verify that all controllers are operational
        self._check_ctrls(ctrls_devops)
    ...
    def reinstall_single_primary_controller_node(self):
        ...
        Scenario:
            ...
            8. Verify that the hostname is not changed on reinstallation
               of the node
            9. Verify that the primary-controller role is not migrated on
        ...
        self._reinstall_nodes(cluster_id, [str(primary_ctrl_nailgun['id'])])

        self.fuel_web.verify_network(cluster_id)
        self.fuel_web.run_ostf(cluster_id, test_sets=['ha', 'smoke', 'sanity'])

        # Verify that all controllers are operational
        self._check_ctrls(self.env.d_env.nodes().slaves[0:3])
    ...
    def reinstall_single_compute_node(self):
        ...
        self._reinstall_nodes(cluster_id, [str(cmps_nailgun[0]['id'])])
    ...
    def full_cluster_reinstallation(self):
        ...
        Scenario:
            ...
            4. Run OSTF
            5. Verify that all controllers are operational:
                - they are in pacemaker cluster
                - they are in rabbitmq cluster
                - they are in galera cluster
            6. Verify that Ceilometer API service is up and running on the
               reinstalled nodes
            7. Verify that all cinder services are up and running on nodes
        ...
        self._reinstall_nodes(cluster_id)

        self.fuel_web.verify_network(cluster_id)
        self.fuel_web.run_ostf(cluster_id, test_sets=['ha', 'smoke', 'sanity'])

        # Verify that all controllers are operational
        ctrls_nailgun = self.fuel_web.get_nailgun_cluster_nodes_by_roles(
            cluster_id, ['controller'])
        ctrls_devops = self.fuel_web.get_devops_nodes_by_nailgun_nodes(
            ctrls_nailgun)
        self._check_ctrls(ctrls_devops)
    ...
    def reinstall_failed_controller_deployment(self):
        ...
        Scenario:
            ...
            9. Run OSTF
            10. Verify that all controllers are operational:
                - they are in pacemaker cluster
                - they are in rabbitmq cluster
                - they are in galera cluster
            11. Verify that Ceilometer API service is up and running on the
               reinstalled node
