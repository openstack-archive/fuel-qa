...
class PartitionPreservation(TestBasic):
    ...
    @staticmethod
    def _preserve_partition(remote, node_id, partition):

        # Retrieve disks config for the given node
        with remote as admin_remote:
    ...
    def cinder_nova_partition_preservation(self):
        ...
        PartitionPreservation._preserve_partition(
            self.env.d_env.get_admin_remote(), cmp_nailgun['id'], "cinder")

        # Mark 'vm' partition to be preserved
        PartitionPreservation._preserve_partition(
            self.env.d_env.get_admin_remote(), cmp_nailgun['id'], "vm")
    ...
    def mongo_mysql_partition_preservation(self):
        ...
        mongo_nailgun = self.fuel_web.get_nailgun_cluster_nodes_by_roles(
            cluster_id, ['mongo'])[0]
        PartitionPreservation._preserve_partition(
            self.env.d_env.get_admin_remote(), mongo_nailgun['id'], "mongo")

        # Mark 'mysql' partition to be preserved
        PartitionPreservation._preserve_partition(
            self.env.d_env.get_admin_remote(), mongo_nailgun['id'], "mysql")
