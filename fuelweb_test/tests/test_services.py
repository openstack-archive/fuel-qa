...
class CeilometerHAMongo(OSTFCeilometerHelper):
    ...
    def ceilometer_test_cli(self):
        ...
        with self.env.d_env.get_ssh_to_remote(_ip) as remote:
            ...
            cmd = ("""
                ceilometer query-samples -f "{\"and\": [{\"=
            ...
            cmd = ('ceilometer sample-list -m $(ceilometer meter-list | awk ' \
                  '\'(NR == 4) ''{print $2}\') | awk \'(NR == 4)\') && ceilometer ' \
                  'sample-create -m' \' $(echo $sample |' \
                  ' awk \'{print $4}\') -r 111111 ' \
                  '--meter-type ' \
                  '$(echo $sample | awk \'{print $6}\') --meter-unit ' \
                  '$(echo $sample | awk \'{print $10}\') --sample-volume ' \
                  '$(echo $sample | awk \'{print $8\') 1>/dev/null')
            result = remote.execute(cmd)
            message = (result['stdout'] + result['stderr'])
            assert_equal(result['exit_code'], 0, err_msg.format(cmd, message))
            ...
            cmd = '(ceilometer alarm-show $(ceilometer alarm-list | awk '
                  '\'/alarm_test_plan_2/{print $2}\'))'
            result = remote.execute(cmd)
            message = (result['stdout'] + result['stderr'])
            assert_equal(result['exit_code'], 0, err_msg.format(cmd, message))
            ...
            """Alarm query history"""
            cmd = ("""ceilometer query-alarm-history -f
                   "{\"and\":[{\"<\":{\"timestamp\":\
                   "$(date +%Y-%m-%dT%H:%M:%S)\"}},{\">\":
            ...
            """Alarm query"""
            cmd = ("""ceilometer query-alarms -f "{\"or\":[{\"=\":{\"state\":
                    \"alarm\"}},{\"=\":{\"state\":\"ok\"}}]}""")
            result = remote.execute(cmd)
            message = (result['stdout'] + result['stderr'])
            assert_equal(result['exit_code'], 0, err_msg.format(cmd, message))
            ...
            cmd = ("""ceilometer alarm-delete $(ceilometer alarm-list
                    ...
                    $(ceilometer alarm-list |
                     awk '/alarm_comb_test_plan/{print $2}') 1>
                     /dev/null """)
            result = remote.execute(cmd)
            message = (result['stdout'] + result['stderr'])
            assert_equal(result['exit_code'], 0, err_msg.format(cmd, message))
