...
class CeilometerHAOneControllerMongo(OSTFCeilometerHelper):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["deploy_ceilometer_ha_one_controller_add_mongo"])
    @log_snapshot_after_test
    def deploy_ceilometer_ha_one_controller_add_mongo(self):
        """Deploy cluster in ha mode with Ceilometer and then add mongo node

        Scenario:
            1. Create cluster. Set install Ceilometer option
            2. Add 1 node with controller roles
            3. Add 2 nodes with mongo role
            4. Add 1 node with compute and cinder role
            5. Deploy the cluster
            6. Verify ceilometer api is running
            7. Run OSTF
            8. Add 1 node with mongo role
            9. Deploy changes
            10. Run OSTF

        Duration 80m
        Snapshot: deploy_ceilometer_ha_one_controller_add_mongo
        ...
        self.env.make_snapshot("deploy_ceilometer_ha_one_controller_add_mongo")
