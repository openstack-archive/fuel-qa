...
class TestOSupgrade(DataDrivenUpgradeBase):
    ...
    def upgrade_ceph(self):
        ...
        ceph_health = self.ssh_manager.execute_on_remote(
            ...
            cmd="ceph health",
            err_msg="octane upgrade-ceph failed")["stdout_str"]

        # There are an issue with PG calculation
    ...
    def upgrade_control_plane(self):
        ...
        non_ping_ips = collect_management_ips(old_computes)
    ...
    @test(depends_on=[upgrade_control_plane], groups=["upgrade_ceph_osd"])
    @log_snapshot_after_test
