...
class TestAddUpdatedNodeToCluster(TestBasic):
    ...
    * FORCE_DISABLE_UPDATES=True
    ...
    def create_delete_instance(self, cluster_id, controller_ip, compute_node):
    ...
    def add_updated_node_to_environment(self, role):
        ...
        self.show_step(6)
        controller_ip = (node['ip'] if role == 'controller'
                         else self.fuel_web.get_public_vip(cluster_id))
        compute_node = (node['fqdn'] if role == 'compute' else None)
        for i in range(5):
            self.create_delete_instance(cluster_id, controller_ip,
                                        compute_node)
