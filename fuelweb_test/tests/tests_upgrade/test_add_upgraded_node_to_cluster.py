...
class TestAddUpdatedNodeToCluster(TestBasic):
    ...
    def download_latest_snapshot(self):
        result = self.ssh_manager.check_call(
            self.admin_ip,
            ('curl {0}/ubuntu/snapshots/9.0-latest.target.txt '
    ...
    def base_deploy_3_ctrl_1_cmp(self):
        ...
        Scenario:
            ...
            5. Set local mirror mos as default for environments
        ...
        self.show_step(5)
        self.env.admin_actions.apply_mirror('ubuntu', 'mos')
    ...
    @test(depends_on=[base_deploy_3_ctrl_1_cmp],
          groups=["add_updated_compute_to_environment"])
    ...
    @test(depends_on=[base_deploy_3_ctrl_1_cmp],
          groups=["add_updated_controller_to_environment"])
