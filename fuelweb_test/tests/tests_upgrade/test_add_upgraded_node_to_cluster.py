...
class TestAddUpdatedNodeToCluster(TestBasic):
    ...
    mirrors_dir = 'mirrors/mos-repos/ubuntu/9.0'
    snapshot_dir = '/var/www/nailgun/snapshot'
    mos_repos = ['mos-updates', 'mos-security', 'mos-holdback']
    ...
    set_autoindex_on = ("sed -ir 's/autoindex off/autoindex on/' "
    ...
    def add_upgraded_controller_to_cluster(self):
        """Add node 9.(x+1) to cluster 9.x without master update
         ...
         Scenario:
             ...
             7. Clone the latest repo snapshot to the local repo
             8. Set local snapshot repo as default for environments
        ...
        self.ssh_manager.execute_on_remote(ip=admin_ip,
                                           cmd=self.set_autoindex_on)
        self.ssh_manager.execute_on_remote(ip=admin_ip, cmd=self.restart_nginx)
        ...
        for repo in repos:
            if repo['name'] in self.mos_repos:
                repo['uri'] = 'http://{0}:8080/{1}/'.format(admin_ip,
                                                            self.mirrors_dir)
        ...
        self.ssh_manager.execute_on_remote(
            ip=admin_ip,
            cmd='chown -R root:root {}'.format(self.snapshot_dir))
        self.ssh_manager.execute_on_remote(
            ip=admin_ip,
            cmd='chmod -R 755 {}'.format(self.snapshot_dir))

        self.show_step(8)
        self.show_step(9)
        for repo in repos:
            if repo['name'] in self.mos_repos:
                repo['uri'] = 'http://{}:8080/snapshot/'.format(admin_ip)
                if repo['name'] == 'mos-updates':
                    repo['suite'] = 'mos9.0-proposed'
                    repo['priority'] = 1150

        logger.info('Updated repos: {}'.format(repos))
        attributes['editable']['repo_setup']['repos']['value'] = deepcopy(
            repos)
        self.fuel_web.client.update_cluster_attributes(cluster_id, attributes)
