...
class MUInstallBase(test_cli_base.CommandLine):
    ...
    def _check_for_potential_updates(self, cluster_id, updated=False):
        ...
        if not updated:
            ...
            # step 1  -download sqlite db's to cudet db folded
            cudet_db_path = "/usr/share/cudet/db/versions/9.0/"
            centos_db_url = "https://jenkins-sandbox.infra.mirantis.net/" \
                            "job/generate-packages-database/8/" \
                            "artifact/9.0-centos-mu-1.sqlite"
            ubuntu_db_url = "https://jenkins-sandbox.infra.mirantis.net/" \
                            "job/generate-packages-database/9/" \
                            "artifact/9.0-ubuntu-mu-1.sqlite"
            ...
            # step 4  -discard changes in cudet main.py
