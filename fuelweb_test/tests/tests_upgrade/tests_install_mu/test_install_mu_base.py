...
class MUInstallBase(test_cli_base.CommandLine):
    ...
    def _install_mu(self, cluster_id, repos='proposed'):
        ...
        if settings.USE_MOS_MU_FOR_UPGRADE:
            logger.info('Enable kernel v4.4')
            ...
            if settings.MOS_MU_PATH:
                ...
                mos_mu_path = ''
            command = \
                '{0} ansible-playbook playbooks/update_ceph -e \'' \
    ...
    def _prepare_cluster_for_mu_via_mos_mu(self):
        ...
        self.env.admin_install_updates_mos_mu(
            repo=settings.UPGRADE_CLUSTER_FROM_PROPOSED_RPM,
            repo_key=settings.UPGRADE_CLUSTER_FROM_PROPOSED_RPM_KEY)
