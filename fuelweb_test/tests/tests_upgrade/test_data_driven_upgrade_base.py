    ...
    def install_octane(self):
        ...
        if OCTANE_PATCHES:
            logger.info("Patching octane with CR: {!r}".format(
                OCTANE_PATCHES))
            self.admin_remote.check_call("mkdir -p /usr/lib/python2.7/site-packages/deployment/puppet")
            self.admin_remote.check_call("mv /var/www/nailgun/octane_code/puppet/octane_tasks /usr/lib/python2.7/site-packages/deployment/puppet/")

            # pylint: disable=no-member
            self.patcher("/usr/lib/python2.7/site-packages", OCTANE_PATCHES)
            # pylint: enable=no-member

            self.admin_remote.check_call("mv /usr/lib/python2.7/site-packages/deployment/puppet/octane_tasks /var/www/nailgun/octane_code/puppet/")
            self.admin_remote.check_call("rm -rf /usr/lib/python2.7/site-packages/deployment")
