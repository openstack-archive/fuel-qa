...
class DataDrivenUpgradeBase(TestBasic):
    ...
    @property
    def local_path(self):
        return os.path.join(self.local_dir_for_backups,
                            self.backup_name)
    ...
    def install_octane(self):
        ...
        if hasattr(settings, 'FUEL_PROPOSED_REPO_URL') and \
                settings.FUEL_PROPOSED_REPO_URL or \
                'FUEL_PROPOSED_REPO_URL' in os.environ:
