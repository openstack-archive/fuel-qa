...
class DataDrivenUpgradeBase(TestBasic):
    ...
    def install_octane(self):
        ...
        """
        logger.info(
            "Installing fuel-octane: branch - '{}', refs - '{}'".format(
                self.OCTANE_BRANCH, self.OCTANE_REFS
            )
        )
        ...
        try:
            ...
            if not self.admin_remote.exists("fuel-octane"):
                raise
        ...
        for ref in self.OCTANE_REFS:
            install_cmds.append(
                "git fetch https://review.openstack.org/openstack/fuel-octane "
    ...
    def base_generate_backup(self, octane_cmd, path):
        ...
        logger.info(
            "Start generating the backup using {} command".format(octane_cmd))
        run_on_remote(self.admin_remote,
    ...
    def base_restore(self, octane_cmd, path):
        ...
        checkers.check_file_exists(self.admin_remote, path)
        logger.info(
            "Start restoring the backup using {} command".format(octane_cmd))
        run_on_remote(
            self.admin_remote,
    ...
    def do_restore(self,
        ...
        run_on_remote(
            self.admin_remote,
            "mkdir -p {}".format(os.path.dirname(backup_path))
        )
        ...
        if repos_backup_path:
            ...
            run_on_remote(
                self.admin_remote,
                "mkdir -p {}".format(os.path.dirname(repos_backup_path))
            )
...
class UpgradePrepare(DataDrivenUpgradeBase):
    ...
    def upgrade_smoke_backup(self):
        ...
        if not self.env.d_env.has_snapshot("ready_7"):
            ...
            self.env.make_snapshot("ready_7", is_make=True)
        self.env.revert_snapshot("ready_7")
    ...
    def upgrade_ceph_ha_backup(self):
        ...
        3. Add 2 node with compute+seph roles
        4. Verify networks
        5. Deploy cluster
        6. Run OSTF
        7. Install fuel-octane package
...
class UpgradeSmoke(UpgradePrepare):
    ...
    def upgrade_smoke_restore(self):
        ...
        self.admin_remote.execute("fuel-bootstrap build --activate")
