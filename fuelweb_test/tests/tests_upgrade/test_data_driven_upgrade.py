...
class UpgradeSmoke(UpgradePrepare):
    ...
    def upgrade_smoke_restore(self):
        ...
        if not self.env.d_env.has_snapshot("upgraded_8"):
            ...
            slave_03.destroy()
            self.env.make_snapshot("upgraded_8", is_make=True)
        self.env.revert_snapshot("upgraded_8")
    ...
    def upgrade_smoke_scale(self):
        ...
        """
        # self.show_step(1, initialize=True)
        # if not self.env.d_env.has_snapshot("middle"):
        ...
        #     self.show_step(6)
        #     self.fuel_web.run_ostf(cluster_id)
        #     self.env.make_snapshot("middle", is_make=True)
        # self.env.revert_snapshot("middle")
...
class UpgradeCephHA(UpgradePrepare):
    ...
    def upgrade_ceph_ha_reboot_ctrl(self):
        ...
        self.show_step(2)
        cluster_id = self.fuel_web.get_last_created_cluster()
        slave = self.fuel_web.get_devops_node_by_nailgun_node(
            self.fuel_web.client.list_cluster_nodes(cluster_id=cluster_id)[0])
        self.fuel_web.warm_restart_nodes(
            self.fuel_web.get_nailgun_node_by_devops_node(
                self.fuel_web.get_nailgun_primary_node(slave)))
        self.show_step(3)
        self.fuel_web.assert_ha_services_ready(cluster_id)
        self.show_step(4)
        self.fuel_web.run_ostf(cluster_id)

    def upgrade_ceph_ha_fuel_mirror(self):
        """Reset after upgrade, change mirrors using fuel-mirror and deploy

        Scenario:
        1. Revert "upgrade_ceph_ha_restore" snapshot.
        2. Reset cluster.
        3. Delete nodes from cluster and re-add it back.
        4. Create mirrors using fuel-mirror utility with --apply flag
        5. Validate that mirrors
        """
