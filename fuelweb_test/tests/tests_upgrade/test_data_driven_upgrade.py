...
class UpgradePrepare(DataDrivenUpgradeBase):
    ...
    def upgrade_smoke_backup(self):
        ...
        if not self.env.d_env.has_snapshot(intermediate_snapshot):
            ...
            cluster_id = self.fuel_web.create_cluster(
                name=self.upgrade_smoke_backup.__name__,
    ...
    def upgrade_ceph_ha_backup(self):
        ...
        use "fuel-createmirror' for validating repo-backup feature

        Scenario:
        1. Create mirrors using "fuel-createmirror" without args
        2. Create cluster with NeutronTUN and ceph for all
        3. Add 3 node with controller role
        4. Add 2 node with compute+ceph roles
        5. Verify networks
        6. Deploy cluster
        7. Run OSTF
        ...
        if not self.env.d_env.has_snapshot(intermediate_snapshot):
            ...
            cluster_id = self.fuel_web.create_cluster(
                name=self.upgrade_ceph_ha_backup.__name__,
                mode=settings.DEPLOYMENT_MODE,
                settings=cluster_settings
            )
            self.show_step(3)
    ...
    def upgrade_detach_plugin_backup(self):
        ...
        self.backup_name = "backup_with_plugin.tar.gz"
        self.repos_backup_name = "repos_backup_with_plugin.tar.gz"
        ...
        run_on_remote(
            ...
            " && ".join([
                ...
                "fpb --build . ",
                "fuel plugins --install *.rpm --user {user} --password {pwd}"
...
class UpgradeSmoke(UpgradePrepare):
    ...
    def upgrade_smoke_new_deployment(self):
        ...
        self.fuel_web.deploy_cluster_wait(3)
...
@test(groups=['upgrade_ceph_ha'])
class UpgradeCephHA(UpgradePrepare):
    def __init__(self):
    ...
    def upgrade_ceph_ha_restore(self):
        ...
        4. Upload the backup back to reinstalled Fuel maser node
        5. Restore master node using 'octane fuel-restore'
        6. Run OSTF for restored cluster
        ...
        self.check_run("upgrade_ceph_ha_restore")
        self.show_step(1, initialize=True)
        assert_true(
    ...
    @test(groups=['upgrade_ceph_ha_scale'],
          depends_on=[upgrade_ceph_ha_restore])
    def upgrade_ceph_ha_scale(self):
        pass

    def upgrade_ceph_ha_reset_deploy(self):
        pass
    def upgrade_ceph_ha_new_deployment(self):
        pass

@test(groups=['upgrade_rollback'])
class UpgradeRollback(UpgradePrepare):
    def __init__(self):
    ...
    def upgrade_rollback_ceph_ha(self):
        ...
        assert_true(os.path.exists(self.repos_local_path))
        assert_true(os.path.exists(self.local_path))
        self.check_run("upgrade_rollback_ceph_ha")
        self.show_step(1)
        assert_true(
            self.env.revert_snapshot("upgrade_ceph_ha_backup"),
            "The test can not use given environment - snapshot "
    ...
    def upgrade_rollback_ceph_ha_scale(self):
        ...
        """
        self.show_step(1)
        self.env.revert_snapshot("upgrade_rollback_ceph_ha")
    ...
    def upgrade_rollback_reset_redeploy(self):
        ...
        cluster_id = self.fuel_web.create_cluster(
            name=self.upgrade_rollback_reset_redeploy.__name__,
            mode=settings.DEPLOYMENT_MODE,
            settings=cluster_settings
        )
    ...
    def upgrade_rollback_new_deploy(self):
        ...
        4. Validate that cluster uses repositories located on fuel-master.
        5. Add 3 controllers.
        6. Add 2 compute + ceph nodes.
        7. Deploy cluster.
        8. Verify networks.
        9. Run OSTF.
        ...
        self.show_step(4)
        repos = self.fuel_web.get_cluster_repos(cluster_id)['value']
        admin_ip = self.env.get_admin_node_ip()
        assert_true(
            all([admin_ip in repo['uri'] for repo in repos]),
            "Repos was not updated after createmirror"
        )
