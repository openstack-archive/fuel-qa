...
class UpgradeCephHA(DataDrivenUpgradeBase):
    ...
    @log_snapshot_after_test
    @test(groups=['upgrade_XXX_ceph_ha_restore'])
    def upgrade_ceph_ha_restore(self):
        ...
        self.show_step(1, initialize=True)
        assert_true(
            self.env.revert_snapshot("XXXDebug"),
            "The test can not use given environment - snapshot "
...
#        self.show_step(7)
        ...
        run_on_remote(self.admin_remote, "sleep 180")
        ...
        cmd = "echo \"update ip_addr_ranges set first = '10.108.10.2' where id = 1;\" | dockerctl shell postgres sudo -u postgres psql nailgun"
        run_on_remote(self.admin_remote, cmd)
    ...
    def upgrade_ceph_ha_restore(self):
        ...
        self.env.get_admin_remote().execute("echo \"update ip_addr_ranges set first = '10.108.10.2' where id = 1;\" | dockerctl shell postgres sudo -u postgres psql nailgun")
        self.deploy_cluster(
            {'name': "env8",
                 'settings': cluster_settings,
                 'nodes': {'slave-01': ['controller'],
