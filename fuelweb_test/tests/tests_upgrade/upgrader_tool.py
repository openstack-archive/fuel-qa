...
class UpgradeCustom(DataDrivenUpgradeBase):
    ...
    def __init__(self):
        ...
        self.backup_snapshot_name = data["source_snapshot_name"]
        self.restore_snapshot_name = data["source_snapshot_name"]
        self.tarball_remote_dir = data["source_snapshot_name"]

        if data.get("backup_name"):
            self.backup_name = data.get("backup_name")
        else:
            self.backup_name = "backup_{}.tar.gz".format(
                self.source_snapshot_name)

        if data.get("repos_backup_name"):
            self.repos_backup_name = data.get("repos_backup_name")
        else:
