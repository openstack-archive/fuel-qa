...
class UpgradeCustom(DataDrivenUpgradeBase):
    ...
    def _get_current_step(self):
        for i in self.upgrade_data:
            if i['action'] == 'backup':
                if self.env.d_env.has_snapshot(i['backup_snapshot_name']):
                    continue
                else:
                    return i
            elif i['action'] == 'restore':
                if self.env.d_env.has_snapshot(i['restore_snapshot_name']):
                    continue
                else:
                    return i
