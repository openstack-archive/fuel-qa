...
class DataDrivenUpgradeBase(TestBasic):
    def __init__(self):
        ...
        self.backup_name = "autobackup.tar.gz"
    ...
    def install_octane(self):
        ...
        version = StrictVersion(
            run_on_remote_get_results(self.admin_remote,
                                      "fuel --version")['stderr'][-1])
        ...
        if self.DEBUG and not use_master:
            install_cmds.append(
                "git checkout -b stable/{version} "
                "origin/stable/{version}".format(version=str(version))
            )
        install_cmds.append("pip install --no-deps -e .")
        run_on_remote(self.admin_remote, " ; ".join(install_cmds))

    def generate_upgrade_backup(self, path):
...
class UpgradeSmoke(UpgradePrepare):
    ...
    def upgrade_smoke_new_deployment(self):
        ...
        release_id = [
            ...
            release['operating_system'].lower() == settings.OPENSTACK_RELEASE
            ][0]
        cluster_id = self.fuel_web.create_cluster(
