...
class TestMultiRackDeployment(DataDrivenUpgradeBase):
    ...
    def assert_net_in_updated_range(self, net_data, update_net_data):
        asserts.assert_true(
            self.is_ip_in_range(net_data['ip'].split('/')[0],
                                update_net_data[0][0],
                                update_net_data[0][-1]))
        ...
        for net_node in netgroup_data:
            for net in net_node:
                if 'storage' in net['name']:
                    asserts.assert_true(
                        self.assert_net_in_updated_range(
                            net, updated_storage))
                if 'management' in net['name']:
                    asserts.assert_true(
                        self.assert_net_in_updated_range(net, updated_mgmt))
    ...
    def prepare_upgrade_multirack_before_backup(self):
        ...
        # check for defaults
        self.assert_net_in_updated_range(mgmt_vrouter_vip,
                                         updated_mgmt_default)
        self.assert_net_in_updated_range(mgmt_vip,
                                         updated_mgmt_default)
        self.show_step(15)
