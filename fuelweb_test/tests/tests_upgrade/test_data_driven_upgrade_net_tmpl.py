...
class TestUpgradeNetworkTemplates(TestNetworkTemplatesBase,
    ...
    @test(groups=["reset_deploy_net_tmpl"])
    @log_snapshot_after_test
    ...
    @test(groups=["replace_controller_net_tmp"])
    @log_snapshot_after_test
    def replace_controller_net_tmpl(self):
        """Reset the existing cluster and redeploy - network templates

        Scenario:
        1. Revert "reset_deploy_net_tmpl" snapshot
        ...
        Snapshot: replace_controller_net_tmp
            """

        self.show_step(1)
        self.env.revert_snapshot("reset_deploy_net_tmpl")
        ...
        self.fuel_web.run_ostf(cluster_id=cluster_id, should_fail=1,
                               test_sets=['smoke', 'sanity', 'ha'])
        self.env.make_snapshot("replace_controller_net_tmp", is_make=True)

    @test(groups=["restart_node_net_tmp"])
    @log_snapshot_after_test
    def restart_node_net_tmpl(self):
        """Reboot some node from cluster after controller replacement -
           network templates

        Scenario:
        1. Revert "replace_controller_net_tmp" snapshot
        2. Reboot node
        3. Verify networks
        4. Run OSTF

        Snapshot: restart_node_net_tmp
            """

        self.show_step(1)
        self.env.revert_snapshot("replace_controller_net_tmp")
        ...
        self.fuel_web.run_ostf(cluster_id=cluster_id, should_fail=1,
                               test_sets=['smoke', 'sanity', 'ha'])
        self.env.make_snapshot("restart_node_net_tmp", is_make=True)
