...
class EnvCustomizationsCheck(TestBasic):
    ...
    FILE1 = "/usr/lib/python2.7/dist-packages/neutronclient/__init__.py"
    FILE2 = "/usr/lib/python2.7/dist-packages/neutronclient/version.py"
    ...
    def customizations_check_env(self):
        ...
        self.env.revert_snapshot("customizations_env")
    ...
    def check_env_customizations(self):
        ...
        """
        self.show_step(1)
        self.env.revert_snapshot("customizations_check_env")

        self.show_step(2)
        cluster_id = self.fuel_web.get_last_created_cluster()
        nodes = self.fuel_web.client.list_cluster_nodes(cluster_id)
        for node in nodes:
            self.add_comment_to_file_on_remote(node['ip'], self.FILE1)
        ...
        for case in cases:
            ...
            asserts.assert_true(
                self.FILE1 in result.stdout_str,
            ...
            for node in cases[case]:
                asserts.assert_true(
                    self.is_file_in_report(cluster_id, node['id'], self.FILE1),
                    "The {0!r} customized file is not referenced in the "
                    "corresponding node report".format(self.FILE1))
    ...
    def regenerate_report(self):
        ...
        cluster_id = self.fuel_web.get_last_created_cluster()
        nodes = self.fuel_web.client.list_cluster_nodes(cluster_id)
        self.add_comment_to_file_on_remote(nodes[0]['ip'], self.FILE1)
        ...
        self.show_step(5)
        self.add_comment_to_file_on_remote(nodes[0]['ip'], self.FILE2)
        ...
        asserts.assert_false(
            self.is_file_in_report(cluster_id, nodes[0]['id'], self.FILE2),
            "The {0!r} customized file is referenced in the report, despite "
            "the fake run of cudet".format(self.FILE2))
        ...
        asserts.assert_true(
            self.is_file_in_report(cluster_id, nodes[0]['id'], self.FILE2),
            "The {0!r} customized file is not referenced in the "
            "corresponding node report".format(self.FILE2))
