...
class OSUpgradeBase(DataDrivenUpgradeBase):
    ...
    def upgrade_db_code(self, seed_cluster_id):
        ...
        # WRONG PLACE
        GIT_URL = "http://github.com/openstack/fuel-nailgun-extension-converted-serializers"
        cmd = "yum install -y python-pip git" \
              "pip install git+{0} && service nailgun restart && sleep 60".format(GIT_URL)
    ...
    def upgrade_ceph_code(self, seed_cluster_id):
        ...
        self.ssh_manager.check_call(
            ip=self.ssh_manager.admin_ip,
            command="octane upgrade-ceph --with-graph 1 2",
    ...
    def upgrade_control_plane_code(self, seed_cluster_id):
        ...
        self.ssh_manager.check_call(
            ip=self.ssh_manager.admin_ip,
            command="octane upgrade-control --with-graph {0} {1}".format(
                self.orig_cluster_id, seed_cluster_id),
    ...
    def upgrade_ceph_osd_code(self, seed_cluster_id):
        ...
        self.ssh_manager.check_call(
            ip=self.ssh_manager.admin_ip,
            command="octane upgrade-osd --with-graph --admin-password {0} {1} {2}".format(
    ...
    def pre_upgrade_computes(self, orig_cluster_id):
        ...
        if LooseVersion(UPGRADE_FUEL_TO) >= LooseVersion('9.0') and \
            ...
            self.ssh_manager.check_call(
                ip=self.ssh_manager.admin_ip,
                command="octane preupgrade-compute --with-graph {0} {1}".format(
                    prev_rel_id,
