...
class OSUpgradeBase(DataDrivenUpgradeBase):
    ...
    def upgrade_db_code(self, seed_cluster_id):
        ...
        cmd = "curl -H \"X-Auth-Token: `fuel token`\" -XPUT -d '[\"volume_manager\", \"converted_serializers\"]' http://localhost:8000/api/clusters/1/extensions && service nailgun restart &	& sleep 60".format(GIT_URL)
