...
class OSUpgradeBase(DataDrivenUpgradeBase):
    ...
    def upgrade_db_code(self, seed_cluster_id):
        ...
        # WRONG PLACE
        self.patcher("/usr/lib/python2.7/site-packages/", "367327 369400")
        ...
        self.ssh_manager.check_call(
            ip=self.ssh_manager.admin_ip,
            command="fuel2 graph upload -e 1 -t upgrade-db -f  /var/www/nailgun/octane_code/puppet/octane_tasks/graphs/orig/upgrade-db.yaml && fuel2 graph execute -t upgrade-db -e 1 -d && sleep 30; octane upgrade-db --with-graph {0} {1}".format(
