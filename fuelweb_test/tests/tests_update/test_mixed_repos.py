...
class TestMixedRepos(TestBasic):
    ...
    def _attributes_remove_repos(self, attributes, repos_to_remove):
        repos = attributes['editable']['repo_setup']['repos']['value']

        for i, repo in repos[:]:
            if repo['name'] in repos_to_remove:
                del repos[i]
    ...
    def test_mixed_repos_env(self):
        ...
        self.show_step(3)
        attrs_default = self.fuel_web.client.get_cluster_attributes(cluster_id)
        self.fuel_web.client.update_cluster_attributes(
            cluster_id,
            self._attributes_remove_repos(attrs_default.copy(), update_repos)
        )
