...
class SeparateAllServices(TestBasic):
    ...
    def separate_all_service(self):
        ...
        checkers.install_plugin_check_code(
            self.env.d_env.get_admin_remote(),
            plugin=os.path.basename(settings.SEPARATE_SERVICE_RABBIT_PLUGIN_PATH))

        checkers.install_plugin_check_code(
            self.env.d_env.get_admin_remote(),
            plugin=os.path.basename(settings.SEPARATE_SERVICE_KEYSTONE_PLUGIN_PATH))
        ...
        plugin_names = ['detach-database', 'detach-keystone', 'detach-rabbitmq']
