...
class SeparateServicesNoha(TestBasic):
    ...
    DETACHED_PLUGINS = {
        ...
        'detach-rabbitmq': {
            ...
            'path': settings.SEPARATE_SERVICE_RABBITMQ_PLUGIN_PATH
        }
    }
    ...
    def install_plugins(self, plugin_names=[]):
        ...
        for plugin in list(set(self.plugins_path) & set(plugin_names)):
            utils.install_plugin_check_code(
                ip=self.ssh_manager.admin_ip,
                plugin=os.path.basename(self.plugins_path[plugin]['path']))
