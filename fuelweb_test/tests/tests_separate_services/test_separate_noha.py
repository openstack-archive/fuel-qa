...
class SeparateServicesNoha(TestBasic):
    ...
    def create_cluster(self, cluster_settings):
        """Create cluster with provided settings

        :param cluster_settings: dict, settings for the cluster
        :return: None
        """
    ...
    def prep_plugins(self):
        ...
        :return: None
        """
        for plugin, options in six.iteritems(self.detached_plugins):
            logger.info('Prepping {plugin}...'.format(plugin=plugin))
            check_plugin_path_env(
                var_name=options['var'],
                plugin_path=options['path'])
            utils.upload_tarball(
                ip=self.ssh_manager.admin_ip,
                tar_path=options['path'],
                tar_target="/var")

    def install_plugins(self, plugin_names):
        """Install plugins into the cluster

        :param plugin_name: list, name of plugins
        :return: None
        """
        for plugin in list(set(self.detached_plugins) & set(plugin_names)):
            logger.info('Installing {plugin}...'.format(plugin=plugin))
    ...
    def enable_plugins(self, plugin_names):
        ...
        for plugin in plugin_names:
            logger.info('Enabling {plugin}...'.format(plugin=plugin))
            assert_true(
                self.fuel_web.check_plugin_exists(self.cluster_id, plugin),
                msg)
            options = {'metadata/enabled': True}
            self.fuel_web.update_plugin_data(self.cluster_id, plugin, options)
