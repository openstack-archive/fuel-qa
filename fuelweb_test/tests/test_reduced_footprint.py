...
class ReducedFootprint(TestBasic):
    ...
    def update_virtual_nodes(self, cluster_id, hw_nodes_dict, nodes_dict):
        ...
        def get_names_from_nodes_dict(nodes_dict):
            nodes_names = []
            for node_name in nodes_dict:
                node_roles = nodes_dict[node_name]
                nodes_names.append(
                    '{}_{}'.format(node_name, "_".join(node_roles)))
            return nodes_names

        hw_nodes_names = get_names_from_nodes_dict(hw_nodes_dict)
        virt_nodes_names = get_names_from_nodes_dict(nodes_dict)

        nodes = self.fuel_web.client.list_nodes()

        virt_nodes = [node for node in nodes
                      if node['name'] not in hw_nodes_names]

        asserts.assert_equal(len(virt_nodes_names),
                             len(virt_nodes),
        ...
        for node in virt_nodes:
            ...
            node['pending_roles'] = ['controller'],
            node['name'] = virt_nodes_names.pop()

            self.fuel_web.client.update_node(node['id'], node)
