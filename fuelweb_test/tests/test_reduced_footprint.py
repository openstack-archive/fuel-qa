...
class TestVirtRole(TestBasic):
    ...
    For proper run set next environment variables:
        * SLAVE_NODE_CPU = 4
        * SLAVE_NODE_MEMORY = 4
    """
...
class ReducedFootprint(TestBasic):
    ...
    @test(depends_on=[TestVirtRole.spawn_one_vm_on_one_virt_node],
          groups=["deploy_reduced_footprint_ha_compact"])
    @log_snapshot_after_test
    def deploy_reduced_footprint_ha_compact(self):
        """Deploy ha_compact on one HW node

        Scenario:
            1. Revert ready_one_virt_one_vm snapshot
            2. Assign controller role to VM and deploy ha_compact
            3. Run Network tests
            4. Run OSTF check

        Duration 310m
        """
        self.env.revert_snapshot("ready_one_virt_one_vm")
