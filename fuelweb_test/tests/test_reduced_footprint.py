...
class ReducedFootprint(TestBasic):
    """Reduced footprint is about deployment on reduced number

    of physical nodes. The minimal nodes count is 5 for the current
    ...
    machine. This should be possible by assigning new role named "virt"
    to physical server,	after that user should upload VMs properties
    as node attributes.
"""
    ...
    def big_reduced_footprint(self):
        ...
        for node in virt_nodes:
            self.fuel_web.create_vms_nodes(node['id'], data_vm)
