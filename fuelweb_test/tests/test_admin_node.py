...
class TestLogrotateBase(TestBasic):
    ...
    def log_file_moved(self, output, log_file_orig_name, log_path='/var/log'):
        if len(output) == 0:
            return

        checker = re.compile(
            r'renaming {path}{name} to {path}(?P<new_name>[a-zA-Z0-9\-]+)'
            r''.format(path=log_path, name=log_file_orig_name))
        found_names = checker.findall(output)
        assert_true(
            len(found_names) == 1,
            'No information about file rename in output:\n{}'.format(output))
        found_name = found_names.pop()
        presents = self.ssh_manager.exists_on_remote(
            self.ssh_manager.admin_ip,
            path=os.path.join(log_path, found_name))
        assert_true(presents, 'No rotated log file found!')
        logger.info(
            'Logrotate renamed {path}{name} to {path}{new_name}'.format(
                name=log_file_orig_name,
                path=log_path,
                new_name=found_name
            ))
    ...
    def execute_logrotate_cmd(
        ...
        result = self.ssh_manager.execute_on_remote(
            remote_ip, cmd, raise_on_assert=False)
        logger.debug(
            'Results of command {0} execution:\n'
            '\tExit_code: {1}\n'
            '\tSTDOUT:\n'
            '{2}\n'
            '\tSTDERR:\n'
            '{3}'.format(
                cmd, result['exit_code'], result['stdout_str'],
                result['stderr_str']))

        if not any_exit_code:
            assert_equal(
                0, result['exit_code'],
                'Command {0} execution failed with code {1}. '
                'Execution result is:\n'
                '\tSTDOUT:\n'
                '{2}\n'
                '\tSTDERR:\n'
                '{3}'.format(
                    ...
                    result['stdout_str'], result['stderr_str']))
    ...
    def check_free_space(self, remote_ip, return_as_is=None):
        ...
        assert_equal(
            ...
            '{0}\n'
            '\tSTDERR:\n'
            '{1}'.format(result['stdout_str'], result['stderr_str']))
        if not return_as_is:
    ...
    def test_log_rotation(self):
        ...
        self.show_step(4)
        res = self.execute_logrotate_cmd(admin_ip, force=False)
        self.log_file_moved(res['stderr_str'], log_file_orig_name='messages')
        ...
        res = self.execute_logrotate_cmd(admin_ip, any_exit_code=True)

        assert_equal(
            True, self.no_error_in_log(res['stderr_str']),
            'Second run of logrotate failed'
            ' with:\n{0}'.format(res['stderr_str']))
