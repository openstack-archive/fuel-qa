...
class TestPublicApi(TestNetworkTemplatesBase):
    ...
    def deploy_env_with_public_api(self):
        ...
        def get_node_roles_from_test_config_template(path):
            test_config = load_yaml(path)

            last_slave_num = 0
            settings = {}
            for item in test_config['nodes']:
                for i in range(int(item['count'])):
                    last_slave_num += 1
                    settings['slave-{}'.format(str(last_slave_num).zfill(2))] = item['roles']
            return settings

        self.fuel_web.update_nodes(
            cluster_id=cluster_id,
            nodes_dict=get_node_roles_from_test_config_template(
                settings.ENABLE_DMZ_TEMPLATE
            ),
    ...
    def public_api_check_security_rules(self):
        ...
        ssh = self.env.d_env.get_ssh_to_remote(
                self.fuel_web.get_node_ip_by_devops_name(
                    controller_devops.name
                )
        )
