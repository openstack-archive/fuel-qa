...
class TestPublicApi(TestNetworkTemplatesBase):
    ...
    def deploy_env_with_public_api(self):
        ...
        #self.fuel_web.assert_os_services_ready(cluster_id, timeout=12*60)
        wait_pass(lambda: self.fuel_web.run_ostf(cluster_id,
                                                test_sets=['sanity', 'smoke']),
                                                interval=10, timeout=12*60)
    ...
    def public_api_check_security_rules(self):
        ...
        # https://10.200.5.2:8443/api/clusters/1/attributes/
        # editable public_ssl hostname value
        # editable public_ssl horizon value
        protocol = 'https' if attributes['editable']['public_ssl']['horizon'][
            'value'] == False else 'http'

        # horizon_url = 'https://public.fuel.local/horizon' # --insecure if controller_ip
        cmd = 'curl -I {proto}://{ip}/horizon --insecure'.format(proto=protocol,
                                                                 ip=controller_ip)
        logger.info('Trying to access horizon from instance: {}'.format(cmd))
        with self.env.d_env.get_ssh_to_remote(vm_floating_ip.ip) as ssh:
            res = ssh.execute(command=cmd)
            #res = ssh.execute_through_host(hostname=vm_floating_ip,
            #                               cmd=cmd,
            #                               auth=cirros_auth)
