...
@test(groups=["rhh", "rhh.ha", "rhh.bvt"])
class RhhHA(TestBasic):
    """RHH-based compute tests"""
    ...
    def connect_rh_image(slave):
        """Upload RHH image into a target node.
    ...
    def set_hostname(remote, host_number=1):
        ...
        :param remote: Remote node for proceed.
        """
        hostname = "rhh-{0}.test.domain.local".format(host_number)
    ...
    def backup_required_information(remote, ip):
        ...
        cmd = ("cd ~/ && mkdir rhh_backup; "
               "scp -r {0}:/root/.ssh rhh_backup/. ; "
               "scp {0}:/etc/astute.yaml rhh_backup/ ; "
               "scp -r {0}:/var/lib/astute/nova rhh_backup/").format(ip)
        result = remote.execute(cmd)
    ...
    def restore_information(self, ip, remote_admin, remote_slave):
        ...
        """
        cmd = "cat ~/rhh_backup/.ssh/authorized_keys"
        ...
        cmd = "cd ~/rhh_backup && scp astute.yaml {0}@{1}:/etc/.".format(
        ...
        cmd = (
            "cd ~/rhh_backup && scp -r nova {0}@{1}:/var/lib/astute/.".format(
                settings.RH_IMAGE_USER, ip)
        )
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["deploy_rhh_compute_ha_tun"])
    @log_snapshot_after_test
    def deploy_rhh_based_compute(self):
        """Deploy RHH-based compute in HA mode with Neutron VXLAN
        ...
        Scenario:
            ...
            8. Boot compute with RHH image.
            9. Prepare node for Puppet run.
            10. Execute modular tasks for compute.
            11. Run OSTF.

        Duration: 150m
        Snapshot: deploy_rhh_compute_ha_tun

        """
        self.show_step(1, initialize=True)
        LOGGER.debug('Check MD5 sum of RHH 7 image')
        ...
        self.show_step(3)
        LOGGER.debug('Create Fuel cluster RHH-based compute tests')
        data = {
        ...
        self.env.make_snapshot("ready_ha_with_rhh_compute", is_make=True)
