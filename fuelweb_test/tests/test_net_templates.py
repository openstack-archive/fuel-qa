...
class TestNetworkTemplates(TestNetworkTemplatesBase):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["no_management_network_tmpl"])
    @log_snapshot_after_test
    def no_management_network_tmpl(self):
        ...
        Duration 180m
        Snapshot no_management_network_tmpl
        """
        class NwkGrp(object):
            __slots__ = [
        ...
        def get_network_groups():
            return [NwkGrp(**grp) for grp in nailgun.get_network_groups()]

        self.show_step(1, initialize=True)
        self.env.revert_snapshot("ready_with_3_slaves")

        nailgun = NailgunClient(admin_node_ip=self.ssh_manager.admin_ip)
        ...
        assert_true(
            nailgun.del_network_group(admin_net.id).code in {200, 204},
        ...
        self.env.make_snapshot('no_management_network_tmpl')
