...
class NeutronGre(TestBasic):
    ...
    @test(depends_on=[step_7_run_openstack_controller],
          groups=['controller_remaining_task'])
    @log_snapshot_on_error
    def step_8_run_controller_remaining_task(self):
        ...
        Snapshot: "step_8_run_controller_remaining_task"
        """
        self.check_run_by_group('step_8_run_controller_remaining_task',
                                'controller_remaining_task')
        ...
        self.fuel_web.run_ostf(cluster_id=cluster_id,
                               test_sets=['sanity'], should_fail=1)

        self.env.make_snapshot("step_8_run_controller_remaining_task")

    @test(depends_on=[step_8_run_controller_remaining_task],
    ...
    def step_9_run_top_role_compute(self):
        ...
        self.env.revert_snapshot("step_8_run_controller_remaining_task")
        cluster_id = self.fuel_web.get_last_created_cluster()
        compute_ids = [
            n['id'] for n in
            self.fuel_web.client.list_cluster_nodes(cluster_id)
