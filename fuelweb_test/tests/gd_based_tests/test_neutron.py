...
class NeutronGre(TestBasic):
    ...
    @log_snapshot_on_error
    def step_8_run_top_role_controller(self):
        ...
        Snapshot: "step_8_run_top_role_controller"
        """
        self.check_run_by_group('step_8_run_top_role_controller',
                                'top_role_controller')
        ...
        self.env.make_snapshot("step_8_run_top_role_controller")

    @test(depends_on=[step_8_run_top_role_controller],
          groups=['top_role_compute'])
    @log_snapshot_on_error
    def step_9_run_top_role_compute(self):
        """Execute top-role-compute task on computes, create snapshot
          Depends:
          "Step 8 run top role controller

          Scenario:
            1. Revert snapshot "step_8_run_top_role_controller"
            2. Get cluster id
        ...
        self.env.revert_snapshot("step_8_run_top_role_controller")
        cluster_id = self.fuel_web.get_last_created_cluster()
        compute_ids = [
            n['id'] for n in
            self.fuel_web.client.list_cluster_nodes(cluster_id)
