...
class NeutronGre(TestBasic):
    ...
    def step_3_run_tasks_end_netconfig(self):
        ...
        if self.get_pre_test(tasks, 'netconfig'):
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=self.get_pre_test(tasks, 'netconfig')[0]['cmd'])
        ...
        hiera_node_maps = {}
        post_hiera = self.get_post_test(tasks, 'hiera')
        post_globals = self.get_post_test(tasks, 'globals')
        post_netconf = self.get_post_test(tasks, 'netconfig')
        for node in ['slave-01', 'slave-02', 'slave-03']:
            try:
                remote = self.fuel_web.get_ssh_for_node(node)
                if post_hiera:
                    gd.run_check_from_task(remote, post_hiera[0]['cmd'])
                gd.check_hiera_resources(remote=remote)
                hiera_node_maps[node] = gd.get_hiera_data(
                    remote=remote, data='node')
                if post_globals:
                    gd.run_check_from_task(remote, post_globals[0]['cmd'])
            except Exception:
                logger.error(
        ...
        for node in ['slave-01', 'slave-02', 'slave-03']:
            try:
                remote = self.fuel_web.get_ssh_for_node(node)
                if post_netconf:
                    gd.run_check_from_task(remote, post_netconf[0]['cmd'])
    ...
    def step_5_run_globals(self):
        ...
        tasks = self.fuel_web.client.get_end_deployment_tasks(
            cluster_id, end='globals')

        pre_global = self.get_pre_test(tasks, 'globals')
        post_global = self.get_post_test(tasks, 'globals')
        if pre_global:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_global[0]['cmd'])
             for node in ['slave-01', 'slave-02', 'slave-03']]
        ...
        for node in ['slave-01', 'slave-02', 'slave-03']:
            try:
                remote = self.fuel_web.get_ssh_for_node(node)
                if post_global:
                    gd.run_check_from_task(remote, post_global[0]['cmd'])
    ...
    def step_6_run_netconfig(self):
        ...
        pre_net = self.get_pre_test(tasks, 'netconfig')
        post_net = self.get_post_test(tasks, 'netconfig')
        if pre_net:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_net[0]['cmd'])
        ...
        for node in ['slave-01', 'slave-02', 'slave-03']:
            try:
                remote = self.fuel_web.get_ssh_for_node(node)
                if post_net:
                    gd.run_check_from_task(remote, post_net[0]['cmd'])
                gd.check_hiera_resources(remote=remote)
                hiera_node_maps[node] = gd.get_hiera_data(
                    remote=remote, data='node')
            except Exception:
    ...
    def step_7_run_tools(self):
        ...
        tasks = self.fuel_web.client.get_end_deployment_tasks(
            cluster_id, end='tools')

        pre_tools = self.get_pre_test(tasks, 'tools')
        post_tools = self.get_post_test(tasks, 'tools')
        if pre_tools:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_tools[0]['cmd'])
             for node in ['slave-01', 'slave-02', 'slave-03']]
        ...
        for node in ['slave-01', 'slave-02', 'slave-03']:
            try:
                remote = self.fuel_web.get_ssh_for_node(node)
                if post_tools:
                    gd.run_check_from_task(remote, post_tools[0]['cmd'])
    ...
    def step_8_run_firewall(self):
        ...
        tasks = self.fuel_web.client.get_end_deployment_tasks(
            cluster_id, end='firewall')

        pre_firewall = self.get_pre_test(tasks, 'firewall')
        post_firewall = self.get_post_test(tasks, 'firewall')
        if pre_firewall:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_firewall[0]['cmd'])
             for node in ['slave-01', 'slave-02', 'slave-03']]
        ...
        for node in ['slave-01', 'slave-02', 'slave-03']:
            try:
                remote = self.fuel_web.get_ssh_for_node(node)
                if post_firewall:
                    gd.run_check_from_task(remote,post_firewall[0]['cmd'])
            except Exception:
                logger.error(
                    'Fail in attempts to check firewall task execution on the slaves')
                logger.error(traceback.format_exc())
                raise
    ...
    def step_9_run_hosts(self):
        ...
        pre_hosts = self.get_pre_test(tasks, 'hosts')
        post_hosts = self.get_post_test(tasks, 'hosts')
        if pre_hosts:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_hosts[0]['cmd'])
             for node in ['slave-01', 'slave-02', 'slave-03']]
    ...
    def step_10_run_cluster_controller(self):
        ...
        #         raise
        func_name = utils.get_test_method_name()
        logger.debug('DDDDDDDDDDDDDDDDDDDDDDd {0}'.format(func_name))
    ...
    @log_snapshot_on_error
    def step_15_run_top_role_controller(self):
        ...
        Snapshot: "step_15_run_top_role_controller"
        """

        func_name = utils.get_test_method_name()
        logger.debug('!!! {0}'.format(func_name))
        ...
        self.env.make_snapshot("step_15_run_top_role_controller",
                               is_make=True)

    @test(depends_on=[step_9_run_hosts],
          groups=['top_role_compute'])
    @log_snapshot_on_error
    def step_16_run_top_role_compute(self):
        """Execute top-role-compute task on computes, create snapshot
          Depends:
          "Step 9 run hosts

          Scenario:
            1. Revert snapshot "step_9_run_hosts"
            2. Get cluster id
            4. Execute top-role-compute task on on computes
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot

        Snapshot: "step_16_run_top_role_compute"
        """
        ...
        self.env.make_snapshot("step_16_run_top_role_compute",
                               is_make=True)