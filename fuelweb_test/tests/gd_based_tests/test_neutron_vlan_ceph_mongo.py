...
class NeutronVlanCephMongo(TestBasic):
    ...
    def step_4_run_top_role_primary_mongo_single(self):
        ...
        primary_mongo = self.fuel_web.get_nailgun_primary_mongo(
    ...
    @test(depends_on=[step_9_run_openstack_primary_controller],
          groups=['primary_controller_remaining_task_ha'])
    @log_snapshot_on_error
    def step_10_run_primary_controller_remaining_task(self):
        """Execute controller_remaining_task task on primary controller
          Depends:
          "step_9_run_openstack_primary_controller
        ...
        Snapshot: "step_10_run_primary_controller_remaining_task"
        """
        self.check_run_by_group(
            'step_10_run_primary_controller_remaining_task',
            'primary_controller_remaining_task_ha')
        ...
        tasks = self.fuel_web.client.get_end_deployment_tasks(
            cluster_id, end='controller_remaining_task')

        pre_net = self.get_pre_test(tasks, 'openstack-network')
        post_net = self.get_post_test(tasks, 'openstack-network')
        if pre_net:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_net[0]['cmd'])
        ...
        pre_top_ctr = self.get_pre_test(tasks, 'controller_remaining_task')
        post_top_ctr = self.get_post_test(tasks, 'controller_remaining_task')
        if pre_top_ctr:
            gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(
                    primary_controller.name),
        ...
        res = self.fuel_web.client.put_deployment_tasks_for_cluster(
            cluster_id, data=['controller_remaining_task'],
            node_id='{0}'.format(pr_controller_id))
        logger.debug('res info is {0}'.format(res))
    ...
    @test(depends_on=[step_10_run_primary_controller_remaining_task],
    ...
    @test(depends_on=[step_15_run_openstack_controller],
          groups=['controller_remaining_task_ha'])
    @log_snapshot_on_error
    def step_16_run_controller_remaining_task(self):
        ...
        Snapshot: "step_16_run_controller_remaining_task"
        """
        self.check_run_by_group('step_16_run_controller_remaining_task',
                                'controller_remaining_task_ha')
        ...
        self.env.make_snapshot(
            "step_16_run_controller_remaining_task")

    @test(depends_on=[step_16_run_controller_remaining_task],
    ...
    def step_17_run_top_role_compute(self):
        ...
        self.env.revert_snapshot("step_16_run_controller_remaining_task")
        cluster_id = self.fuel_web.get_last_created_cluster()
        compute_ids = [
            n['id'] for n in
            self.fuel_web.client.list_cluster_nodes(cluster_id)
