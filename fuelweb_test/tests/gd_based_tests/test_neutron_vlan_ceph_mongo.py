    ...
    def step_10_run_primary_controller_remaining_tasks(self):
        """Execute controller_remaining_tasks task on primary controller
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute controller_remaining_tasks on primary controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot
        ...
        tasks = self.fuel_web.client.get_end_deployment_tasks(
            cluster_id, end='controller_remaining_tasks')
        ...
        pre_top_ctr = self.get_pre_test(tasks, 'controller_remaining_tasks')
        post_top_ctr = self.get_post_test(tasks, 'controller_remaining_tasks')
        if pre_top_ctr:
            gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(
                    primary_controller.name),
        ...
        res = self.fuel_web.client.put_deployment_tasks_for_cluster(
            cluster_id, data=['controller_remaining_tasks'],
            node_id='{0}'.format(pr_controller_id))
        logger.debug('res info is {0}'.format(res))
    ...
    def step_16_run_controller_remaining_tasks(self):
        """Execute controller_remaining_tasks task on controller
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute controller_remaining_tasks  on on controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot
