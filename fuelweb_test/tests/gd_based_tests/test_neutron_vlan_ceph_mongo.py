...
class NeutronVlanCephMongo(TestBasic):
    ...
    def step_2_run_tasks_env_with_hosts(self):
        ...
        """
        self.check_run_by_group('step_1_create_5_node_cluster_provision',
    ...
    @test(depends_on=[step_3_run_top_role_mongo_single],
          groups=['top_role_primary_mongo_single'])
    @log_snapshot_on_error
    def step_4_run_top_role_primary_mongo_single(self):
        ...
        mongo_primary_id = [
            n['id'] for n in
            self.fuel_web.client.list_cluster_nodes(cluster_id)
            if 'mongo-primary' in n['roles']]

        self.sync_manifest_to_the_slaves(
            cluster_id=cluster_id,
            node_ids=mongo_primary_id)

        task = self.fuel_web.client.put_deployment_tasks_for_cluster(
            cluster_id, data=['top-role-primary-mongo'],
            node_id=mongo_primary_id)
        ...
        mongo_pr_devops = self.fuel_web.find_devops_node_by_nailgun_fqdn(
            mongo_primary_fqdn[0])

        if self.get_post_test(tasks, 'top-role-primary-mongo'):
            gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(mongo_pr_devops.name),
    ...
    @test(depends_on=[step_4_run_top_role_primary_mongo_single],
          groups=['cluster_primary_controller'])
    @log_snapshot_on_error
    def step_5_run_cluster_primary_controller(self):
        """Execute cluster task on primary controller, create snapshot
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute cluster task on primary controller
    ...
    @test(depends_on=[step_5_run_cluster_primary_controller],
          groups=['virtual_ips_primary_controller'])
    @log_snapshot_on_error
    def step_6_run_virtual_ips_primary_controller(self):
        """Execute virtual_ips task on primary controller, create snapshot
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute virtual ips task on primary controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot
        ...
        self.check_run_by_group('step_6_run_virtual_ips_primary_controller',
                                'virtual_ips_primary_controller')
    ...
    @test(depends_on=[step_6_run_virtual_ips_primary_controller],
          groups=['cluster_haproxy_primary_controller'])
    @log_snapshot_on_error
    def step_7_run_cluster_haproxy_primary_controller(self):
        """Execute cluster-haproxy task on primary controller, create snapshot
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute cluster-haproxy task on primary controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot
        ...
        self.check_run_by_group(
            ...
            'cluster_haproxy_primary_controller')
    ...
    @test(depends_on=[step_7_run_cluster_haproxy_primary_controller],
          groups=['openstack_haproxy_primary_controller'])
    @log_snapshot_on_error
    def step_8_run_openstack_haproxy_primary_controller(self):
        """Execute openstack-haproxy task on primary controller
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute openstack-haproxy task on primary controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot
        ...
        self.check_run_by_group(
            ...
            'openstack_haproxy_primary_controller')
    ...
    @test(depends_on=[step_8_run_openstack_haproxy_primary_controller],
          groups=['openstack_primary_controller'])
    @log_snapshot_on_error
    def step_9_run_openstack_primary_controller(self):
        """Execute openstack-controller task on primary controller
          ...
          Scenario:
            ...
            "step_8_run_openstack_haproxy_primary_controller"
            2. Get cluster id
            4. Execute openstack-controller task on primary controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot
    ...
    @test(depends_on=[step_9_run_openstack_primary_controller],
          groups=['top_role_primary_controller'])
    @log_snapshot_on_error
    def step_10_run_top_role_primary_controller(self):
        """Execute top-role-controller task on primary controller
          Depends:
          "step_9_run_openstack_primary_controller
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute top-role-controller task on primary controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot

        Snapshot: "step_10_run_top_role_primary_controller"
        """
        self.check_run_by_group(
            'step_10_run_top_role_primary_controller',
            'top_role_primary_controller')
        ...
        tasks = self.fuel_web.client.get_end_deployment_tasks(
            cluster_id, end='top-role-controller')

        pre_top_ctr = self.get_pre_test(tasks, 'top-role-controller')
        post_top_ctr = self.get_post_test(tasks, 'top-role-controller')
        if pre_top_ctr:
            gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(
                    primary_controller.name),
                path=pre_top_ctr[0]['cmd'])

        res = self.fuel_web.client.put_deployment_tasks_for_cluster(
            cluster_id, data=['top-role-controller'],
            node_id='{0}'.format(pr_controller_id))
        logger.debug('res info is {0}'.format(res))
    ...
    @test(depends_on=[step_10_run_top_role_primary_controller],
    ...
    @test(depends_on=[step_15_run_openstack_controller],
          groups=['top_role_controller'])
    @log_snapshot_on_error
    def step_16_run_top_role_controller(self):
        """Execute top-role-controller task on controller, create snapshot
          Depends:
          ...
          Scenario:
            ...
            2. Get cluster id
            4. Execute top-role-controller task on on controller
            5. Verify that task was finished with success.
            6. Assert task execution
            7. Create snapshot

        Snapshot: "step_16_run_top_role_controller"
        """
        self.check_run_by_group('step_16_run_top_role_controller',
                                'top_role_controller')
        ...
        self.env.make_snapshot("step_16_run_top_role_controller",
                               is_make=True)


    @test(depends_on=[step_16_run_top_role_controller],
          groups=['top_role_compute'])
    @log_snapshot_on_error
    def step_17_run_top_role_compute(self):
        """Execute top-role-compute task on computes, create snapshot
          Depends:
          "step_16_run_top_role_controller

          Scenario:
            1. Revert snapshot "step_16_run_top_role_controller"
            2. Get cluster id
        ...
        self.env.revert_snapshot("step_16_run_top_role_controller")
        cluster_id = self.fuel_web.get_last_created_cluster()
        compute_ids = [
            n['id'] for n in
            self.fuel_web.client.list_cluster_nodes(cluster_id)
        ...
        tasks = self.fuel_web.client.get_end_deployment_tasks(
            cluster_id, end='top-role-compute')
    ...
    def step_19_run_post_deployment(self):
        ...
        res = self.fuel_web.client.put_deployment_tasks_for_cluster(
            cluster_id, data=['post_deployment'],
