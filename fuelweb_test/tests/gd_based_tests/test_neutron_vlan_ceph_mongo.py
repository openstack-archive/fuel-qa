...
class NeutronVlanCephMongo(TestBasic):
    ...
    @test(depends_on=[step_9_run_openstack_primary_controller],
          groups=['top_role_primary_controller_ha'])
    @log_snapshot_on_error
    def step_10_run_top_role_primary_controller(self):
        ...
        self.check_run_by_group(
            ...
            'top_role_primary_controller_ha')
        ...
        pre_swift = self.get_pre_test(tasks, 'swift')
        if pre_swift:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_swift[0]['cmd'])
             for node in ['slave-01']]
        pre_heat = self.get_pre_test(tasks, 'heat')
        if pre_heat:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=pre_heat[0]['cmd'])
        ...
        res = self.fuel_web.client.put_deployment_tasks_for_cluster(
            cluster_id, data=['swift'],
            node_id='{0}'.format(pr_controller_id))
        logger.debug('res info is {0}'.format(res))

        self.fuel_web.assert_task_success(task=res)
        post_swift = self.get_post_test(tasks, 'swift')

        if post_swift:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=post_swift[0]['cmd'])
             for node in ['slave-01']]
        res = self.fuel_web.client.put_deployment_tasks_for_cluster(
            cluster_id, data=['heat'],
            node_id='{0}'.format(pr_controller_id))
        self.fuel_web.assert_task_success(task=res)

        post_heat = self.get_post_test(tasks, 'heat')
        if post_heat:
            [gd.run_check_from_task(
                remote=self.fuel_web.get_ssh_for_node(node),
                path=post_heat[0]['cmd'])
             for node in ['slave-01']]
