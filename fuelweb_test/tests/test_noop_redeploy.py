...
class RedeployNoop(TestBasic):
    ...
    def redeploy_noop(self):
        ...
        remote_cmds = [
            ...
            "do sed -i \"s/puppet_manifest: \(.*\)$/puppet_manifest: "
            "\\1 --noop --logdest \/var\/log\/puppet_noop.json/g\" $taskfile; "
            "done",
        ...
        for command in remote_cmds:
            assert_true(self.ssh_manager.execute_on_remote(
                ...
                cmd=command
            )['exit_code'] == 0, 'master node: "%s"' % command)

        self.fuel_web.redeploy_cluster_changes_wait_progress(cluster_id, None)

        self.env.make_snapshot("redeploy_noop")
