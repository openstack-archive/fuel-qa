...
class RhelHA(TestBasic):
    ...
    def remove_old_compute_services(remote, hostname):
        cmd = ("source ~/openrc && "
               "for i in $(nova service-list | awk '/{0}/{print $2}'); do "
               "nova service-delete $i; done".format(hostname))
        result = remote.execute(cmd)
        LOGGER.debug(result)
        asserts.assert_equal(result['exit_code'], 0, 'Can not remove '
                                                     'old nova computes')
        ...
        cmd = ("source ~/openrc && "
               "for i in $(neutron agent-list | awk '/{0}/{print $2}'); do "
               "neutron agent-delete $i; done".format(hostname))
        result = remote.execute(cmd)
        LOGGER.debug(result)
        asserts.assert_equal(result['exit_code'], 0, 'Can not remove '
                                                     'old neutron agents')
