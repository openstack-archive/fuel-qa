...
class RhelHA(TestBasic):
    ...
    def wait_for_slave_provision(self, slave_name, timeout=10 * 60):
        wait(lambda: _tcp_ping(self.env.d_env.get_node(name=slave_name).
    ...
    def enable_rhel_repos(remote):
        ...
        if result['exit_code'] != 0:
            raise Exception('Enabling RHEL repos failed')
    ...
    def set_hostname(remote):
        ...
        LOGGER.debug(result)
        if result['exit_code'] != 0:
            raise Exception('Setting hostname for node failed')
    ...
    def backup_required_information(remote, ip):
        ...
        cmd = ("cd ~/ && mkdir rhel_backup && "
               ...
               "scp -r {0}:/var/lib/astute/nova .").format(ip)
        result = remote.execute(cmd)
        LOGGER.debug(result)
        if result['exit_code'] != 0:
            raise Exception("Can not back up required information "
                            "from node")
    ...
    def restore_information(ip, remote_admin, remote_slave):
        ...
        LOGGER.debug(result)
        if result['exit_code'] != 0:
            raise Exception("Can not restore ssh-keys and astute.yaml")
        ...
        LOGGER.debug(result)
        if result['exit_code'] != 0:
            raise Exception("Can not restore ssh-keys for nova")
    ...
    def set_repo_for_perestroika(remote):
        ...
        if result['exit_code'] != 0:
            ...
            LOGGER.debug(result)
            if result["exit_code"] != 0:
                msg = "Can not create config file for repo"
                LOGGER.debug(msg)
                raise Exception(msg)
