...
class RrhBase(TestBasic):
    """RRH-based compute tests base"""
    ...
    @staticmethod
    def connect_rrh_image(slave):
        """Upload RRH image into a target node.
    ...
    @staticmethod
    def register_rrh_subscription(remote):
        """Register RRH subscription.
...
@test(groups=["rrh", "rrh_ha_one_controller", "rrh.basic"])
class RrhHaOneController(RrhBase):
    """RRH-based compute HA One Controller basic test"""

    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["deploy_rrh_compute_ha_one_controller_tun"])
    @log_snapshot_after_test
    def deploy_rrh_compute_ha_one_controller_tun(self):
        """Deploy RRH-based compute in HA One Controller mode
        with Neutron VXLAN
...
@test(groups=["rrh", "rrh_ha", "rrh.basic"])
class RrhHA(RrhBase):
    """RRH-based compute HA basic test"""
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["deploy_rrh_compute_ha_tun"])
    @log_snapshot_after_test
    def deploy_rrh_compute_ha_tun(self):
        """Deploy RRH-based compute in HA mode with Neutron VXLAN
...
@test(groups=['rrh', 'rrh_ha', 'rrh.migration'])
class RrhHAUbuntuMigration(RrhBase):
    """RRH-based compute HA migration test"""
    @test(depends_on=[SetupEnvironment.prepare_slaves_7],
          groups=["check_vm_migration_rrh_ubuntu_ha_tun"])
    def check_vm_migration_rrh_ubuntu_ha_tun(self):
