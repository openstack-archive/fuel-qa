...
class RhelHA(TestBasic):
    ...
    def register_rhel_subscription(remote):
        ...
        reg_command = (
            ...
            "--org={3} --release={4} --auto-attach".format(
    ...
    def set_repo_for_perestroika(remote):
        ...
        cmd = ("echo '[mos]\n"
               ...
               "yum install yum-utils yum-priorities -y && "
    ...
    def remove_old_compute_services(remote, hostname):
        cmd_first_part = ("source ~/openrc && "
                          "for i in $(nova service-list | "
                          "awk '/{0}/".format(hostname))
        cmd_second_part = "{print $2}'); do nova service-delete $i; done"
        # NOTE(freerunner): Separate command into two parts due to python
        # interprets awk command {print $2} as formatter.
        ...
        cmd_first_part = ("source ~/openrc && "
                          "for i in $(neutron agent-list | "
                          "awk '/{0}/".format(hostname))
        cmd_second_part = "{print $2}'); do neutron agent-delete $i; done"
        cmd = cmd_first_part + cmd_second_part
        result = remote.execute(cmd)
        LOGGER.debug(result)
        asserts.assert_equal(result['exit_code'], 0, 'Can not remove '
