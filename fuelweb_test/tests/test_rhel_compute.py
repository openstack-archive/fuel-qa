...
class RhelHA(TestBasic):
    ...
    def set_hostname(remote):
        ...
        :param remote: Remote node for proceed
        """
        cmd = "sysctl kernel.hostname=rhel.test.domain.local"
    ...
    def apply_compute_puppet(remote):
        puppet_run = (
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/hiera/hiera.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/globals/globals.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/firewall/firewall.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/tools/tools.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/netconfig/netconfig.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/roles/compute.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/openstack-network/common-config.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/openstack-network/plugins/ml2.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/openstack-network/agents/l3.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/openstack-network/agents/metadata.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/openstack-network/compute-nova.pp; "
            "puppet apply -vd --logdest syslog /etc/puppet/modules/"
            "osnailyfacter/modular/astute/enable_compute.pp")

        LOGGER.debug("Applying puppets...")
        result = remote.execute(puppet_run)
