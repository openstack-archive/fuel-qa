...
class VcenterDeploy(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["vcenter_vlan_dualhv"])
    @log_snapshot_on_error
    def vcenter_vlan_dualhv(self):
        """Deploy enviroment of vcenter+qemu with nova vlan and CephOSD backend
           for glance.
        ...
        self.fuel_web.update_nodes(
            ...
            {'slave-01': ['controller'],
             'slave-02': ['controller'],
             'slave-03': ['controller'],
             'slave-04': ['cinder', 'compute'],
             'slave-05': ['cinder-vmware', 'compute'], })
        ...
        # FIXME when OSTF test will be fixed in bug #
        ...
        self.fuel_web.run_ostf(
            cluster_id=cluster_id, test_sets=['smoke', 'sanity', 'ha'],
            should_fail=1,
            failed_test_name=[('Launch instance with file injection')])
