...
class TestUseMirror(TestBasic):
    ...
    def _fix_fuel_mirror_config(self, admin_ip, cluster_id):
        ...
        cmd = 'cp {0} {0}.old'.format(cfg_path)
        self.ssh_manager.execute_on_remote(admin_ip, cmd)

        self.ssh_manager.download_from_remote(admin_ip, cfg_path, '.')
        with open('ubuntu.yaml', 'r') as f:
            config = yaml.safe_load(f)
        config['repos'] = cluster_repos['all']
        config['groups']['ubuntu'] = cluster_repos['ubuntu']
        with open('ubuntu.yaml', 'w') as f:
            yaml.safe_dump(config, f)
        self.ssh_manager.upload_to_remote(admin_ip, 'ubuntu.yaml', cfg_path)
    ...
    def deploy_with_custom_mirror(self):
        ...
        self.show_step(5)
        self._fix_fuel_mirror_config(admin_ip, cluster_id)
        cluster_repos = self._get_cluster_repos(cluster_id)
