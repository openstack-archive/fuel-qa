    ...
    def admin_reboot_and_wait(self,
        ...
        if not reboot:
            logger.info(
                ...
                "REBOOT_MASTER_AFTER_UPDATE={}".format(
                    settings.REBOOT_MASTER_AFTER_UPDATE))
            return
        admin_devops = self.env.d_env.nodes().admin
        admin = self.ssh_manager.get_remote(self.ssh_manager.admin_ip)
        uptime_old = get_process_uptime(admin, 'systemd')
        try:
            self.fuel_web.warm_restart_nodes([admin_devops])
        except Exception:
            logger.info("Reboot failed. Hard resetting admin node",
                        exc_info=True)
            self.fuel_web.cold_restart_nodes([admin_devops])
