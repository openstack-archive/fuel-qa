...
class TestBasic(object):
    ...
    def fuel_post_install_actions(self,
        ...
        if settings.FORCE_DISABLE_UPDATES and settings.UPDATE_MASTER:
            logger.warning(
                "Cannot use FORCE_DISABLE_UPDATES and UPDATE_MASTER "
                "at the same time.\nFORCE_DISABLE_UPDATES have higher priority"
                " so UPDATE_MASTER will be unset")
            settings.UPDATE_MASTER = False
        if settings.FORCE_DISABLE_UPDATES:
            ...
            admin_ip = self.env.d_env.get_admin_node_ip()
