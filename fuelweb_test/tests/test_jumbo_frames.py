...
class TestJumboFrames(base_test_case.TestBasic):
    ...
    def ping_instance_from_instance(self, source_instance,
        ...
        with self.fuel_web.get_ssh_for_node("slave-01") as ssh:
            command = "ping -c {0} -s {1} {2}".format(count, size,
                                                      destination_ip)
            logger.info(
    ...
    def check_mtu_size_between_instances(self, mtu_offset, diff_net=False):
        ...
        Scenario:
            1. Boot one instances on first compute host
            2. Create new network
            3. Create subnet for it
            4. Add created subnet to router
            5. Boot one instances on second compute host and created subnet
            6. Ping one instance from another
            7. Delete instances
        ...
        if diff_net:
            ...
            net1 = self.os_conn.create_network(net_destination)
        ...
        self.os_conn.wait_for_server_status_active(destination_instance)
    ...
    def prepare_5_slaves_with_jumbo_frames(self):
        ...
        logger.info(
            "Found {0} interfaces for update: {1}".format(
        ...
        for iface in bridge_interfaces:
            ...
            logger.debug("New {0} interface properties:\n"
                         "{1}".format(iface, self.get_host_iface(iface)))
