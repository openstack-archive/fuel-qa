    ...
    def deploy_cluster_with_dpdk_bond(self):
        ...
        Scenario:
            ...
            4. Setup bonding for all interfaces (including admin interface
               bonding)
            5. Enable DPDK for bond with private network on all computes except
               one node
            6. Run network verification
        ...
        asserts.assert_true(settings.BONDING,
                            'Bonding is disabled! Aborting test...')

        asserts.assert_true(settings.KVM_USE,
                            'KVM is disabled! Aborting test...')
        ...
        for node in computes:
            ...
            #skip slave-03, don't enable DPDK on tha node
            if 'slave-03' in node['name']:
                continue
