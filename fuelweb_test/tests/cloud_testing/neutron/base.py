...
class TestNeutronBase(test_neutron_base.TestNeutronFailoverBase):
    ...
    def check_ping_between_nodes(self, ip, servers, keypair, by_floating=True):
        """Make ping between each servers pair

        :param ip: node ip from which will attempt connect to vm
        :param servers: list of servers to check
        :param keypair: RSA keypair (each server should be created with it)
    ...
    def test_ban_l3_agent(self, ban_count=1):
