...
class RadosGW(TestBasic):
    ...
    def radosgw_without_os_services_usage(self):
        ...
        self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd='radosgw-admin user create --uid="s3_main" '
                '--display-name="s3_main"')['stdout']

        keys = self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd='radosgw-admin user create '
                '--uid="s3_mainn" --display-name="s3_mainn"',
        ...
        self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd="git clone {0} /tmp/tool".format(settings.REPO_S3_TOOL))
