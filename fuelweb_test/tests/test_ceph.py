...
class CephRadosGW(TestBasic):
    ...
    def ceph_rados_gw(self):
        ...
        assert_true(checkers.check_radosgw_started(_ip),
...
class RadosGW(TestBasic):
    ...
    def radosgw_without_os_services_usage(self):
        ...
        Scenario:
            ...
            10. Compare custom image IDs via glance and swift client
            11. Check S3 API
        ...
        assert_true(checkers.check_radosgw_started(node['ip']),
        ...
        self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd="apt-get install -y git libdigest-hmac-perl")

        self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd="git clone {0} /tmp/tool && cd /tmp/tool && "
                "git checkout {1}".format(
                    settings.REPO_S3_TOOL,
                    "5a1c21d1a5a2080d4cb29a1954bd17586dddb92d"))

        pub_contr_ip = self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd="ip -o -4 addr show br-ex | "
                "awk {'print $4'}")['stdout'][0].split('/')[0]

        config_path = "/tmp/tool/.s3curl"

        self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd="echo -e '%awsSecretAccessKeys = (rados => "
                "{{id =>"'"{0}"'",  key =>"'"{1}"'",}},);'"
                ">{2}".format(access_key, secret_key, config_path))

        self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd="chmod 600 {0}".format(config_path))

        result = self.ssh_manager.execute_on_remote(
            ip=node['ip'],
            cmd="/tmp/tool/s3curl.pl --id=rados --endpoint --head -- -v "
                "http://{0}:6780".format(pub_contr_ip))

        if 'HTTP/1.1 200 OK' not in ''.join(result['stdout']):
            raise Exception(
