...
class RhBase(TestBasic):
    ...
    def register_rh_subscription(self, ip):
        ...
        if settings.RH_POOL_HASH:
            result = self.ssh_manager.execute(ip, reg_command)
            logger.debug(result)
            ...
            result = self.ssh_manager.execute(ip, reg_pool_cmd)
            logger.debug(result)
            asserts.assert_equal(result['exit_code'], 0,
    ...
    def apply_networking_puppet(self, ip):
        ...
        if result['exit_code'] == 0:
            ...
            result = self.ssh_manager.execute(ip, remove_iface)
            logger.debug(result)
        prep = "screen -dmS netconf"
        result = self.ssh_manager.execute(ip, prep)
        logger.debug(result)
        ...
        result = self.ssh_manager.execute(ip, net_puppet)
    ...
    def restore_information(self, ip, remote_admin_ip, ceph=False, node=1):
        ...
        result = self.ssh_manager.execute(remote_admin_ip, cmd)
        key = result['stdout']
        logger.debug(result)
        asserts.assert_equal(result['exit_code'], 0,
    ...
    def install_ruby_puppet(self, ip):
        ...
        result = self.ssh_manager.execute(ip, puppet_install_cmd)
        logger.debug(result)
        asserts.assert_equal(result['exit_code'], 0,
    ...
    def rsync_puppet_modules(self, master_node_ip, ip):
        ...
        cmd = ("rsync -avz /etc/puppet/modules/* "
               "{0}@{1}:/etc/puppet/modules/".format(settings.RH_IMAGE_USER,
                                                     ip))
        result = self.ssh_manager.execute(master_node_ip, cmd)
        logger.debug(cmd)
        asserts.assert_equal(result['exit_code'], 0,
