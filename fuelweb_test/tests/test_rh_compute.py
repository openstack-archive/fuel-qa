...
class RhBase(TestBasic):
    ...
    def check_instance_connectivity(remote, dhcp_namespace, instance_ip,
        ...
        asserts.assert_true(dhcp_namespace in namespaces,
        ...
        asserts.assert_equal(0, res['exit_code'],
...
@test(groups=['rh', 'rh.ha', 'rh.migration'])
class RhHAUbuntuMigration(RhBase):
    """RH-based compute HA migration test"""
    @test(depends_on=[SetupEnvironment.prepare_slaves_9],
          groups=["check_vm_migration_rh_ubuntu_ha_tun"])
    def check_vm_migration_rh_ubuntu_ha_tun(self):
        ...
        srv = os.create_server_for_migration(
            ...
            label=net_name)
        LOGGER.info("Srv is currently in status: {0}".format(srv.status))
        ...
        LOGGER.info("Server is on host {0}".format(srv_host))
        ...
        LOGGER.info("Server is now on host {0}".format(
        ...
        LOGGER.info("out of mounting volume is: {0}".format(out['stdout']))
        ...
        self.env.make_snapshot("check_vm_migration_rh_ubuntu_ha_tun")


@test(groups=['rh', 'rh.ha', 'rh.reboot'])
class RrhHAUbuntuReboot(RhBase):
    """RRH-based compute reboot"""
    @test(depends_on=[SetupEnvironment.prepare_slaves_7],
          groups=["check_rrh_node_work_after_reboot"])
    def check_rrh_node_work_after_reboot(self):
        """Deploy environment with RRH and Ubuntu computes in HA mode with
           neutron VXLAN

        Scenario:
            1. Check required image.
            2. Revert snapshot 'ready_with_7_slaves'.
            3. Create a Fuel cluster.
            4. Update cluster nodes with required roles.
            5. Deploy the Fuel cluster.
            6. Run OSTF.
            ...
            10. Execute modular tasks for compute.
            11. Run OSTF
            12. Create server on Rhel-compute
            13. Reboot Rhel node
            14. Check VMs can access Internet, and we can access VM via
                Floating IP
        ...
        target_node_name = compute['name'].split('_')[0]
        LOGGER.debug('Target node name: {0}'.format(target_node_name))
        target_node_hostname = compute['fqdn']
        LOGGER.debug('Target node hostname: {0}'.format(target_node_hostname))
        target_node = self.env.d_env.get_node(name=target_node_name)
        LOGGER.debug('DevOps Node: {0}'.format(target_node))
        target_node_ip = self.fuel_web.get_nailgun_node_by_name(
            target_node_name)['ip']
        ...
        os_conn.create_flavor(name='ephemeral{0}'.format(flavor_id),
                              ...
                              disk=1,
                              flavorid=flavor_id)
        LOGGER.info("Create a new server on Rh compute")
        srv = os.create_server(name, net_id, security_group, flavor_id)
        floating_ip = os.assign_floating_ip(srv)
        LOGGER.info("Srv is currently in status: {0}".format(srv.status))

        self.show_step(13)

        self.fuel_web.warm_restart_nodes([target_node])
        self.fuel_web.assert_cluster_ready(os_conn, smiles_count=13)
        ...
        os_con = os_actions.OpenStackActions(
            self.fuel_web.get_public_vip(cluster_id))
        instance_keypair = os_con.create_key(key_name='instancekey')
        instance_ip = self.create_instance_with_keypair(
            os_con, instance_keypair.name,
