...
class IacGit(TestBasic):
    ...
    def iac_git_download_settings(self):
        ...
        test_iac_git_extension/cluster_iac_git_download_settings.yaml'
        target = '/root/project.git/cluster_iac_git_download_settings.yaml'
        ...
        self.ssh_manager.check_call(
            ...
            expected=[0],
            error_info='Repository check failed')
        ...
        self.ssh_manager.check_call(
            ...
            expected=[0],
            error_info='Configuration check failed')
    ...
    def iac_git_install_remove_package(self):
        ...
        self.ssh_manager.check_call(
            ...
            command='git config --global user.email iac@mirantis.com')
        source = 'fuelweb_test/tests/\
        test_iac_git_extension/cluster_iac_git_install_remove_package.yaml'
        target = '/root/project.git/cluster_iac_git_download_settings.yaml'
        self.ssh_manager.upload_to_remote(ip=admin_ip,
                                          source=source,
                                          target=target)
        ...
        self.ssh_manager.check_call(
            verbose=True,
            ip=admin_ip,
            command='ssh node-1 "dpkg -l mc"',
            expected=[1],
            error_info='Package check failed')
        logger.info('Check if "virt-what" package installed before re-deploy:')
        self.ssh_manager.check_call(
            verbose=True,
            ip=admin_ip,
            command='ssh node-1 "dpkg -l virt-what"',
            expected=[0],
            error_info='Package check failed')
    ...
    def iac_git_set_non_openstack_settings(self):
        ...
        self.ssh_manager.check_call(
            ...
            command='git config --global user.email iac@mirantis.com')
        source = 'fuelweb_test/tests/\
        test_iac_git_extension/cluster_iac_git_set_non_openstack_settings.yaml'
        target = '/root/project.git/cluster_iac_git_download_settings.yaml'
        self.ssh_manager.upload_to_remote(ip=admin_ip,
                                          source=source,
                                          target=target)
