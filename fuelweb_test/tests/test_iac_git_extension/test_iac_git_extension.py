...
class IacGit(TestBasic):
    ...
    def iac_git_download_settings(self):
        ...
        self.ssh_manager.check_call(
            ip=admin_ip, command='mkdir -p /root/project.git/node_1_configs')
        self.ssh_manager.check_call(
            ip=admin_ip, command='mkdir -p /root/project.git/node_2_configs')

        source = 'fuelweb_test/tests/test_iac_git_extension/cluster.yaml'
        ...
        self.ssh_manager.check_call(
            ip=admin_ip,
            command='ssh node-2 "cat /etc/nova/nova.conf \
