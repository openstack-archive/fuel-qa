...
class TestMultipath(base_test_case.TestBasic):
    ...
    def bootstrap_multipath(self):
        ...
        self.show_step(3)
        count = self.check_multipath_devices(ip)
        assert_true(count == int(settings.SLAVE_MULTIPATH_DISKS_COUNT),
                    "Wrong multipath count: {0}. Must be {1}".format(
                        count, settings.SLAVE_MULTIPATH_DISKS_COUNT))
