...
@test(groups=["deploy_reset_five_controllers_neutron"])
class HAFiveControllerNeutronRestart(TestBasic):
    """HAFiveControllerNeutronRestart."""  # TODO documentation

    @test(depends_on=[SetupEnvironment.prepare_slaves_all],
          groups=["deploy_reset_five_controllers_neutron"])
    @log_snapshot_after_test
    def deploy_reset_five_controllers_neutron(self):
        """
        ...
        2. Verify networks
        3. Run OSTF tests
        4. Reset cluster, Change openstack username, password,
           tenant and re-deploy
        ...
        Duration 120m
        Snapshot deploy_reset_five_controllers_neutron
        ...
        cluster_id = self.fuel_web.create_cluster(
            ...
            settings={
                ...
                "net_provider": 'neutron',
                "net_segment_type": NEUTRON_SEGMENT['vlan'],
                'tenant': 'anotherSimpleVlan',
                'user': 'anotherSimpleVlan',
                'password': 'anotherSimpleVlan'
            }
        )
        ...
        self.env.make_snapshot("deploy_reset_five_controllers_neutron")
