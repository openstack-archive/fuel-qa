...
class Load(TestBasic):
    ...
    def load_ceph_partitions_cold_reboot(self):
        ...
        #         file_name = "test_data"
        #         file_path = os.path.join('/var/lib/mysql/', file_name)
        #         # create empty file, size:15G
        #         result = remote.execute(
        #             'fallocate -l 15G {0}'.format(file_path))['exit_code']
        #         assert_equal(result, 0, "The file {0} was not "
        #                                 "allocated".format(file_name))
        #         # push the file on ceph storages
        #         wait(lambda: (remote.execute('rados -p images put {0} {1}'.
        #                              format(file_name, file_path))
        #               ['exit_code']) == 0, timeout=600)
