...
class FailoverGroup3(TestBasic):
    ...
    def shutdown_ceph_for_all(self):
        ...
        volume_size = int(ceph_size // 1024) + 1
        self.create_ceph_volume(cluster_id=cluster_id, volume_size=volume_size)
        self.create_ceph_volume(cluster_id=cluster_id, volume_size=volume_size)

        file_size = int(ceph_size * 0.15)
        md5s = dict()
        md5s[instance_1.ip] = self.mount_volume_and_create_bigfile(
            instance_ip=instance_1.ip,
            file_size=file_size
        )
        md5s[instance_2.ip] = self.mount_volume_and_create_bigfile(
            instance_ip=instance_2.ip,
            file_size=file_size
        )
    ...
    def power_outage_ceph_for_all(self):
        ...
        self.show_step(4)
        n_nodes = self.fuel_web.client.list_cluster_nodes(cluster_id=cluster_id)
