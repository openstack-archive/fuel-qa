...
class FailoverGroup3(TestBasic):
    ...
    def shutdown_ceph_for_all(self):
        ...
        for node in ceph_nodes:
            ...
            total_disk_size += float(self.ssh_manager.execute_on_remote(
                ip=ip,
                cmd="df -h |grep {0} | awk '{{printf($2)}}'".format(device)
            )['stdout'][0][:-1])
            total_used += float(self.ssh_manager.execute_on_remote(
                ip=ip,
                cmd="df -h |grep {0} | awk '{{printf($3)}}'".format(device)
            )['stdout'][0][:-1])

        thirty_percent = 0.3 * total_disk_size[:-1]
    ...
    def shutdown_cinder_cluster(self):
        ...
        logger.info('USED: {}; Need to fill: {}; To Fill: {}'.format(
            used, need_to_fill, [volume_size, to_fill_mb]))
        ...
        logger.info('SLEEP')  # TODO need to delete
        time.sleep(1800)
