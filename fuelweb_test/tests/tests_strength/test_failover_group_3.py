...
class FailoverGroup3(TestBasic):
    ...
    def shutdown_cinder_cluster(self):
        ...
        first_server = os.nova.servers.get(instance_1.id)
        ...
        logger.info("Wait for ping from instance {}".format(instance_2.id))

        second_server = os.nova.servers.get(instance_2.id)
        ...
        volume_1 = os.create_volume(size=to_fill+1)
        volume_2 = os.create_volume(size=to_fill+1)

        logger.info([volume_1.id, volume_2.id])

        ip = self.fuel_web.get_nailgun_node_by_name("slave-01")['ip']

        # cmd = r"cinder create {size} --display-name {name}" \
        #       " --availability-zone nova"
        # self.ssh_manager.execute(
        #     ip=ip,
        #     cmd='. openrc; ' + cmd.format(size=to_fill, name='first_volume')
        # )
        # self.ssh_manager.execute(
        #     ip=ip,
        #     cmd='. openrc; ' + cmd.format(size=to_fill, name='second_volume')
        # )
        #
        # cmd = "cinder list|grep {name}|awk {{'print$2'}}"
        #
        # volume_1 = self.ssh_manager.execute(
        #     ip=ip,
        #     cmd='. openrc; ' + cmd.format(name='first_volume')
        # )['stdout'][0].strip()
        #
        # volume_2 = self.ssh_manager.execute(
        #     ip=ip,
        #     cmd='. openrc; ' + cmd.format(name='second_volume')
        # )['stdout'][0].strip()
