...
class FailoverGroup3(TestBasic):
    ...
    def shutdown_cinder_cluster(self):
        ...
        ip = os.get_nova_instance_ip(instance, net_name=net_name,
                                     type='fixed')
        logger.info("Instance {0} has IP {1}".format(instance.id, ip))
        ip = os.assign_floating_ip(instance)
        ...
        first_ip = os.get_nova_instance_ip(
            first_server, net_name=net_name, type='fixed')

        second_ip = os.get_nova_instance_ip(
            second_server, net_name=net_name, type='fixed')
        ...
        with self.fuel_web.get_ssh_for_node("slave-01") as ssh:
            ...
            cmd2 = 'ls -la /dev'
            try:
                os.execute_through_host(ssh, first_ip, cmd, creds)
                os.execute_through_host(ssh, first_ip, cmd2, creds)

                os.execute_through_host(ssh, second_ip, cmd, creds)
                os.execute_through_host(ssh, second_ip, cmd2, creds)
