...
class FailoverGroup3(TestBasic):
    ...
    def shutdown_ceph_for_all(self):
        ...
        for node in ceph_nodes:
            ...
            sizes = self.ssh_manager.execute_on_remote(
                ip=ip,
                cmd="df -h |grep {0}".format(device)
            )['stdout'][0].split()

            total_disk_size += \
                float(sizes[1][:-1 if sizes[1].endswith('G') else -2])
            total_used += \
                float(sizes[2][:-1 if sizes[2].endswith('G') else -2])

        thirty_percent = 0.3 * total_disk_size
        need_to_fill = int((thirty_percent - total_used) * 1024)
        to_fill_mb = need_to_fill / 2
        volume_size = int(round(to_fill_mb / 1024)) + 1
    ...
    def shutdown_cinder_cluster(self):
        ...
        disk_size = float(sizes[1][:-1 if sizes[1].endswith('G') else -2])
        used = float(sizes[2][:-1 if sizes[2].endswith('G') else -2])

        thirty_percent = 0.3 * disk_size
        used = used
        need_to_fill = int((thirty_percent - used) * 1024)
        to_fill_mb = need_to_fill / 2
        volume_size = int(round(to_fill_mb / 1024.0)) + 1

        logger.info('30% is {}'.format(thirty_percent))
        logger.info('USED: {}Gi; Need to fill: {}Mb; To Fill: {}Mb'.format(
            used, need_to_fill, to_fill_mb))
