...
class FailoverGroup3(TestBasic):
    ...
    def shutdown_cinder_cluster(self):
        ...
        first_srv = os.create_server_for_migration(
            neutron=True,
            scenario='./fuelweb_test/helpers/instance_initial_scenario',
            label=net_name)
        second_srv = os.create_server_for_migration(
            neutron=True,
            scenario='./fuelweb_test/helpers/instance_initial_scenario',
            label=net_name)

        logger.info("Srv_1 is in status: {}".format(first_srv.status))
        logger.info("Srv_2 currently in status: {}".format(second_srv.status))
        logger.info('Create volumes')
        first_volume = os.create_volume(size=3)
        second_volume = os.create_volume(size=3)

        logger.info('Attach volumes')
        os.attach_volume(first_volume, first_srv)
        os.attach_volume(second_volume, second_srv)

        first_floating_ip = os.assign_floating_ip(first_srv, True)
        second_floating_ip = os.assign_floating_ip(second_srv, True)

        floating_ips = [first_floating_ip, second_floating_ip]
