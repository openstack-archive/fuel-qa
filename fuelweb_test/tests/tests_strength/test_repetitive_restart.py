...
from fuelweb_test.helpers.utils import fill_ceph_partitions
...
class RepetitiveRestart(TestLoadBase):
    ...
    def ceph_partitions_repetitive_cold_restart(self):
        ...
        Scenario:
            1. Prepare cluster in HA mode with ceph for load tests or
            revert snapshot 'prepare_load_ceph_ha' if it already exists
        ...
        ceph_nodes = self.fuel_web.get_nailgun_cluster_nodes_by_roles(
            cluster_id, ['ceph-osd'])
        fill_ceph_partitions(ceph_nodes=ceph_nodes, gb=30)
