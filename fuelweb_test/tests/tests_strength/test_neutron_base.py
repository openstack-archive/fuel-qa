...
class TestNeutronFailoverBase(base_test_case.TestBasic):
    ...
    def neutron_l3_migration_after_reset(self):
        ...
        wait(lambda: self.fuel_web.get_nailgun_node_by_devops_node(
             ...
             timeout_msg=err_msg.format(node=new_devops))
    ...
    def neutron_l3_migration_after_destroy(self):
        ...
        wait(lambda: self.get_node_with_dhcp(os_conn, net_id), timeout=60 * 3,
             timeout_msg=err_msg.format(node=devops_node, net=net_id))
        new_devops_node = self.get_node_with_dhcp(os_conn, net_id)
