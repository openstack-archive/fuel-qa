...
class CICMaintenanceMode(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["cic_maintenance_mode"])
    @log_snapshot_on_error
    def cic_maintenance_mode(self):
    ...
    @test(depends_on=[cic_maintenance_mode],
    ...
    def cic_maintenance_mode_single_node(self):
        ...
        pcm_nodes = ' '.join(self.fuel_web.get_pcm_nodes(
            self.env.d_env.nodes().slaves[0], pure=True)['Online'])
        ...
        for nailgun_node in self.env.d_env.nodes().slaves[0:3]:
            ...
            try:
                ...
                logger.debug('Node %s do not become offline after '
                             'switching maintenance mode', nailgun_node.name)
            ...
            try:
                ...
                logger.debug('Node %s do not become online after exiting '
                             'maintenance mode', nailgun_node.name)
            ...
            try:
                ...
                logger.debug('Rabbitmq not ready')
    ...
    def cic_maintenance_mode_single_node_auto(self):
        ...
        for nailgun_node in self.env.d_env.nodes().slaves[0:3]:
            ...
            try:
                ...
                logger.debug('Node %s do not become offline after unexpected '
            ...
            try:
                ...
                logger.debug('Node %s do not become online after unexpected '
                             'reboot', nailgun_node.name)
            ...
            try:
                ...
                logger.debug('Node %s do not become online after '
                             'reboot', nailgun_node.name)
