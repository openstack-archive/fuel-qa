...
class CICMaintenanceMode(TestBasic):
    ...
    def manual_cic_maintenance_mode(self):
        ...
        logger.info('Wait a node-{0} offline status after switching '
        ...
        try:
            ...
            asserts.assert_false(
                ...
                'switching maintenance mode'.format(_id))
        ...
        try:
            ...
            asserts.assert_true(
                ...
                'exiting maintenance mode'.format(_id))
        ...
        _wait(lambda:
              self.fuel_web.run_single_ostf_test(
                  ...
                  test_name=ostf_test_mapping.OSTF_TEST_MAPPING.get(
                      'Check that required services are running'), retries=2),
              timeout=1500)
    ...
    def auto_cic_maintenance_mode(self):
        ...
        _wait(lambda:
              self.fuel_web.run_single_ostf_test(
                  cluster_id, test_sets=['ha'],
                  test_name=ostf_test_mapping.OSTF_TEST_MAPPING.get(
                      'RabbitMQ availability'), retries=2),
              timeout=1500)
        logger.info('RabbitMQ cluster is available')
