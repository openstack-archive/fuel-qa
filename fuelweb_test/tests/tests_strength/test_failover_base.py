...
class TestHaFailoverBase(TestBasic):
    ...
    def check_alive_rabbit_node_not_kicked(self):
        ...
        with self.env.d_env.get_admin_remote() as remote:
            cmd = ("grep -P 'Ignoring alive node rabbit@\S*\b{0}\b' /var/log/"
                   "remote/{1}/rabbit-fence.log").format(rabbit_slave1_name,
                                                         pcm_nodes[0])
            try:
                wait(
    ...
    def check_dead_rabbit_node_kicked(self):
        ...
        with self.env.d_env.get_admin_remote() as remote:
            ...
            cmd = ("grep -P 'Forgetting cluster node rabbit@\S*\b{0}\b' /var/l"
                   "og/remote/{1}/rabbit-fence.log").format(rabbit_slave1_name,
                                                            pcm_nodes[0])
            try:
                wait(
                    lambda: not remote.execute(cmd)['exit_code'],
            ...
            assert_equal(0, remote.execute(
                "grep -P 'Got rabbit@\S*\b{0}\b that left cluster' /var/log/re"
                "mote/{1}/rabbit-fence.log".format(slave1_name,
                                                   pcm_nodes[0]))['exit_code'],
            ...
            assert_equal(0, remote.execute(
                ...
                "Node {} wasn't prepared for fencing".format(
                    rabbit_slave1_name))
            assert_equal(0, remote.execute(
                "grep -P 'Disconnecting node rabbit@\S*\b{0}\b' /var/log/remot"
                "e/{1}/rabbit-fence.log".format(rabbit_slave1_name,
                                                pcm_nodes[0]))['exit_code'],
