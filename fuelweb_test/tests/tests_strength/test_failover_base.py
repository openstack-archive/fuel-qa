...
class TestHaFailoverBase(TestBasic):
    ...
    def ha_corosync_stability_check(self):
        ...
        with self.fuel_web.get_ssh_for_node(
            ...
            live_nodes = [node for node in ctrl_nodes
            ...
            for count in xrange(500):
                logger.debug('Count number: {0}'.format(count))
                _wait(
                    lambda: assert_equal(
                ...
                _wait(
                    lambda: assert_true(
                        _check_all_pcs_nodes_status(
                            ...
                            'Offline'), 'Caught splitbrain at count-{0}'.
                        format(count)), timeout=20)
                _wait(
                    lambda: assert_equal(
                        remote_controller.execute(
                            'service corosync start && service pacemaker '
                            'restart')['exit_code'], 0, 'Corosync was not '
                                                        'started'), timeout=20)
                _wait(
                    lambda: assert_true(
                        _check_all_pcs_nodes_status(
