...
class TestHaFailoverBase(TestBasic):
    ...
    def check_alive_rabbit_node_not_kicked(self):
        ...
        slave1_name = ''.join(slave1_remote.execute('hostname')['stdout'])
        ...
        slave2_remote = self.fuel_web.get_ssh_for_node(
            self.env.d_env.nodes().slaves[1].name)
        rabbit_status = slave2_remote.execute('rabbitmqctl'
        ...
        self.fuel_web.assert_pacemaker(self.env.d_env.nodes().slaves[0],
