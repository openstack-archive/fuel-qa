...
class TestHaFailoverBase(TestBasic):
    ...
    def change_pacemaker_parameter_not_break_rabbitmq(self):
        err_msg = 'Cannot execute command {}. Timeout exceeded'
        self.env.revert_snapshot(self.snapshot_name)
        ...
        for count in xrange(REPEAT_COUNT):
            ...
            with self.fuel_web.get_ssh_for_node(
                ...
                _wait(lambda: assert_equal(
                    remote_master_rabbit.execute(cmd)['exit_code'], 0,
                    'Fail to delete pcs constraint {0} on count {1}'.format(
                        remote_master_rabbit.execute(cmd), count)),
                      timeout=20)

                # Move stopped resource to rabbit slave
                ...
                cmd_clear = ('pcs constraint delete '
                             'location-p_rabbitmq-server 2>&1 >/dev/null')
                _wait(lambda: assert_equal(
            ...
            # check ha
            self.fuel_web.assert_ha_services_ready(cluster_id, timeout=900)
            ...
            # suspend devops node with rabbit master
            master_rabbit_2.suspend(False)
