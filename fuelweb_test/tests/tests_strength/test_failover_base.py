...
class TestHaFailoverBase(TestBasic):
    ...
    def ha_corosync_stability_check(self):
        ...
        with self.env.d_env.get_admin_remote() as remote, \
                self.fuel_web.get_ssh_for_node(
                self.env.d_env.nodes().slaves[0].name) as remote_controller:
            count = 0
            while count != 1000:
                _wait(
                    lambda: assert_equal(
                ...
                defects = remote.execute(
                    'echo {0} | grep -HE -q -e UNCLEAN -e WITHOUT -e '
                    '\'OFFLINE: \[ (\S+) (\S+) \]\''
                    .format(corosync_output))['exit_code']
                if not defects:
                    raise Exception(
                        "Caught splitbrain at count-{0}".format(count))
                    break
                _wait(
                    lambda: assert_equal(remote_controller.execute(
                        'service corosync start && service pacemaker restart')
                        ['exit_code'], 0, 'Corosync was not started'),
                    timeout=20)
                _wait(
                    lambda: assert_equal(remote_controller.execute(
                        "crm_mon -1 | grep -E 'Online: "
                        "\[ (\S+) (\S+) (\S+) \]'")['exit_code'], 0,
                        'Corosync was not started on controller'), timeout=20)
                count += 1
