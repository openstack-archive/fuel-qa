    ...
    def check_virtual_router(self):
        ...
        for devops_node in self.env.d_env.nodes().slaves[:5]:
            ...
            try:
                ...
                raise TimeoutError(
                    "No Internet access from {0}".format(devops_node))
        ...
        if OPENSTACK_RELEASE == 'ubuntu':
            ...
            remote_compute.execute(
                ...
                " cd {0} && wget {2}'".format(file_path, DOWNLOAD_LINK))
            try:
                wait(
        ...
        try:
            wait(
                lambda: remote_compute.execute(cmd)['exit_code'] == 0,
                timeout=60)
        except TimeoutError:
                raise TimeoutError(
                    "No Internet access from {0} after destroy primary "
                    "controller".format(devops_node))
        if OPENSTACK_RELEASE == 'ubuntu':
            check_file_size(remote_compute, file_name, file_path)
