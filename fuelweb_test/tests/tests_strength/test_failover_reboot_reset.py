...
class FailoverRestartShutdown(TestBasic):
    ...
    def shutdown_primary_controller_ceph(self):
        """Shut down primary controller for Neutron on ceph cluster
        ...
        p_d_ctrl = self.fuel_web.get_nailgun_primary_node(
            self.env.d_env.nodes().slaves[0])
        self.fuel_web.warm_shutdown_nodes([p_d_ctrl])

        self.show_step(3)
        self.fuel_web.verify_network(cluster_id)

        self.show_step(4)  # TODO

        self.show_step(5)  # TODO
        n_ctrls = self.fuel_web.get_nailgun_cluster_nodes_by_roles(
                cluster_id,
                ['controller']
        )
        ctrls = {self.fuel_web.get_devops_node_by_nailgun_node(n_ctrls)} - \
                {[p_d_ctrl]}
        with self.fuel_web.get_ssh_for_node(ctrls[0].name) as remote:
            assert_true(
                check_ping(remote, DNS, deadline=120, interval=10),
                "No Internet access from {0}".format(ctrls[0].name)
            )
        ...
        self.show_step(7)
        self.show_step(8)
        self.fuel_web.warm_start_nodes([p_d_ctrl])
