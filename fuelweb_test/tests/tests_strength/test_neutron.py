...
class TestNeutronFailover(base_test_case.TestBasic):
    ...
    @classmethod
    @logwrap
    def create_keypair_on_master_node_and_add_to_other_nodes(cls, self):
        remote_admin = self.env.d_env.get_admin_remote()
        cmd = " ssh-keygen -f ~/.ssh/webserver_rsa -N ''"
        remote_admin.execute(cmd)
        logger.error("!!!!! tk ssh-keygen has been done  ")
        ...
        for node in controller_nodes:
            ip = node['ip']
            cmd = "scp ~/.ssh/webserver_rsa %s:~/.ssh/" % ip
            remote_admin.execute(cmd)
            cmd = "scp ~/.ssh/webserver_rsa %s:~/.ssh/" % ip
            remote_admin.execute(cmd)
            logger.error("!!!!! tk copy file to node %s" % ip)
            logger.error("!!!!! tk copy file to node %s" % node['name'])

        devops_node = self.env.d_env.nodes().slaves[0]
        logger.error("!!!!! tk devops_node slave0 = %s" % devops_node)
        remote = self.get_remote(self, devops_node)
        remote.execute(
            '. openrc;'
            'nova keypair-add --pub-key ~/.ssh/work_key_1.pub instancekey')
        logger.error("!!!!! tk Created key pair on %s" % devops_node)

        list_nodes = ['slave-01', 'slave-02', 'slave-03']
        for name in list_nodes:
            remote = self.fuel_web.get_ssh_for_node(name)
            remote.execute("hostname > /root/test_hostname")
            logger.error("Run hostname on %s" % name)
            try:
                remote.execute('chmod 400 /root/test_hostname')
                logger.error("!!!!! chmod for test")
            finally:
                logger.error("!!!!! Can't chmod")

        for node in controller_nodes:
            ip = node['ip']
            logger.error("!!!!! tk try to get remote for controller %s" % ip)
            remote = self.env.d_env.get_ssh_to_remote(ip)
            logger.error("Get ssh to node %s" % node)
            logger.error("Get ssh to node ip %s" % ip)
    ...
    def get_node_with_dhcp(cls, self, node_dhcp):
        ...
        logger.error("!!!!! new node with dhcp is {0}".format(node_dhcp))
        devops_node = self.fuel_web.find_devops_node_by_nailgun_fqdn(
            node_with_dhcp_fqdn,
            self.env.d_env.nodes().slaves[0:6])
        return devops_node
    ...
    def get_remote(cls, self, devops_node):
        ...
        logger.error("!!!!! _ip_for_node = %s" % _ip)
        x = self.fuel_web.get_nailgun_node_by_name(devops_node.name)
        logger.error("!!!!! node = %s" % x)
    ...
    def copy_vm_key_on_all_nodes(cls, self, remote):
        admin_node_ip = self.env.get_admin_node_ip()
        logger.debug("! node_for_copy = %s" % admin_node_ip)
        cmd = "scp /root/.ssh/webserver_rsa %s:/root/.ssh/" % admin_node_ip
        remote.execute(cmd)
        ...
        for node in controller_nodes:
            ip = node['ip']
            cmd = "scp /root/.ssh/webserver_rsa %s:/root/.ssh/" % ip
            remote_admin.execute(cmd)
    ...
    def neutron_l3_migration_after_reset(self, segment_type):
        ...
        cluster_id = self.fuel_web.get_last_created_cluster()
        logger.error("!!!!! tk_ 1) get cluster id = %s" % cluster_id)
        os_conn = os_actions.OpenStackActions(
            self.fuel_web.get_public_vip(cluster_id))
        ...
        devops_node = self.env.d_env.nodes().slaves[0]
        logger.error("!!!!! tk_ 2) get devops_node = %s" % devops_node)

        self.create_keypair_on_master_node_and_add_to_other_nodes(self)
        ...
        # get remote with qdhcp-namespace
        node_with_dhcp = os_conn.get_dhcp_agent_hosts(net_id)
        logger.error("!!!!! node_with_dhcp %s" % node_with_dhcp)
        devops_node_with_dhcpc = self.get_node_with_dhcp(self, node_with_dhcp)
        logger.error("!!!!! dev node dhcp %s" % devops_node_with_dhcpc)
        remote_qdhcp = self.get_remote(self, devops_node_with_dhcpc)

        cmd = 'ip netns | grep {0}'.format(net_id)
        dhcp_namespace = ''.join(remote_qdhcp.execute(cmd)['stdout']).rstrip()
        logger.debug('dhcp namespace is {0}'.format(dhcp_namespace))
        logger.error("!!!!! dhcp namespace %s" % dhcp_namespace)
        logger.error("!!!!! net_id: %s" % net_id)

        router_id = os_conn.get_routers_ids()[0]

        node_with_l3 = os_conn.get_l3_agent_hosts(router_id)
        logger.error("!!!!! router id: %s" % router_id)
        logger.error("!!!!! Before resheduling: %s" % node_with_l3)

        logger.error("!!!!! Start manually resheduling")
        self.reshedule_router_manually(os_conn, router_id)
        logger.error("!!!!! End manually resheduling")

        node_with_l3 = os_conn.get_l3_agent_hosts(router_id)
        logger.error("!!!!! After resheduling: %s" % node_with_l3)

        self.check_instance_connectivity(remote_qdhcp, dhcp_namespace,
                                         instance_ip)

        node_with_l3 = os_conn.get_l3_agent_hosts(router_id)[0]
        logger.error("!!!!! Now l3 is on %s" % node_with_l3)
        ...
        # get remote with qdhcp-namespace
        node_with_dhcp = os_conn.get_dhcp_agent_hosts(net_id)
        logger.error("!!!!! new node with dhcp: %s" % node_with_dhcp)
        devops_node_with_dhcpc = self.get_node_with_dhcp(self, node_with_dhcp)
        remote_qdhcp = self.get_remote(self, devops_node_with_dhcpc)
    ...
    @test(depends_on=[deploy_ha_neutron_vxlan],
          groups=["neutron_l3_migration"])
    def neutron_l3_migration_vxlan_env(self):
    ...
    @test(depends_on=[deploy_ha_neutron_vxlan],
          groups=["neutron_l3_migration_after_reset"])
    def neutron_l3_migration_after_reset_vxlan_env(self):
    ...
    @test(depends_on=[deploy_ha_neutron_vxlan],
          groups=["neutron_l3_migration_after_destroy"])
    def neutron_l3_migration_after_destroy_vxlan_env(self):
