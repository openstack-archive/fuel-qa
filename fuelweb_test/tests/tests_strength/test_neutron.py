...
class TestNeutronFailover(base_test_case.TestBasic):
    ...
    @classmethod
    @logwrap
    def create_instance_with_keypair(cls, os_conn, remote, sync_host=None):
        remote.execute(
            '. openrc;'
        ...
        if sync_host:
            for host in sync_host:
                remote.execute("scp /root/.ssh/webserver_rsa "
                               "{0}:/root/.ssh/".format(host))
    ...
    def neutron_l3_migration(self):
        ...
        instance_ip = \
            self.create_instance_with_keypair(
                os_conn,
                remote,
                sync_host=ctrl_hostnames).addresses['net04'][0]['addr']
