...
class GraphExtension(TestBasic):
    ...
    def graph_extension_cli(self):
        ...
        with self.ssh_manager.open_on_remote(
            ...
            yaml.safe_dump(self.graph, f)
        cmd = 'fuel2 graph upload -t my_graph -f graph.yaml'
        ...
        self.ssh_manager.execute_on_remote(admin_ip, cmd)
        self.show_step(6)
        # tasks = self.fuel_web.get_last_task_id(cluster_id=cluster_id,
        #                                       task_name='deployment')
        self.fuel_web.client.assert_all_tasks_completed(cluster_id)
