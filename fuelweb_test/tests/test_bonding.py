...
class BondingHA(BondingTest):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=['deploy_bonding_with_template'])
    @log_snapshot_after_test
    def deploy_bonding_with_template(self):
        """Deploy cluster with active-backup bonding and Neutron VXLAN
        with network template.

        Scenario:
            1. Create cluster
            2. Add 3 nodes with controller role
            3. Add 1 node with compute role and 1 node with cinder
            4. Setup bonding for all interfaces (including admin interface
               bonding)
            ...
            9. Run OSTF
            10. Save network configuration from slave nodes
            11. Reboot all environment nodes
            12. Verify that network configuration is the same after reboot

        Duration 70m
        Snapshot deploy_bonding_with_template
        """
        ...
        self.env.make_snapshot('deploy_bonding_with_template')
