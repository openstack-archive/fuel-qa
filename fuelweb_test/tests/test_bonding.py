...
class BondingHA(BondingTest):
    ...
    def bonding_conf_consistency(self):
        ...
        # Get ID of a (pending) controller
        pending_ctrls = [n for n in nailgun_nodes
                         if 'controller' in n['pending_roles']]
        node_id = pending_ctrls[0]['id']

        self.show_step(6)
        task = self.fuel_web.client.provision_nodes(cluster_id, [str(node_id)])
        ...
        self.show_step(8)
        task = self.fuel_web.client.deploy_nodes(cluster_id, [str(node_id)])
