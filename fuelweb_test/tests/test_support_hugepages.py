...
class SupportHugepages(TestBasic):
    ...
    def boot_instance_with_hugepage(self, target_compute_name,
        ...
        target_compute_fqdn = \
            self.fuel_web.get_nailgun_node_by_name(target_compute_name)['fqdn']
        ...
        server = self.os_conn.create_server_for_migration(
            neutron=True,
            label=net_name,
            availability_zone="nova:{0}".format(target_compute_fqdn),
            flavor=flavor.id())

        logger.info(server.get('OS-EXT-SRV-ATTR:host'), target_compute_fqdn)
        ...
        for compute in computes:
            self.ssh_manager.execute_on_remote(
                ...
                cmd="fuel2 node show {0} | grep hugepages | "
                    "grep 2048".format(compute['fqdn']),
                err_msg="2Mb HugePages doesn't present in CLI for node "
                        "{0}".format(compute))
            self.ssh_manager.execute_on_remote(
                ip=self.ssh_manager.admin_ip,
                cmd="fuel2 node show {0} | grep hugepages | "
                    "grep 1048576".format(compute['fqdn']),
    ...
    def check_hugepages_after_reboot(self):
        ...
        Snapshot: check_hugepages_nova_scheduler
        """
        self.env.revert_snapshot("check_hugepages_distribution_per_numa")
