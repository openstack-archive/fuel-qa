...
class SupportHugepages(TestBasic):
    ...
    def basic_env_for_hugepages(self):
        ...
        self.ssh_manager.execute_on_remote(
            ...
            err_msg="HugePages don't support 2M",
            assert_ec_equal=0)

        self.ssh_manager.execute(
            ip=self.ssh_manager.admin_ip,
            cmd="grep \"pdpe1gb\" {0}".format(log_path),
            err_msg="HugePages don't support 1GB",
            assert_ec_equal=0)
        ...
        for compt in computes:
            self.ssh_manager.execute(
                ip=self.ssh_manager.admin_ip,
                cmd="fuel2 node show {0} | grep hugepages".format(compt['id']),
                err_msg="HP size isn't present in CLI for "
                        "{0}".format(compt['id']),
                assert_ec_equal=0)
