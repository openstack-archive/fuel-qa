...
class OlHAOneControllerMigration(ExtraComputesBase):
    ...
    def check_vm_migration_ol_ha_one_controller_tun(self):
        """Deploy environment with OL computes in HA mode with neutron VXLAN
        ...
        data = {
            ...
            'tenant': 'OlHAMigration',
            'user': 'OlHAMigration',
            'password': 'OlHAMigration',
            'volumes_ceph': True,
            'ephemeral_ceph': True,
            'images_ceph': True,
            'objects_ceph': True,
        ...
        self.verify_image_connected(target_node_one_ip, types='ol')
        self.verify_image_connected(target_node_two_ip, types='ol')
        ...
        new_host_one = self.set_hostname(target_node_one_ip, types='ol')
        self.clean_custom_repos(target_node_one_ip)
        self.install_yum_components(target_node_one_ip)
        self.enable_extra_compute_repos(target_node_one_ip, types='ol')
        ...
        new_host_two = self.set_hostname(target_node_two_ip, host_number=2,
                                         types='ol')
        self.clean_custom_repos(target_node_two_ip)
        self.install_yum_components(target_node_two_ip)
        self.enable_extra_compute_repos(target_node_two_ip, types='ol')
        ...
        self.env.make_snapshot("ready_ha_one_controller_with_ol_computes")
