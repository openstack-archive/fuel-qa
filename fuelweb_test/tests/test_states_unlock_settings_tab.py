        ...
        for setting in old_settings:
            if setting != 'networks':
                if old_settings[setting] != new_settings[setting]:
                    logger.debug("Old setting are:{}".format(
                        old_settings[setting]))
                    logger.debug("New setting are:{}".format(
    ...
    def compare_settings(old_attrs, new_attrs):
        skipped_options =\
            [
                ...
                'service_user.metadata',
                'public_ssl.cert_data',
                'storage.bootstrap_osd_key',
                'storage.radosgw_key',
                'storage.admin_key',
                'storage.fsid',
                'storage.mon_key',
                'workloads_collector.password',
                'murano_settings.murano_glance_artifacts_plugin',
                'additional_components.murano_glance_artifacts_plugin',
                'additional_components.metadata',
                'repo_setup.metadata',
                'cgroups.metadata',
                'common.debug'
            ]
        ...
        logger.debug("Old default cluster settings: {}".format(editable_old))
        logger.debug("New default cluster settings: {}".format(editable_new))
        for group in editable_old:
            for opt in editable_old[group]:
                key = '.'.join([group, opt])
                if key in skipped_options:
                    continue
                else:
