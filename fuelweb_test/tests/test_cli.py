...
class CommandLineTest(test_cli_base.CommandLine):
    ...
    def cli_selected_nodes_deploy(self):
        ...
        # Get public urls and serivce name from endpoint list by ID
        endpoint_info_list = self.get_endpoint_info(controller_node,
                                                    endpoint_list)
        logger.info(endpoint_info_list)
        #Check protocol and domain names for endpoints
        for endpoint in endpoint_info_list:
