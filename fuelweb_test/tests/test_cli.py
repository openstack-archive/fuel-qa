...
class CommandLine(TestBasic):
    ...
    def node_deletion_check(self):
        ...
        hostname = (self.fuel_web.get_nailgun_node_by_devops_node(
            ...
            ['fqdn']).split('.')[0]
        assert_true(check_node(remote, hostname, flag=True),
                    "node is not found")
        self.env.get_virtual_environment().nodes().slaves[0].destroy()
        try:
            wait(
        ...
        assert_false(check_node(remote, hostname, flag=True),
                     "node is found in cobbler list")
        assert_false(check_node(remote, hostname), "node is found in fuel "
                                                   "list")
