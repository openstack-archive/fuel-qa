...
class CommandLineClient(object):
    ...
    def __init__(self, remote, fuel_web):
        self.remote = remote
        self.fuel_web = fuel_web

    def __enter__(self):
        # Replace NailgunClient with CommandLineClient
        self.nailgun_client = self.fuel_web.client
        self.fuel_web.client = self

    def __exit__(self, exp_type, exp_value, traceback):
        # Set NailgunClient back
        self.fuel_web.client = self.nailgun_client
...
class CommandLine(TestBasic):
    ...
    def assert_cli_task_success(
        ...
        with CommandLineClient(remote, self.fuel_web):
            # Use assert_task_success() with CommandLineClient methods
    ...
    def cli_selected_nodes_deploy(self):
        ...
        with self.env.d_env.get_admin_remote() as remote:
            ...
            cmd = ('fuel --env-id={0} node --deploy --node {1},{2}'
                   .format(cluster_id, node_ids[1], node_ids[2]))
            task = run_on_remote(remote, cmd, jsonify=True)
            self.assert_cli_task_success(task, remote, timeout=30 * 60)
