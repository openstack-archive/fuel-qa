...
class CommandLineTest(test_cli_base.CommandLine):
    ...
    def cli_selected_nodes_deploy(self):
        ...
        with self.env.d_env.get_admin_remote() as remote:
            ...
            # Get endpoint list
            endpoint_list = self.get_endpoint_info(controller_node)
            ...
            assert_equal(SSL_CN, current_ssl_cn,
                         message="SSL CNs are not equal")
        ...
        with open(PATH_TO_PEM) as pem_file:
            ...
            actual_floating_ranges = \
                self.hiera_floating_ranges(controller_node)
            logger.info(
