...
class CommandLineTest(test_cli_base.CommandLine):
    ...
    def cli_selected_nodes_deploy_huge(self):
        ...
        assert_equal(
            2,
            len(self.fuel_web.get_nailgun_node_by_status('discover')),
            'Some unexpected nodes were provisioned,'
            ' current list of provisioned '
            'nodes {}'.format([node['id'] for node in self.fuel_web.get_nailgun_node_by_status('discover')]))
        ...
        assert_equal(
            ...
            ' current list of nodes in discover status'
            '{}'.format([node['id']
                         for node in self.fuel_web.get_nailgun_node_by_status(
                         'discover')]))
        # Run OSTF
        self.show_step(19)
        self.fuel_web.run_ostf(cluster_id=cluster_id,
