...
class ZabbixPlugin(TestBasic):
    ...
    def check_event_message(self, zabbix_web, zabbix_hostgroup, message):
        ...
        soup = bs4.BeautifulSoup(statuses_html, "lxml")
        status_lines = soup.find_all('tr', {'class': 'even_row'})
    ...
    def deploy_zabbix_snmptrap_ha(self):
        ...
        with self.env.d_env.get_ssh_to_remote(management_vip) as remote:
            cmd = ('grep netSnmpExampleHeartbeatNotification '
                   '/var/log/zabbix/zabbix_server.log | '
                   'grep "Status Events"')
            check_snmp_event = lambda: \
                "".join(remote.execute(cmd)['output']).strip()
    ...
    def deploy_zabbix_snmp_emc_ha(self):
        ...
        self.env.d_env.get_admin_remote().execute(snmp_emc_critical_command)
    ...
    def deploy_zabbix_snmp_extreme_ha(self):
        ...
        self.env.d_env.get_admin_remote().execute(
            snmp_extreme_critical_command)
