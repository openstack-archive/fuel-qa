...
class ZabbixPlugin(TestBasic):
    ...
    def deploy_zabbix_ha(self):
        ...
        response = self.fuel_web.execute_remote_cmd(remote, cmd)
        assert_true("is running" in response, response)
        ...
        res = session.get("{0}/screens.php".format(zabbix_url)).content
        assert_equal(res.status_code, 200, "HTTP {0}".format(res.status_code))
