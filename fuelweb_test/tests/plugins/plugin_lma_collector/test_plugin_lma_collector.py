...
@test(groups=["plugins"])
class LmaCollectorPlugin(TestBasic):
    ...
    def deploy_lma_collector_ha(self):
        ...
        InfluxDB-Grafana plugin since they work togehter with the LMA collector
        plugin.
        ...
        analytics_node_ip = self.fuel_web.get_nailgun_node_by_name(
            analytics_node_name).get('ip')
        assert_is_not_none(
            analytics_node_ip,
            "Fail to retrieve the IP address of {}".format(analytics_node_name)
        )
