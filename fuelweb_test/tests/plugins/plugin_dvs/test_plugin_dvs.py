...
class TestDVSPlugin(TestBasic):
    ...
    cluster_setings = {'net_provider': 'neutron',
            ...
            for srv in srv_list:
                ...
                fip = os_conn.neutron.create_floatingip(
                    {'floatingip': {'floating_network_id': ext_net['id'],
                     'tenant_id': tenant_id}})
                os_conn.nova.servers.add_floating_ip(srv, fip)
                assert_true(
    ...
    def dvs_vcenter_diff_networks(self):
        ...
        for net in net_data:
            ...
            subnet = os_conn.create_subnet(network=network,
                                           cidr=net[net.keys()[0]],
    ...
    def dvs_vcenter_networks(self):
        ...
        subnet = os_conn.create_subnet(
            network=network,
            cidr=net_data[0][net_data[0].keys()[0]],
            tenant_name=SERVTEST_TENANT
        )
        assert_true(
            os_conn.get_network(network['name'])['id'] == network['id']
