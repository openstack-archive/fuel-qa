...
class TestDVSPlugin(TestBasic):
    ...
    def update_ostf(self):
        logger.info('update ostf')
        remote = self.env.d_env.get_admin_remote()
        commands = "dockerctl shell ostf; cd /tmp; yum install git -y;" \
                   "git clone https://github.com/stackforge/fuel-ostf.git;" \
                   "cd fuel-ostf;" \
                   "git branch --track stable/6.1 origin/stable/6.1;" \
                   "git checkout stable/6.1;" \
                   "rsync -avz fuel_health /usr/lib/python2.6/site-packages;" \
                   "exit; dockerctl restart ostf;"
    ...
    def dvs_vcenter_diff_networks(self):
        ...
        os_conn.add_subnet_to_router(router_1['id'], subnets[0]['id'])
        os_conn.add_subnet_to_router(router_2['id'], subnets[1]['id'])
    ...
    def dvs_vcenter_security(self):
        ...
        os_conn.add_subnet_to_router(router['id'], subnet['id'])
    ...
    def dvs_vcenter_tenants_isolation(self):
        ...
        test.add_subnet_to_router(router_1['id'], subnet['id'])
    ...
    def dvs_vcenter_same_ip(self):
        ...
        # Add net_1 to router_1
        admin.add_subnet_to_router(router_1['id'], subnet['id'])
