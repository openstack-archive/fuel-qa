...
class TestDVSPlugin(TestBasic):
    ...
    def dvs_vcenter_diff_networks(self):
        ...
        Scenario:
            ...
            10. Launch instances VM_1 and VM_2 in the network192.168.101.0/24
               with image TestVM and flavor m1.micro in nova az.
            11. Launch instances VM_3 and VM_4 in the 192.168.102.0/24
               with image TestVMDK and flavor m1.micro in vcenter az.
            12. Verify that VMs of same networks should communicate
               between each other. Send icmp ping from VM 1 to VM2,
               VM 3 to VM4 and vice versa.
            13. Verify that VMs of different networks should not communicate
               between each other. Send icmp ping from VM 1 to VM3,
               VM_4 to VM_2 and vice versa.
            14. Create Router_01, set gateway and add interface
               to external network.
            15. Attach private networks to router.
            16. Verify that VMs of different networks should communicate
                between each other. Send icmp ping from VM 1 to VM3, VM_4
                to VM_2 and vice versa.
            17. Add new Router_02, set gateway and add interface to
                external network.
            18. Deatach net_02 from Router_01 and attache to Router_02
            19. Verify that VMs of different networks should communicate
                between each other. Send icmp ping from VM 1 to VM3, VM_4
                to VM_2 and vice versa
    ...
    def dvs_vcenter_same_ip(self):
        ...
        tenant_id = admin.get_tenant('test').id
        self.check_connection_vms(
            os_conn=test, srv_list=srv_1,
            packets=3, tenant_id=tenant_id
        )
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["dvs_reset_controller", 'dvs_vcenter_system'])
    @log_snapshot_after_test
    def dvs_reset_controller(self):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["dvs_reset_controller", 'dvs_vcenter_system'])
    @log_snapshot_after_test
    def dvs_shutdown_controller(self):
        """Verify that vmclusters should be migrate after reset controller.
