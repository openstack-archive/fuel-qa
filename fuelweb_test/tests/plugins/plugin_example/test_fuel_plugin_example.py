...
class ExamplePlugin(TestBasic):
    ...
    def deploy_ha_one_controller_neutron_example_v3(self):
        ...
        res_node_01 = self.env.d_env.get_ssh_to_remote(_ip_node_01)
        checkers.check_file_exists(res_node_01, 'plugin+100.0.all', '/tmp')
        checkers.check_file_exists(res_node_01, 'plugin+100.all', '/tmp')
        checkers.check_file_exists(res_node_01, 'fuel_plugin_example_v3_sh', '/tmp')

        # check if slave-02 contain
        # plugin+100.0.all
        # plugin+100.al
        _ip_node_02 = self.fuel_web.get_nailgun_node_by_name("slave-02")['ip']
        res_node_02 = self.env.d_env.get_ssh_to_remote(_ip_node_02)
        checkers.check_file_exists(res_node_02, 'plugin+100.0.all', '/tmp')
        checkers.check_file_exists(res_node_02, 'plugin+100.all', '/tmp')
        ...
        res_node_03 = self.env.d_env.get_ssh_to_remote(_ip_node_03)
        checkers.check_file_exists(res_node_03, 'plugin+100.0.all', '/tmp')
        checkers.check_file_exists(res_node_03, 'plugin+100.all', '/tmp')
        checkers.check_file_exists(res_node_03, 'fuel_plugin_example_v3_sh', '/tmp')
        checkers.check_file_exists(res_node_03, 'fuel_plugin_example_v3_puppet', '/tmp')

        # check if service ran on slave-03
        logger.debug("Start to check service on node {0}".format('slave-03'))
        cmd_curl = 'curl localhost:8234'
        cmd = 'pgrep -f fuel-simple-service'
