...
class ExamplePluginPostDeploy(TestBasic):
    ...
    def _redeploy_controller_nodes(self, nodes, cluster_id=None):
        if cluster_id is None:
            cluster_id = self._get_cluster_id()
    ...
    def three_ctrl_enable_installed_after_create_redeploy(self):
        ...
        self.show_step(2)
        self._redeploy_controller_nodes(nodes=[node], cluster_id=cluster_id)
    ...
    @test(
        depends_on=[five_ctrl_enable_installed_after_create_add],
        groups=[
            "install_plugin_after_cluster_create",
            "five_ctrl_update_primary_installed_after_create"])
    @log_snapshot_after_test
    def five_ctrl_update_primary_installed_after_create(self):
        """Deploy cluster with one controller and example plugin

        Scenario:
            1. Enable plugin
            2. Re-deploy primary controller node (Node Under Test)
            3. Run network verification
            4. Check plugin health on NUT
            5. Run OSTF

        Duration 35m
        Snapshot five_ctrl_update_primary_installed_after_create
        """
        self.env.revert_snapshot("five_ctrl_enable_installed_after_create_add")
        cluster_id = self._get_cluster_id()
    ...
    @test(
        depends_on=[five_ctrl_enable_installed_after_deploy_add],
        groups=[
            "install_plugin_after_cluster_deploy",
            "five_ctrl_update_primary_installed_after_deploy"])
    @log_snapshot_after_test
    def five_ctrl_update_primary_installed_after_deploy(self):
        """Deploy cluster with one controller and example plugin
        ...
        Snapshot five_ctrl_update_primary_installed_after_create
        """
        self.env.revert_snapshot("five_ctrl_enable_installed_after_deploy_add")
        cluster_id = self._get_cluster_id()
