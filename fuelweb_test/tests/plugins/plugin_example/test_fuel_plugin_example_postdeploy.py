...
class ExamplePluginPostDeploy(TestBasic):
    ...
    def _get_primary_controller(self, cluster_id=None):
        controller_nodes = self._get_controller_nodelist(cluster_id=cluster_id)
    ...
    def _redeploy_controller_node(self, node, cluster_id=None):
        if cluster_id is None:
            cluster_id = self._get_cluster_id()

        logger.info('Removing node {!s} from cluster'.format(node))
        self.fuel_web.update_nodes(
            cluster_id=cluster_id,
            nodes_dict={node: ['controller']},
            pending_addition=False, pending_deletion=True
        )
        self.fuel_web.deploy_cluster_wait(cluster_id)

        logger.info('Re-adding node {!s} from cluster'.format(node))
        self.fuel_web.update_nodes(
            cluster_id,
            {node: ['controller']}
        )
    ...
    @test(
        ...
        groups=[
            "install_plugin_after_cluster_create",
            "three_controllers_install_enable_example_after_cluster_create"])
    @log_snapshot_after_test
    def three_controllers_install_enable_example_after_cluster_create(self):
        ...
        Scenario:
            ...
            3. Install plugin
            4. Enable plugin
            5. Add 2 nodes with controller role
        ...
        Duration 35m
        Snapshot three_controllers_install_enable_example_after_cluster_create
        """
    ...
    @test(
        ...
        groups=[
            "install_plugin_after_cluster_create",
            "three_controllers_install_example_after_cluster_create"])
    @log_snapshot_after_test
    def three_controllers_install_example_after_cluster_create(self):
        """Deploy cluster with one controller and example plugin
        ...
        Duration 35m
        Snapshot three_controllers_install_example_after_cluster_create
        """
        self.check_run(
            'three_controllers_install_example_after_cluster_create')
        self.env.revert_snapshot("ready_with_3_slaves")
    ...
    @test(
        ...
        groups=[
            "install_plugin_after_cluster_create",
            "three_controllers_enable_example_installed_after_cluster_create"])
    @log_snapshot_after_test
    def three_controllers_enable_example_installed_after_cluster_create(self):
        """Deploy cluster with one controller and example plugin
        ...
        Scenario:
            ...
            4. Check plugin health on NUT
            5. Run OSTF

        Duration 35m
        Snapshot three_controllers_enable_example_installed_after_cluster_create
        """
        ...
        node = self._get_primary_controller(cluster_id=cluster_id)
        logger.info('Node under test: {!s}'.format(node))

        self.show_step(2)
        self._redeploy_controller_node(node=node, cluster_id=cluster_id)
    ...
    @test(
        ...
        groups=[
            "install_plugin_after_cluster_create",
            "three_controllers_install_example_after_cluster_deploy"])
    @log_snapshot_after_test
    def three_controllers_install_example_after_cluster_deploy(self):
        """Deploy cluster with one controller and example plugin

        Scenario:
            1. Create cluster
            2. Add 2 nodes with controller role
        ...
        Duration 35m
        Snapshot three_controllers_install_example_after_cluster_deploy
        """
        self.check_run(
            'three_controllers_install_example_after_cluster_deploy')
        self.env.revert_snapshot("ready_with_3_slaves")
    ...
    @test(
        ...
        groups=[
            "install_plugin_after_cluster_create",
            "three_controllers_enable_example_installed_after_cluster_deploy"])
    @log_snapshot_after_test
    def three_controllers_enable_example_installed_after_cluster_deploy(self):
        """Deploy cluster with one controller and example plugin
        ...
        Duration 35m
        Snapshot three_controllers_enable_example_installed_after_cluster_deploy
        """
