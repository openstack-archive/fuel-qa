...
class ContrailPlugin(TestBasic):
    master_path = '/var/www/nailgun/plugins/contrail-1.0.0'

    @classmethod
    def upload_packets(cls, node_ssh, pack_path, master_path):
        if os.path.splitext(pack_path)[1] not in [".deb"]:
    ...
    @classmethod
    def install_packets(cls, remote, master_path):
        commands = ["cd " + master_path, "./install.sh"]
        for cmd in commands:
            logger.debug(cmd)
