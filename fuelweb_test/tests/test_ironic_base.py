...
# new testcase Configuration BigHA
# Big HA [9 nodes, with ceph]
# 3 controllers nodes
# 2 ironics (on separate nodes)
# 3 ceph nodes (replication factor 2, all ceph options enabled)
# 1 compute (separate node)

    @test(depends_on=[SetupEnvironment.prepare_slaves_9],
          groups=["ironic_deploy_ceph"])
    @log_snapshot_after_test
    def ironic_deploy_big(self):
        """Deploy ironic with 1 baremetal node

        Scenario:
            1. Create cluster
            2. Add 3 nodes with Controller role
            3. Add 3 nodes with Ceph-OSD roles
            4. Add 1 node with Compute rols
            5. Add 2 nodes with Ironic roles
        ...
        self.env.make_snapshot("ironic_deploy_big")
