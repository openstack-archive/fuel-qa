...
class TestIronicDeploy(TestBasic):
    ...
    def deploy_scale_controller_ironic(self):
        ...
        Scenario:
            1. Create cluster
            2. Add 2 Controller nodes
            3. Add 1 Compute node
            4. Add 1 Controller+Ironic node
            5. Deploy the cluster
            6. Run OSTF tests
            7. Boot, check connectivity, delete Ironic VM
            8. Rebalance Swift rings
            9. Add 1 Controller node
            10. Add 1 Controller+Ironic node
            11. Redeploy the cluster
            12. Run OSTF tests
            13. Boot, check connectivity, delete Ironic VM
            14. Rebalance Swift rings
            15. Remove 1 Controller node
            16. Remove 1 Controller+Ironic node
            17. Redeploy the cluster
            18. Run OSTF tests
            19. Boot, check connectivity, delete Ironic VM
        ...
        primary_node = self.fuel_web.get_nailgun_primary_node(
            self.env.d_env.nodes().slaves[0])
        common.Common.rebalance_swift_ring(primary_node)
        self.show_step(9)
        self.show_step(10)
        self.fuel_web.update_nodes(
            cluster_id,
