...
from proboscis.asserts import assert_true, assert_equal, assert_not_equal, assert_raises
...
class UbuntuBootstrapBuild(base_test_case.TestBasic):
    ...
    def build_default_bootstrap(self):
        ...
        self.env.fuel_bootstrap_actions.\
            activate_bootstrap_image(uuid, notify_webui=True)
    ...
    def create_list_import_delete_bootstrap_image(self):
        ...
        assert_equal(2, bootstrap_uuids,
...
class UbuntuBootstrap(base_test_case.TestBasic):
    ...
    def deploy_with_two_ubuntu_bootstraps(self):
        ...
        with self.env.d_env.get_admin_remote() as remote:
            ...
            uuid = fuel_bootstrap.get_active_bootstrap_uuid()
        ...
        with self.env.d_env.get_admin_remote() as remote:
            fuel_bootstrap = fuel_actions.FuelBootstrapCliActions(remote)
            new_uuid, _ = fuel_bootstrap.build_bootstrap_image(activate=True)
    ...
    def deploy_stop_on_deploying_ubuntu_bootstrap(self):
        ...
        with self.env.d_env.get_admin_remote() as remote:
            fuel_bootstrap = fuel_actions.FuelBootstrapCliActions(remote)
            _, uuid = \
                fuel_bootstrap.build_bootstrap_image(activate=True)
