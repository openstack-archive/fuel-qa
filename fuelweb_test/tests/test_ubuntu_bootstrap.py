...
class UbuntuBootstrapBuild(base_test_case.TestBasic):
    def _get_main_repo(self, repos, repo_name):
        for repo in repos:
            if repo_name not in repo["name"]:
                continue
            if "updates" in repo["suite"] or "security" in repo["suite"]:
                continue

            return repo
        raise Exception("Can not find main repo {0} in repo list: {1}"
                        .format(repo_name, repos))
    ...
    def build_simple_bootstrap(self):
        ...
        ubuntu_repo = self._get_main_repo(bootstrap_default_params["repos"],
                                          "ubuntu")
        mos_repo = self._get_main_repo(bootstrap_default_params["repos"],
                                       "mos")

        bootstrap_params = {
            "ubuntu-release": "trusty",
            "repo": ["'deb {0} {1}'".format(ubuntu_repo['uri'],
                                            ubuntu_repo['suite']),
                     "'deb {0} {1}'".format(mos_repo['uri'],
                                            mos_repo['suite'])],
