...
class TestMultipleClusterNets(TestBasic):
    ...
    def add_custom_nodegroup(self):
        ...
        3. Add node from new nodegroup to the environment with compute role
        4. Run network verification
        5. Deploy changes
        6. Run network verification
        7. Run OSTF
        8. Check that nodes from 'default' nodegroup can reach nodes
        ...
        for network in self.netconf_all_group['networks']:
            if not any(network['group_id'] == ng['id']
        ...
        self.fuel_web.client.update_network(
            cluster_id,
            self.netconf_all_group['networking_parameters'],
            self.netconf_all_group['networks'])
        ...
        with self.fuel_webget_ssh_for_node('slave-07') as remote:
