...
class TestMultipleClusterNets(TestBasic):
    ...
    def deploy_controllers_from_custom_nodegroup(self):
        ...
        #removeme
        logger.info('changes to network settings:\n{0}'.format(new_settings))

        logger.info('current cluster id: {0}'.format(cluster_id))
        self.fuel_web.update_cluster_network(cluster_id, new_settings)

        #removeme
        logger.info('changed network settings:\n{0}'.format(json.dumps(self.fuel_web.get_networks(cluster_id), indent=1)))
        #
