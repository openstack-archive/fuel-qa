...
class TestMultipleClusterNets(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_release],
          groups=["add_nodes_from_custom_nodegroup", "thread_7"
                  "multiple_cluster_networks", "multiple_cluster_net_ceph_ha"])
    @log_snapshot_after_test
    def add_nodes_from_custom_nodegroup(self):
        ...
        Scenario:
            ...
            8. Run OSTF
            9. Create custom nodegroup
            10. Bootstrap slaves from custom nodegroup
            11. Add nodes from 'custom' nodegroup to the environment
            12. Run network verification
            13. Deploy changes
            14. Run network verification
            15. Run OSTF
            16. Check that nodes from 'default' nodegroup can reach nodes from
                'custom' nodegroup via management and storage networks

        Duration 180m
        Snapshot add_nodes_from_custom_nodegroup
        """
        ...
        self.show_step(3)
        # TODO(mstrukov): add create_nodegroups=False
        logger.warn('All nodegroups created with cluster. Adding nodegroups '
                    'to already deployed cluster are not ready now')
        cluster_id = self.fuel_web.create_cluster(
            name=self.__class__.__name__,
            mode=DEPLOYMENT_MODE_HA,
            settings={
        ...
        self.env.make_snapshot("add_nodes_from_custom_nodegroup")
