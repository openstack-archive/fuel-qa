...
class TestMultipleClusterNets(TestBasic):
    ...
    def add_custom_nodegroup(self):
        ...
        3. Bootstrap node from custom-2 nodegroup
        ...
        for network in self.netconf_all_groups['networks']:
            if network['group_id'] is not None and not any(network['group_id'] == ng['id'] for ng in self.fuel_web.client.get_nodegroups()):
