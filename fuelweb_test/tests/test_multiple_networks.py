...
class TestMultipleClusterNets(TestBasic):
    ...
    def deploy_controllers_from_custom_nodegroup(self):
        ...
        self.show_step(4)
        current_settings = self.client.get_networks(cluster_id)
        logger.info('current network settings: {0}'.format(current_settings))
        ...
        self.show_step(6)
        logger.info('NODEGROUPS is: {0}'.format(NODEGROUPS))
