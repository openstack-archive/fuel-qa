...
class TestMultipleClusterNets(TestBasic):
    ...
    def multiple_cluster_net_setup(self):
        ...
        Scenario:
            1. Revert snapshot with 7 slaves
    ...
    def add_nodes_from_custom_nodegroup_to_deployed_env(self):
        ...
        2. Add nodes from 'default' nodegroup
        3. Run network verification
        4. Deploy environment
        5. Run network verification
        6. Run OSTF
        7. Create new 'custom' nodegroup for the environment and configure its
        networks
        8. Add nodes from 'custom' nodegroup to the environment
        9. Run network verification
        10. Deploy changes
        11. Run network verification
        12. Run OSTF
        13. Check that nodes from 'default' nodegroup can reach nodes from
        ...
        self.fuel_web.update_nodes(
            cluster_id,
            {
                'slave-02': [['controller'], default_nodegroup],
                'slave-04': [['controller'], default_nodegroup],
                'slave-06': [['controller'], default_nodegroup],
                'slave-01': [['compute'], default_nodegroup],
        ...
        self.fuel_web.update_nodes(
            cluster_id,
            {
                ...
                'slave-01': [['compute'], default_nodegroup],
                'slave-03': [['cinder'], default_nodegroup],
                'slave-05': [['compute'], custom_nodegroup],
                'slave-07': [['compute'], custom_nodegroup],
            }
        )
