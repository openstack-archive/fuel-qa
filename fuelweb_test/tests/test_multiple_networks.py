...
import json #TODO(mstrukov): remove me
import random
...
class TestMultipleClusterNets(TestBasic):
    ...
    def deploy_controllers_from_custom_nodegroup(self):
        ...
        strlog = 'Devops networks:\n'
        for nodegroup in NODEGROUPS:
            strlog += 'nodegroup: ' + nodegroup['name'] + '\n'
            for pool in nodegroup['pools']:
                strlog += 'pool: ' + pool + '\n'
                strlog += 'ip=' + str(self.env.d_env.get_network(name=pool).ip) + '\n'
                strlog += 'ip_pool_start=' + str(self.env.d_env.get_network(name=pool).ip_pool_start) + '\n'
                strlog += 'ip_pool_end=' + str(self.env.d_env.get_network(name=pool).ip_pool_end) + '\n'
                strlog += 'netmask=' + str(self.env.d_env.get_network(name=pool).netmask) + '\n'
                strlog += 'default_gw=' + str(self.env.d_env.get_network(name=pool).default_gw) + '\n'
        logger.info(strlog)
        ...
        for k in check:
            ...
            logger.info('{0} is {1}'.format(k, str(vip)))
            custom_net = self.env.d_env.get_network(name=check[k]).ip

            # TODO(mstrukov): remove me
            logger.info('type of custom_net is {}'.format(type(custom_net)))

            logger.info('{0} is {1}'.format(check[k], str(custom_net)))
