...
class TestMultipleClusterNets(TestBasic):
    ...
    def add_nodes_from_custom_nodegroup_to_deployed_env(self):
        ...
        with self.fuel_web.get_ssh_for_node('slave-07') as remote:
            ...
            logger.info('slave07: ifconfig -a\n{0}'.format(run_on_remote(remote, cmd='ifconfig -a')))
            logger.info('slave07: ip addr\n{0}'.format(run_on_remote(remote, cmd='ip addr')))
            logger.info('slave07: hiera node\n{0}'.format(run_on_remote(remote, cmd='hiera node')))
        ...
        with self.fuel_web.get_ssh_for_node('slave-02') as remote:
            ...
            logger.info('slave02: ip addr\n{0}'.format(run_on_remote(remote, cmd='ip addr')))
            logger.info('slave02: hiera node\n{0}'.format(run_on_remote(remote, cmd='hiera node')))

        with self.fuel_web.get_ssh_for_node('slave-02') as remote:
            slave02manage_ip = get_hiera_data_as_json(remote, 'node')['network_roles']['management']
            slave02storage_ip = get_hiera_data_as_json(remote, 'node')['network_roles']['storage']
        with self.fuel_web.get_ssh_for_node('slave-07') as remote:
            assert_true(
                check_ping(remote, slave02manage_ip),
                "{0} (ip:{1}) not accessible from {2} via {3} network".format('slave02', slave02manage_ip,'slave07', 'management'))
            assert_true(
                check_ping(remote, slave02storage_ip),
                "{0} (ip:{1}) not accessible from {2} via {3} network".format('slave02', slave02storage_ip,'slave07', 'storage'))
