    ...
    def deploy_neutron_tun_ha_nodegroups(self):
        ...
        with self.env.d_env.get_admin_remote() as remote:
            ...
            asserts.assert_equal(
                ...
                'Looks like management range fro default nodegroup was '
            ...
            asserts.assert_equal(
                ...
                'Looks like management range fro custom nodegroup was '
                'not updated. Expected {0}, Actual: {1}'.format(
        ...
        for net_node in net_data_default_group:
            logger.info('net_node {0}'.format(net_node))
            for net in net_node:
                logger.info('net is {0}'.format(net))
                if 'storage' in net['name']:
                    asserts.assert_true(
                        self.is_ip_in_range(
                            net['ip'],
                            updated_storage_default[0],
                            updated_storage_default[-1]))
                if 'management' in net['name']:
                    asserts.assert_true(
                        self.is_ip_in_range(
                            net['ip'],
                            updated_mgmt_default[0],
                            updated_mgmt_default[-1]))
            ...
            for net in net_node:
                ...
                if 'storage' in net['name']:
                    asserts.assert_true(
                        self.is_ip_in_range(
                            net['ip'],
                            updated_storage_custom[0],
                            updated_storage_custom[-1]))
                if 'management' in net['name']:
                    asserts.assert_true(
                        self.is_ip_in_range(
                            net['ip'],
                            updated_mgmt_custom[0],
                            updated_mgmt_custom[-1]))
