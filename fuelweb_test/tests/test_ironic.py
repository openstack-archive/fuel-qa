...
class TestIronic(base_test_case.TestBasic):
    ...
    def add_ironic_to_cluster(self):
        ...
        self.fuel_web.update_nodes(  # Ironic role must be here
    ...
    @test(depends_on=[base_test_case.SetupEnvironment.prepare_slaves_5],
          groups=["ironic"])
    def add_ironic_to_ha_cluster(self):
        """Add ironic node to cluster in HA mode
        ...
        Scenario:
            ...
            4. Validate cluster was set up correctly, there are no dead
               services, there are no errors in logs
            5. Add 2 node with role Ironic
            6. Deploy changes
            7. Validate cluster was set up correctly, there are no dead
               services, there are no errors in logs
            8. Verify services list on compute nodes
            9. Run OSTF

        Duration 60m
        Snapshot: add_ironic_to_ha_cluster
        ...
        self.fuel_web.update_nodes(
            cluster_id,
            {
                'slave-03': ['compute'],  # Ironic role must be here
                'slave-04': ['compute']  # Ironic role must be here
            },
            True, False)
        ...
        self.env.make_snapshot("add_ironic_to_ha_cluster")
