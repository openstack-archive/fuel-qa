...
class LCMTestBasic(TestBasic):
    ...
    @staticmethod
    def upload_patched_puppetd(nailgun_node):
        ssh = SSHManager()
        ip = nailgun_node['ip']
    ...
    def check_extra_tasks(self, slave_nodes, deployment):
        ...
        failed_nodes = [node_refs
                        ...
                        result['extra_actual_tasks'].items()
                        if not failed_tasks]
        return failed_nodes

    def generate_fixture(self, deployment, node_refs):
...
class SetupLCMEnvironment(LCMTestBasic):
    ...
    def deploy_1_ctrl_1_cmp_1_cinder(self):
        ...
        if not node_refs:
            self.show_step(8)
            self.generate_fixture(deployment, node_refs)
