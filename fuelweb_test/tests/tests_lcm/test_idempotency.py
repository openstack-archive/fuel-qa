...
class NeutronTun(TestBasic):
    ...
    def test_idempotency(self):
        ...
        puppetd_local = os.path.join(root_dir, "puppetd.rb")
        fixtures_home = os.path.join(root_dir, "fixtures/1_ctrl_1_cmp_1_cinder")
        tasks_description = self.get_tasks_description()
        result = []

        for node in slave_nodes:
            # Check only compute for debug purposes
            if node['roles'] != ["compute"]:
                continue

            fixture = yaml.load(open(os.path.join(
                fixtures_home, "{0}.yaml".format(node['roles'][0]))))

            tasks = self.get_nodes_tasks(node['id'])
            first_assert = ("First assert:\nCompare: {0}\n{1}\n\nResult: {3}"
                            .format(tasks, fixture['tasks'].keys(),
                                    all([expected in fixture['tasks'].keys()
                                         for expected in tasks])))
            result.append(first_assert)
            logger.info(first_assert)
            ...
            for task in tasks:
                ...
                if not self.check_task_type(tasks_description, task, 'puppet'):
                    logger.info("Skip checking of {0} task, it is not puppet, fixture: {1}"
                                .format(task, fixture_task))
                    # second_assert = (
                    #     "Check task {0}, result {1}".format(
                    #         task, fixture['tasks'].
                    #               get(task, {}).get("no_puppet_run") == True))
                    # result.append(second_assert)
                    continue
                ...
                try:
                    ...
                    if fixture_task.get("no_puppet_run") is None:
                        second_assert = "Unexpected no puppet run for task: {0}, expected {1}".format(task, fixture_task)
                        logger.info(second_assert)
                        result.append(second_assert)
                        continue
                    else:
                        raise
                ...
                for resource_name, resource_stats in report['resource_statuses'].items():
                    if resource_stats['changed'] and resource_name not in skip:
                        temp = True
                        third_assert = "Failed task, resource: {0} {1}".format(task, resource_name)
                        logger.info(third_assert)
                        result.append(third_assert)
