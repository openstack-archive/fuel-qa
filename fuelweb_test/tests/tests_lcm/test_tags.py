...
class TagsCRUD(TestBasic):
    ...
    def tags_crud(self):
        ...
        for parent_id, parent, tag_name in \
            ...
            err_msg = 'Tags current data: "{new_data}" is not equal ' \
                      'to the data on creation: "{old_data}"'.format(
                        new_data=new_tag_data, old_data=tag_data)
            ...
            err_msg = 'Newly created tag {tag_name} ' \
                      'is not in the list of tags {all_tags}'.format(
                        tag_name=tag_name, all_tags=all_tags)
            ...
            all_tags = self.nailgun.get_all_tags(parent_id, parent)
            err_msg = 'Deleted tag {tag_name} is still presented ' \
                      'in the list of tags {all_tags}'.format(
                        tag_name=tag_name, all_tags=all_tags)
    ...
    def roles_to_tags(self):
        ...
        self.show_step(3)
        role_data['meta']['tags'].append("non_existing_tag")
        exp_code = 400
        try:
            self.nailgun.add_new_role(rel_id, role_data)
        except HttpError as exc:
