...
class Upgrade9X(TestBasic):
    ...
    def upgrade_9x(self):
        ...
        def show_step_for_sub_test(test_method):
            def wrapper(step, details='', initialize=False):
                ...
                if str(step) in steps:
                    logger.info("\n" + " " * 55 + "<<< {0} {1}>>>".format(
                        steps[str(step)], details_msg
                    ))
                else:
                    logger.info("\n" + " " * 55 + "<<< {0}. {1}>>>".format(
                        str(step), details_msg
                    ))
            return wrapper
        ...
        for test_case in test_group:
            ...
            test_instance = test_method.im_class()
            test_instance.show_step = show_step_for_sub_test(test_method)

            for dependency in test_case.info.depends_on:
                dependency(test_instance)
