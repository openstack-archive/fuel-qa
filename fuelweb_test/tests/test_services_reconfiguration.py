...
class ServicesReconfiguration(TestBasic):
    ...
    def check_nova_ephemeral_disk(self, os_conn, cluster_id,
        ...
        instance = os_conn.create_server_for_migration(
            neutron=True, label=net_name, flavor=6, **kwargs)
    ...
    def reconfiguration_scalability(self):
        ...
        self.show_step(6)
        bs_nodes = self.env.bootstrap_nodes(self.env.d_env.get_nodes()[5:6],
                                            skip_timesync=True)
        target_compute = [bs_nodes[0]]
        target_controller = [bs_nodes[1]]
        self.fuel_web.update_nodes(
            cluster_id,
            {'slave-05': ['compute']})
        self.fuel_web.update_nodes(
        ...
        hypervisor = self.os_conn.get_hypervisors()[1].hypervisor_hostname
        self.check_nova_ephemeral_disk(os_conn, cluster_id,
