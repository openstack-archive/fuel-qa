...
def get_structured_config_dict(config):
    ...
    for key in config.keys():
        ...
        if key == 'neutron_api_config':
            helper(key, '/etc/neutron/neutron_api_config')
        if key == 'nova_config':
...
class ServicesReconfiguration(TestBasic):
    ...
    def reconfigure_with_new_fields(self):
        ...
        Scenario:
            ...
            4. Upload a new openstack configuration for controller
            5. Wait for configuration applying
            6. Check that neutron related services were restarted
            7. Verify configuration file on each controller
            8. Upload a new openstack configuration for compute
            9. Get uptime of keystone process on each compute
            10. Get uptime of nova-compute on each compute
            11. Wait for configuration applying
            12. Check that nova-compute service was restarted
            13. Check that keystone service was restarted
            14. Verify configuration file on each compute
        ...
        self.show_step(6)
        for service_name in service_list:
            self.check_service_was_restarted(controllers,
        ...
        self.show_step(10)
        uptimes_keystone = self.get_service_uptime(computes, 'keystone')
