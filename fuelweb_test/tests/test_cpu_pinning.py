...
class NumaCpuPinning(TestBasic):
    ...
    @logwrap
    def assert_entry_in_config(self, conf, conf_name, section, option, value):
    ...
    @logwrap
    def assert_quantity_in_config(self, conf, conf_name, section, option,
                                  value):
        ...
        :param section: a string of section name in configuration file
        :param option: a string of option name in configuration file
        :param value: !!!!!!!!!an int length of strings in configuration file
        """
        current_value = conf.get(section, option)
    ...
    def create_pinned_instance(self, os_conn, cluster_id,
        ...
        aggregate = os_conn.create_aggregate(name, metadata=meta,
                                             hosts=[hostname])
    ...
    def cpu_pinning_on_two_compute(self):
        ...
        first_config = self.fuel_web.client.get_node_attributes(
            first_compute['id'])
        first_config['cpu_pinning']['nova']['value'] = str(
            first_compute_cpu - 1)
        first_config['cpu_pinning']['dpdk']['value'] = '0'
        first_config['hugepages']['dpdk']['value'] = '0'
        ...
        second_config['cpu_pinning']['dpdk']['value'] = '0'
        second_config['hugepages']['dpdk']['value'] = '0'
        self.fuel_web.client.upload_node_attributes(
        ...
        asserts.assert_equal(
            ...
            str(first_compute_cpu - 1),
            "CPU pinning wasn't applied on {}".format(first_compute['ip']))
        ...
        asserts.assert_equal(
            ...
            "1",
            "CPU pinning wasn't applied on {}".format(second_compute['ip']))
