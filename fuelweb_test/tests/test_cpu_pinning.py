...
class NumaCpuPinning(TestBasic):
    ...
    def basic_env_for_numa_cpu_pinning(self):
        ...
        elif not (settings.DRIVER_ENABLE_ACPI):
            raise exceptions.FuelQAVariableNotSet(
                settings.DRIVER_ENABLE_ACPI,
                'true')

        elif not (settings.HARDWARE['slave_node_cpu'] == 6):
            raise exceptions.FuelQAVariableNotSet(
                settings.HARDWARE['slave_node_cpu'],
                6)
