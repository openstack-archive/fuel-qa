...
class NumaCpuPinning(TestBasic):
    """NumaCpuPinning."""
    def __init__(self):
        super(NumaCpuPinning, self).__init__()
        self.min_numa_count = 1

    def get_numa_from_master(self, node):
        self.min_numa_count = len(
    ...
    def basic_env_for_numa_cpu_pinning(self):
        ...
        for node in ('slave-01', 'slave-02'):
            ...
            numas_from_fuel = self.get_numa_from_master(target_node)
            ...
            if not numas_on_remote:
                ...
                asserts.assert_equal(numas_on_remote, numas_from_fuel,
                                     ...
                                     "has {2}".format(
                                         ...
                                         numas_from_fuel))
                logger.info("There is {0} NUMA nodes on node {}".format(
                    numas_on_remote, target_node['ip']))
