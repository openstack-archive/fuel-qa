...
class TestCgroupHa(TestBasic):
    ...
    def check_cgconfig_setup(self, config, service, resource,
                             limit=None, value=None):
        actual_limit = config[service][resource]
    ...
    def deploy_ha_cgroup(self):
        ...
        for nailgun_node in n_ctrls:
            ...
            self.check_cgconfig_setup(
                config=cgroups_config, service="keystone",
                resource="cpu", limit="cpu.shares", value=70)
    ...
    def apply_cgroups_after_deploy(self):
        ...
        self.fuel_web.client.put_deployment_tasks_for_cluster(
            ...
            data=["cgroups"],
            node_id=",".join(nailgun_node['id'] for nailgun_node in n_ctrls))

        for nailgun_node in n_ctrls:
            cgroups_config = self.get_cgroups_config(nailgun_node)
            self.check_cgconfig_setup(
                config=cgroups_config, service="mysqld",
                resource="memory", limit="memory.swappiness", value=0)
            self.check_cgconfig_setup(
                config=cgroups_config, service="rabbitmq",
                resource="blkio", limit="blkio.weight", value=500)
            self.check_cgconfig_setup(
                config=cgroups_config, service="rabbitmq",
                resource="memory", limit="memory.swappiness", value=0)
            self.check_cgconfig_setup(
                config=cgroups_config, service="keystone",
                resource="cpu", limit="cpu.shares", value=50)

            lscgroups = ["cpu:/keystone", "memory:/rabbitmq",
                         "memory:/mysqld", "blkio:/rabbitmq"]
            for lscgroup in lscgroups:
            ...
            cgrules = [("mysqld", "memory"), ("keystone", "cpu"),
                       ("rabbitmq", "blkio"), ("rabbitmq", "memory")]

            for cgrule in cgrules:
