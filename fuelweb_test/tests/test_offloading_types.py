...
class TestOffloading(TestBasic):
    ...
    def offloading_neutron_vlan(self):
        ...
        iface1 = settings.iface_alias('eth1')
        ...
        modes = []
        modes.append(
            self.prepare_offloading_modes(iface1, offloadings_1, False))
        modes.append(
            self.prepare_offloading_modes(iface2, offloadings_2, True))

        logger.info("offloading_modes = {}".format(modes))
    ...
    def offloading_neutron_vxlan(self):
        ...
        for node in nodes:
            ...
            for offload_type in self.offloadings_1:
                if offload_type in modes:
                    offloadings_1.append(offload_type)
            modes = self.fuel_web.get_offloading_modes(node['id'], [iface2])
            for offload_type in self.offloadings_2:
                if offload_type in modes:
                    offloadings_2.append(offload_type)
