...
class TestOffloading(TestBasic):
    ...
    for iface in INTERFACES:
        interfaces[iface] = list(INTERFACES[iface])
    ...
    def offloading_neutron_vlan(self):
        ...
        offloading_modes = [{
            ...
            'offloading_modes': [{
                ...
                'sub': []}]}, {
            'name': self.interfaces['management'],
            'offloading_modes': [{
                'state': 'false',
        ...
        for node in nodes:
            with self.env.d_env.get_ssh_to_remote(node['ip']) as remote:
                ...
                result = check_offload(
                    remote, self.interfaces['public'], 'rx-vlan-offload')
                assert_equal(result, "on",
                             "Offload type {0} is {1} on remote host"
                             .format('rx-vlan-offload', result))

                result = check_offload(
                    remote, self.interfaces['public'], 'tx-vlan-offload')
                assert_equal(result, "on",
                             "Offload type {0} is {1} on remote host"
