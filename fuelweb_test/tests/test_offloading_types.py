...
class TestOffloading(TestBasic):
    ...
    def check_offload(self, node, eth, offload_type):
        command = ("ethtool --show-offload %s | grep %s "
                   "| awk {'print $2'}")
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["offload_nova", "offload"])
    @log_snapshot_after_test
    def offloading_nova(self):

        """Deploy cluster with new offload modes and nova flat
        Scenario:
            1. Create cluster
        ...
        for remote in remotes:
            ...
            result = self.check_offload(remote, 'eth1', 'tx-nocache-copy')
            logger.info("Verify Offload types")
            assert_equal(result, "on",
                         "Offload type {0} is {1} "
                         "on remote host".format('tx-nocache-copy', result))
            result = self.check_offload(remote, 'eth1', 'rx-all')
            assert_equal(result, "on",
                         "Offload type {0} is {1} "
                         "on remote host".format('rx-all', result))
            result = self.check_offload(remote, 'eth1', 'rx-all')
            assert_equal(result, "on",
                         "Offload type {0} is {1} "
                         "on remote host".format('rx-all', result))
            result = self.check_offload(remote, 'eth2', 'rx-checksumming')
            assert_equal(result, "off",
                         "Offload type {0} is {1} "
                         "on remote host".format('rx-checksumming', result))

    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["offload_neutron", "offload"])
    @log_snapshot_after_test
    def offloading_neutron(self):
        ...
        for remote in remotes:
            ...
            assert_equal(result, "on",
                         "Offload type {0} is {1} "
                         "on remote host".format('rx-vlan-offload', result))
            result = self.check_offload(remote, 'eth1', 'tx-vlan-offload')
            assert_equal(result, "on",
                         "Offload type {0} is {1} "
                         "on remote host".format('tx-vlan-offload', result))
            result = self.check_offload(remote, 'eth2',
                                        'large-receive-offload')
            assert_equal(result, "off",
                         "Offload type {0} is {1} "
                         "on remote host".format('large-receive-offload',
                                                 result))
