...
class TestSRIOV(TestBasic):
    ...
    def deploy_cluster_with_sriov(self):
        ...
        Scenario:
            ...
            3. Add 3 controller, 1 cinder and 2 compute nodes
        ...
        sriov_nailgun_nodes = [node['name'] for node in nodes
                               if self.check_sriov(node)]
        ...
        assert_true(len(sriov_nodes) >= 2,
                    'At least 2  nodes with SR-IOV support are required for '
                    'this test! But, only {0} nodes are '
                    'available!'.format(sriov_nodes)
                    )
        self.show_step(3)
