...
class UnlockSettingsTab(TestBasic):
    ...
    def revert_snapshot(self, nodes_count):
        """
        :param nodes_count: the dict with nodes
    ...
    def get_cluster_attribs(self):
        return self.fuel_web.client.get_cluster_attributes(self.cluster_id)

    def update_cluster_attribs(self, new_attrs):
        try:
    ...
    def deploy_with_redeploy_and_modify_settings(self):
        ...
        Duration xxx m
        Snapshot for all failed configurations
        """
        self.show_step(1)
        for conf in self.load_config('cluster_configs.json'):
            logger.info(
            ...
            self.show_step(9)
            if not self.update_cluster_attribs(new_attrs):
                continue
