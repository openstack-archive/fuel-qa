...
class UnlockSettingsTab(TestBasic):
    ...
    def unlock_settings_tab_positive(self):
        ...
        current_networks =\
            self.fuel_web.client.get_networks(cluster_id)['networks']
        # current_networks = current_networks['networks']
        #
        ...
        self.show_step(11)
        assert_equal(current_cluster_settings, deployed_settings,
                     message="Cluster settings before deploy"
                             " are not equal with deployed settings")
        assert_equal(current_networks, deployed_net_conf,
        ...
        self.show_step(13)
        assert_equal(default_config, default_settings,
                     message="Default settings are not equal")
        self.show_step(14)
        self.fuel_web.redeploy_cluster_changes_wait_progress(cluster_id=cluster_id,
                                                             progress=30)
        self.show_step(15)
        self.fuel_web.stop_deployment_wait(cluster_id)
        self.show_step(16)
