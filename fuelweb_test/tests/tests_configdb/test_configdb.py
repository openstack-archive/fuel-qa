...
@test(groups=["base_test_configdb"])
class TestsConfigDB(TestBasic):
    """Tests ConfigDB"""  # TODO documentations

    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["install_configdb_extension", "smoke_test_configdb"])
    ...
    def install_configdb_extension(self):
        ...
        logger.info('Check creating component')
        a = self.fuel_web.client.post_components(component)
        logger.info("DEBUG_1: {}".format(a))
        logger.info('Check creating env with component')
        b = self.fuel_web.client.post_envirionments(env)
        logger.info("DEBUG_2: {}".format(b))

        logger.info('Check getting env resource value')
        global_res = self.fuel_web.client.get_global_resource_value(
            env_id=1, resource_id=1)
        logger.info('Check getting node resource value')
        ...
        logger.info('Upload global and node value')
        ...
        logger.info('Check global overrided data affects on node level')
        node_effective = self.fuel_web.client.get_node_resource_value(
            env_id=1, resource_id=1, node_id=1, effective=True)
        assert_equal(node_effective, merged_value)

        logger.info('Upload new global data')
        global_new = {'global_key': 'global_new'}

        self.fuel_web.client.put_global_resource_value(
            env_id=1, resource_id=1, data=global_new)

        logger.info('Check new global data does not affect on node level')
        node_effective = self.fuel_web.client.get_node_resource_value(
            env_id=1, resource_id=1, node_id=1, effective=True)
        assert_equal(node_effective, merged_value)
        ...
        logger.info('Check second nodes data has only global overrided data')
        # Beacuse Overrided global data takes prority
        node_effective = self.fuel_web.client.get_node_resource_value(
