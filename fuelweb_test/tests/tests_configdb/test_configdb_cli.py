...
class TestsConfigDBAPI(TestBasic):
    ...
    def merge_overrides_with_level(self):
        ...
        res = self.ssh_manager.check_call(
            admin_ip, get_resource_cmd)['stdout_str']
        res_obj = json.loads(res)
        # TODO(akostrikov) Bug on leaking level
        """
        [root@nailgun ~]# fuel2 config set --env 2 --resource 3 --value
        '{"a": 1, "b": null}' --key key  --type json --level nodes=1
        [root@nailgun ~]# fuel2 config get --env 2 --resource 3 --level nodes=1
        {"key": {"a": 1, "b": null}}
        [root@nailgun ~]# fuel2 config get --env 2 --resource 3
        {"key": {"a": 1, "b": null}}
        """
        assert_equal(res_obj, {})
