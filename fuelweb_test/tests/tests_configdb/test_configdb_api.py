...
from fuelweb_test.models.nailgun_client import ConfigDBClient
...
class TestsConfigDBAPI(TestBasic):
    ...
    def __init__(self):
        super(TestsConfigDBAPI, self).__init__()
        self._configdb_client=None
    
    @property
    def configdb_client(self):
        if not self._configdb_client:
            self._configdb_client = ConfigDBClient(
                self.env.get_admin_node_ip())
        return self._configdb_client
    ...
    def create_component_and_env_configdb(self):
        ...
        _wait(lambda: self.configdb_client.get_components(),
              timeout=45)

        logger.debug('Get env and component data')
        components = self.configdb_client.get_components()
        envs = self.configdb_client.get_environments()
        ...
        self.show_step(3)
        self.configdb_client.create_component(component)
        self.show_step(4)
        self.configdb_client.create_environment(environment)
        self.show_step(5)
        comp = self.configdb_client.get_components(comp_id=1)
        env = self.configdb_client.get_environments(env_id=1)
    ...
    def get_upload_resource_value(self):
        ...
        self.show_step(2)
        global_res = self.configdb_client.get_global_resource_id_value(
            env_id=1, resource_id=1)
        self.show_step(3)
        node_res = self.configdb_client.get_node_resource_id_value(
        ...
        self.configdb_client.put_node_resource_value(
            env_id=1, resource=1, node_id=1, data=node_value)
        self.configdb_client.put_global_resource_value(
            env_id=1, resource=1, data=global_value)

        self.show_step(5)
        glob = self.configdb_client.get_global_resource_id_value(
            env_id=1, resource_id=1)
        glob_eff = self.configdb_client.get_global_resource_id_value(
        ...
        self.show_step(6)
        node_uploaded_n = self.configdb_client.get_node_resource_name_value(
            env_id=1, resource_name=self.RESOURCE_NAME_1, node_id=1)
        global_uploaded_n = \
            self.configdb_client.get_global_resource_name_value(
                env_id=1, resource_name=self.RESOURCE_NAME_1)
        assert_equal(global_uploaded_n, glob)

        self.show_step(7)
        node_uploaded = self.configdb_client.get_node_resource_id_value(
            env_id=1, resource_id=1, node_id=1)
        node_effective = self.configdb_client.get_node_resource_id_value(
        ...
        glob_slashed = self.configdb_client.get_global_resource_id_value(
            env_id=1, resource_id=2)
        glob_slashed_n = self.configdb_client.get_global_resource_name_value(
    ...
    def override_resource_value(self):
        ...
        self.show_step(3)
        self.configdb_client.put_global_resource_override(
        ...
        self.configdb_client.put_node_resource_overrides(
