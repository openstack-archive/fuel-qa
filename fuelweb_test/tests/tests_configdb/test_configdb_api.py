...
class TestsConfigDBAPI(TestBasic):
    ...
    def create_component_and_env_configdb(self):
        ...
        env = self.fuel_web.client.get_envirionments(env_id=1)
        ...
        logger.info('Compare original component with '
                    'received component from API')
        assert_equal(comp, expected_comp)
        logger.info('Compare original env with received env from API')
    ...
    def get_upload_resource_value(self):
        ...
        logger.info('Get global value by resource id and compare with'
                    'original global value')
        assert_equal(glob, global_value)
        logger.info('Get global effective value by resource id and compare'
        ...
        logger.info('Get node value by resource id and compare with'
                    'original node value')
        assert_equal(node_uploaded, node_value)
        logger.info('Get node value by resource name and compare with'
                    'original node value')
