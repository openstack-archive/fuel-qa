...
class TestsConfigDBAPI(TestBasic):
    ...
    def test_create_component_and_env_configdb(self):
        ...
        self.check_run('test_create_component_and_env_configdb')
        self.show_step(1)
        ...
        self.show_step(4)
        self.fuel_web.client.post_envirionments(environment)
        self.show_step(5)
        comp = self.fuel_web.client.get_components(id=1)
        env = self.fuel_web.client.get_envirionments(id=1)
    ...
    def test_get_upload_resource_value(self):
        ...
        Scenario:
            ...
            6. Check getting resource value by resource name redirects to
        ...
        """
        self.show_step(1)
        self.env.revert_snapshot('test_create_component_and_env_configdb')
        ...
        node_uploaded_n = self.fuel_web.client.get_node_resource_value(
            env_id=1, resource=self.RESOURCE_NAME_1, node_id=1)
        global_uploaded_n = self.fuel_web.client.get_global_resource_value(
            env_id=1, resource=self.RESOURCE_NAME_1)
        ...
        glob_slashed = self.fuel_web.client.get_global_resource_value(
            env_id=1, resource=2)
        glob_slashed_n = self.fuel_web.client.get_global_resource_value(
            env_id=1, resource=self.SLASHED_RESOURCE)
