...
def exit_codes_success():
    """Exit codes to expect when command successful"""
    return [0]


def exit_codes_file_not_found():
    """Exit codes to expect when file not found"""
    return [2]


def exit_codes_service_not_found():
    """Exit codes to expect when service not found"""
    return [1]
...
class TestCustomGraph(TestBasic):
    ...
    def move_ubuntu_target_image(self, release_id, cluster_id):
        ...
        move_img_cmd = (
            ...
            '_ubuntu_1404_amd64.yaml').format(
    ...
    def pre_provision_ubuntu_slaves_3(self):
        ...
        upload_tasks_cmd = 'cd /tmp && python /tmp/prepare_release_image.py'
        self.ssh_manager.execute_on_remote(
            ip=self.ssh_manager.admin_ip,
    ...
    @test(depends_on=[pre_provision_ubuntu_slaves_3],
          groups=['custom_graph', 'graph_merge', 'prepare_test_graph_env'])
    @log_snapshot_after_test
    def merge_cluster_and_release(self):
        ...
        Scenario:
            ...
            8. Upload 'custom_graph' tasks to cluster
            9. Download 'custom_graph' deployment graph
            10. Verify that 'custom_graph' is a merge of
             release and cluster graphs.
            11. Run 'custom_graph' deployment.
            12. Verify that 'custom_graph' release tasks
             are called on all nodes
            13. Verify that 'custom_graph' cluster tasks
             are called on all nodes
            14. Create snapshot
        ...
        custom_tasks = self.fuel_web.client.get_custom_cluster_deployment_tasks(
            cluster_id,
            graph_type)

        release_tasks_filename = 'release_custom_tasks.yaml'
        local_tasks_file = os.path.join(os.path.dirname(fuelweb_test.__file__),
                                        'config_templates',
                                        release_tasks_filename)
        with open(local_tasks_file, 'r') as yaml_file:
            release_tasks_yaml_data = yaml.load(yaml_file)

        release_tasks_filename = 'custom_graph_tasks.yaml'
        local_tasks_file = os.path.join(os.path.dirname(fuelweb_test.__file__),
                                        'config_templates',
                                        release_tasks_filename)
        with open(local_tasks_file, 'r') as yaml_file:
        ...
        assert_equal(diff, set([]), 'The diff is not empty!')
    ...
    def information_at_graphs_handler(self):
        ...
        check_default_cmd = 'fuel2 graph list -e 1|grep default'
        self.ssh_manager.execute_on_remote(
            ip=self.ssh_manager.admin_ip,
            cmd=check_default_cmd)

        self.show_step(10)  # Verify list shows 'custom' tasks
        check_custom_cmd = 'fuel2 graph list -e 1|grep custom'
        self.ssh_manager.execute_on_remote(
            ip=self.ssh_manager.admin_ip,
            cmd=check_custom_cmd)

        self.show_step(11)  # Verify list shows 'yaql' tasks
        check_yaql_cmd = 'fuel2 graph list -e 1|grep yaql'
        self.ssh_manager.execute_on_remote(
            ip=self.ssh_manager.admin_ip,
            cmd=check_yaql_cmd)
