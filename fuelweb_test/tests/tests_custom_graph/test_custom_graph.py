...
class TestCustomGraph(TestBasic):
    ...
    def merge_cluster_and_release(self):
        ...
        cl = self.fuel_web.client.get_custom_cluster_deployment_tasks(
            cluster_id,
            graph_type)
        ...
        generated_names = set([x['task_name'] for x in cl])
        uploaded_names = set(
            [x['id'] for x in release_tasks_yaml_data] +
            [x['id'] for x in cluster_tasks_yaml_data])
    ...
    @test(depends_on=[pre_provision_ubuntu_slaves_3],
          groups=['custom_graph', 'custom_graph_edges'])
    @log_snapshot_after_test
    def reexecute_on_custom_graph(self):
