...
class IBPTest(TestBasic):
    ...
    def check_ibp_add_package(self):
        ...
        logger.debug('Adding lynx to the initial packages list')
        pkg_list.add('lynx')
    ...
    def check_ibp_remove_package(self):
        ...
        Scenario:
            ...
            3. Add one node to cluster
            4. Provision nodes
            5. Check that all packages from list are installed, removed
            package is not
        ...
        logger.debug('Removing curl from the initial packages list')
        pkg_list.remove('curl')
        pkg_list = self.fuel_web.update_cluster_ibp_packages(
            cluster_id, pkg_list)
        logger.debug(
            'Cluster IBP packages after update: {}'.format(sorted(pkg_list))
    ...
    def check_ibp_add_wrong_package(self):
        ...
        Check that provision ends successfully.
        ...
        Scenario:
            ...
            3. Add one node to cluster
            4. Provision nodes
            5. Check that all packages besides wrong one are installed
        ...
        self.show_step(5)
        self.check_node_packages('slave-01', default_pkg_list)
