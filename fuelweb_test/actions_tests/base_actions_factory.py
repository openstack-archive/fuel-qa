...
from fuelweb_test.helpers.decorators import step_start_stop
...
class BaseActionsFactory(base_test_case.TestBasic):
    ...
    def caseclass_factory(cls, case_group):
        ...
        for step, action in enumerate(cls.get_actions_order()):
            ...
            step_method_name = "{}__Step{:03d}_{}".format(class_name,
                                                          step,
                                                          n_action)
            ...
            if step > 0:
                prev_step_name = "{}__Step{:03d}_{}".format(
                    class_name,
                    step - 1,
        ...
        start_method = utils.copy_func(
            ...
            "{}__Start".format(class_name))
        test_steps["{}__Start".format(class_name)] = before_class(
            start_method)
        finish_method = utils.copy_func(
            getattr(cls, "_finish_case"),
            "{}__Finish".format(class_name))
        test_steps["{}__Finish".format(class_name)] = after_class(
            finish_method)
