...
def map_test():
    ...
    for my_test in program.plan.tests:
        if all(patching_group in my_test.entry.info.groups for
            ...
            break
    if deployment_test:
        if errata['installed_on'] == 'master':
            #register(test_home=deployment_test.entry.home,
            #         depends_on_groups='patching_test')
            del deployment_test.entry.info.depends_on_groups[:]
            deployment_test.entry.info.depends_on_groups.append('patching_test')
            register(groups='patching_master',
