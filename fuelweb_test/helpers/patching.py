...
def get_errata(path, bug_id, method):
    errata = None
    scenario_path = '{0}/bugs/{1}/errata.yaml'.format(path, bug_id)
    if method == 'remote':
        errata = yaml.load(urlopen(scenario_path).read())
    elif method == 'local':
        with open(scenario_path) as f:
            errata = yaml.load(f.read())
    return errata


def get_script_content(path, bug_id, script, method):
    script_content = None
    scripts_path = '{0}/bugs/{1}/tests/{2}'.format(path, bug_id, script)
    if method == 'remote':
        script_content = urlopen(scripts_path).read()
    elif method == 'local':
