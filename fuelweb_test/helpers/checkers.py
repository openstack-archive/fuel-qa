...
def external_ntp_check(remote_slave, vrouter_vip):
    ...
    try:
        wait(
            lambda: not remote_slave.execute(status_command)['exit_code'],
            timeout=120)
    except Exception as e:
        logger.error(e)
        status = remote_slave.execute(status_command)
        assert_equal(
            status['exit_code'], 0, "Failed updated ntp. Command execution "
                                    "result is {0}".format(status))
