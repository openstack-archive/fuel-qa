...
def check_log_lines_order(remote, log_file_path, line_matcher):
    ...
    for current_line in line_matcher:
        ...
        # line not found case
        assert_equal(res['exit_code'] or len(res['stdout']) < 1,
                     0,
                     "Line {0} not found after line {1} in the file {2}"
                     .format(current_line, previous_line, log_file_path))

        # few lines found case
        assert_true(len(res['stdout']) > 1,
                    "Found {0} lines like {1} but should be only 1 in {2}: {3}"
                    .format(len(res['stdout']),
                            current_line,
                            log_file_path,
                            res['stdout']))

        current_line_pos = res['stdout'][0]
