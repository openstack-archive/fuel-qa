...
def check_intance_ip_version(instance, network, ip_version):
    """Check ip version of network which was assigned to instance

    :param instance: obj, object of instance
    :param network: obj, object of assigned network
    :param ip_version: int, ip version
    :return:
    """
    instance_ip = [addr['addr']
                   for addr in instance.addresses[network['name']]
                   if addr['OS-EXT-IPS:type'] == 'fixed'][0]
    logger.debug(
        '\ninstance: {instance_id!s}\n'
        '\tIPv6 address: {ipv6!s}'.format(
            instance_id=instance.id,
            ipv6=instance_ip))
    assert_equal(instance_ip['version'], ip_version,
                 'Current ip does not match version {}. Details:\n {}'
                 .format(ip_version, instance_ip))
