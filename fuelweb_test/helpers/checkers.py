...
@logwrap
def get_ceph_partitions(remote, device, fstype="xfs"):
    ret = remote.check_call("parted {device} print | grep {fstype}".format(
                            device=device, type=fstype))['stdout']
    if not ret:
