...
def check_snapshot_logs(ip, snapshot_name, cluster_id, controllers, computes):
    ...
    def find_absent_files(ip, path, files):
        ...
        for file in files:
            ...
            _path = path.format(file_path=file)
            cmd = "ls {}".format(_path)
    ...
    absent_files.append(
        find_absent_files(
            ip=ip,
            path='{dump_path}/{hostname}/{{file_path}}'.format(
    ...
    absent_files.append(
        find_absent_files(
            ip=ip,
            path='{dump_path}/config/files/cluster-{cluster_id}'
                 '/{hostname}/{{file_path}}'.format(
                dump_path=snapshot_path_master,
    ...
    absent_files.append(
        find_absent_files(
            ip=ip,
            path='{dump_path}/config/cmds/cluster-{cluster_id}'
                 '/{hostname}/{hostname}-{ip}-{{file_path}}'.format(
                dump_path=snapshot_path_master,
                cluster_id='0',
                hostname=master_hostname,
                ip='127.0.0.1'
    ...
    for controller in controllers:
        ...
        absent_files.append(
            find_absent_files(
                ip=ip,
                path='{dump_path}/{hostname}/var/log/remote'
        ...
        absent_files.append(
            find_absent_files(
                ip=ip,
                path="{dump_path}/logs-{hostname}/{{file_path}}".format(
                    dump_path=snapshot_path_master,
        ...
        for compute in computes:
            ...
            absent_files.append(
                find_absent_files(
                    ...
                    files=snapshot_logs['master']['remote'][
                        'compute'].split()
                )
            )
            # COMPUTES ITSELF
