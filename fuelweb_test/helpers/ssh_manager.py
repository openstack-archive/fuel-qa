...
from fuelweb_test.helpers.utils import SingletonMeta
...
class SSHManager(object):
    ...
    def __init__(self):
        logger.info('SSH_LOG: Run constructor SSHManager')
        self.connections = {}
        self.admin_ip = None
    ...
    def _get_remote(self, ip, port=22):
        ...
        if (ip, port) not in self.connections:
            logger.info('SSH_LOG:Create new connection for {ip}:{port}'.format(
                ip=ip,
                port=port
            ))
        ...
        logger.info('SSH_LOG:Return existed connection for {ip}:{port}'.format(
            ip=ip,
            port=port
        ))
        logger.info('SSH_LOG: {0}'.format(self.connections))
