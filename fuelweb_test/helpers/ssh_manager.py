...
class SSHManager(object):
    ...
    def get_remote(self, ip, port=22, sudo=False):
        ...
        self.connections[(ip, port)].sudo_mode = sudo
    ...
    def execute(self, ip, cmd, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
        return remote.execute(cmd)

    def check_call(self, ip, cmd, port=22, verbose=False, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
    ...
    def execute_on_remote(self, ip, cmd, port=22, err_msg=None,
                          ...
                          raise_on_assert=True, sudo=False):
    ...
    def execute_async_on_remote(self, ip, cmd, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
    ...
    def open_on_remote(self, ip, path, mode='r', port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
        return remote.open(path, mode)

    def upload_to_remote(self, ip, source, target, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
    ...
    def exists_on_remote(self, ip, path, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
        return remote.exists(path)

    def isdir_on_remote(self, ip, path, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
        return remote.isdir(path)

    def isfile_on_remote(self, ip, path, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
        return remote.isfile(path)

    def mkdir_on_remote(self, ip, path, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
        return remote.mkdir(path)

    def rm_rf_on_remote(self, ip, path, port=22, sudo=False):
        remote = self.get_remote(ip=ip, port=port, sudo=sudo)
        return remote.rm_rf(path)

    def cond_upload(self, ip, source, target, port=22, condition='',
                    clean_target=False, sudo=False):
