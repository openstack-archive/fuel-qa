...
class SSHManager(object):
    ...
    def __init__(self, admin_ip=None, login=None, password=None):
        """ It only runs the first time you call class.

        :param admin_ip: admin_ip
        :param login: login
        :param password: password
        """
        logger.info('SSH_MANAGER: Run constructor SSHManager')
        self.connections = {}
    ...
    def _get_remote(self, ip, port=22):
        ...
        logger.info('SSH_MANAGER:Return existed connection for '
                     '{ip}:{port}'.format(ip=ip, port=port))
        logger.info('SSH_MANAGER: Connections {0}'.format(self.connections))
