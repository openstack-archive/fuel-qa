...
from fuelweb_test.helpers import ssh_manager_helpers
...
class SSHManager(object):
    ...
    def __getattr__(self, item):
        if item not in dir(ssh_manager_helpers):
            raise AttributeError(
                'Helper {item!s} not found'.format(item=item))
        try:
            helper = getattr(ssh_manager_helpers, item)
            return lambda *args, **kwargs: helper(self, *args, **kwargs)
        except BaseException as exc:
            tb = traceback.format_exc()
            logger.error(
                'Helper {item!s} load failed with\n'
                '\tError:     {exc!r}\n'
                '\tTraceback: {tb!s}'.format(item=item, exc=exc, tb=tb))
            raise
