...
class Common(object):
    ...
    def __init__(self, controller_ip, user, password, tenant):
        ...
        try:
            ...
            LOGGER.debug(
                'Ceilometer endpoint is {0}'.format(ceilometer_endpoint))
        except ClientException.EndpointNotFound:
            LOGGER.debug('Ceilometer endpoint not found.')

        if ceilometer_endpoint:
            self.ceilometer = ceilometerclient.Client(
                2, endpoint=ceilometer_endpoint,
                token=token, cacert=path_to_cert)
