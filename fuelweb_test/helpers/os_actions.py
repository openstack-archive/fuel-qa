...
class OpenStackActions(common.Common):
    ...
    def get_flavor_by_name(self, name):
        return [flavor for flavor in self.nova.flavors.list()
                if flavor.name == name].pop()
    ...
    def enable_nova_service(self, service, timeout=60):
        self.nova.services.enable(service.host, service.binary)
        helpers.wait(lambda: self.get_nova_service_status(service) ==
                             "enabled", timeout=timeout,
                     timeout_msg="Service {0} on {1} does not reach enabled "
                                 "state, current state "
                                 "is {2}".format(service.binary,
                                                 service.host,
                                                 service.status))

    def disable_nova_service(self, service, timeout=60):
        self.nova.services.disable(service.host, service.binary)
        helpers.wait(lambda: self.get_nova_service_status(service) ==
                             "disabled", timeout=timeout,
