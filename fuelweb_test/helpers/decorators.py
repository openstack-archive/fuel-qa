        ...
        if settings.UPDATE_FUEL:
            ...
            if centos_files_count > 0:
                ...
                try:
                    ...
                    remote.execute(
                        "for r in auxiliary nailgun; do"
                        "sed -i '/^priority=.*/d' /etc/yum.repos.d/${r}.repo;"
                        "done;"
                        "echo 'priority=20' >> /etc/yum.repos.d/nailgun.repo;"
                        "echo 'priority=15' >> /etc/yum.repos.d/auxiliary.repo"
                        )
                    res = remote.execute(
                        "for container in $(dockerctl list);do "
                        "dockerctl shell $container bash -c "
                        "'yum -y install yum-plugin-priorities'; done")
