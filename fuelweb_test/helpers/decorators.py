...
def check_network_settings(func):
    ...
    def wrapper(*args, **kwargs):
        env = get_current_env(args)
        env.fuel_web.client.generate_ip_address_before_deploy_start(
            env.fuel_web.get_last_created_cluster())
        ...
        logger.info('Network settings before deploy {}'.format(networks))
        logger.info('Network settings after deploy {}'.format(
            network_configuration))
...
def check_cluster_settings(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.info('Cluster settings before deploy {}'.format(
            cluster_attributes))
        logger.info('Cluster settings after deploy {}'.format(
            cluster_settings))
