    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            if settings.UPLOAD_PATCHSET:
                ...
                with environment.d_env.get_admin_remote() as remote:
                    remote.upload(settings.PATCH_PATH,
                                  pack_path)
                # Renew fuel-agent in Mcollective
                ...
                # Also need to renew code in boostrap

                # Step 1 - unpack bootstrap
                cmd = ("cp /var/www/nailgun/bootstrap/initramfs.img ./"
                       "mkdir initramfs"
                       "cd initramfs/"
                ...
                bootstrap_agent_path = "{}".format(
                    "initramfs/usr/lib/python2.6/site-packages/fuel_agent")
                image_rebuild = "{} | {} | {}".format(
                    "find . -xdev",
                    "cpio --create --format='newc'",
                    "gzip -9 > ../initramfs.img.updated")
                ...
                bootstrap_path = "/var/www/nailgun/bootstrap"
                cmd = ("rm {}/initramfs.img"
                       "cp ../initramfs.img.updated {}/initramfs.img"
                       "chmod +r {}/initramfs.img"
