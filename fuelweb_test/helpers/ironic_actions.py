...
class IronicActions(object):
    ...
    def create_image_for_ironic_virtual_nodes(self,
                                              image_name='virtual_trusty'):
    ...
    def create_virtual_node(self, server_ip, ldap_username, ldap_password):
        ...
        # Sleep is needed to update hypervisor hw parameters by nova
        time.sleep(180)
    ...
    def boot_ironic_vm(self, fuel_web, cluster_id, image_name, net_name, flavor_id,
        ...
        srv = self.os_conn.nova.servers.create(
            ...
            key_name=key_name,
            **kwargs)
        # TODO: TEMPORARY! Restart dhcp agent on controller
        time.sleep(60)
        controllers = fuel_web.get_nailgun_cluster_nodes_by_roles(
            cluster_id,
            roles=['controller']
        )
        for c in controllers:
            remote = fuel_web.get_ssh_for_node(c['name'].split('_')[0])
            logger.info(remote.execute('service neutron-dhcp-agent restart'))
            time.sleep(5)
