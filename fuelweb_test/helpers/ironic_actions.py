...
class IronicActions(object):
    ...
    def import_ironic_image(self, image_name='virtual_trusty', disk='vda'):
        ...
        with open(os.environ['UBUNTU_IMAGE_PATH']) as data:
            img = self.os_conn.create_image(
                name=image_name + str(random.randint(1, 0xfff)),
    ...
    # @logwrap
    def check_boot_success(self, remote, dhcp_namespace, instance_ip,
                           instance_keypair):
        # get controller ip
        # put private key there

        # TODO re-ask Vasya why public net is fake for ironic environments
        # TODO what to choose: ssh to VM from ironic node (is it possible at all with correct key?)
        # TODO or assign floating IP and ssh from anywhere (I prefer this case actually)
        ...
        cmd = (". openrc; ip netns exec {0} ssh -i {1}"
               ...
               " ubuntu@{2} \"ping -c 1 {3}\"").format(dhcp_namespace,
                                                       instance_key_path,
                                                       instance_ip,
                                                       settings.PUBLIC_TEST_IP)
        helpers.wait(lambda: remote.execute(cmd)['exit_code'] == 0,
                     timeout=2 * 60)
