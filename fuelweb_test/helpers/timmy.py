...
def collect_logs(path,
    ...
    timmy_ver = timmy.get_timmy_version(timmy_path=timmy_path)
    if not timmy_ver:
        timmy.setup_timmy()
    else:
    ...
    res = timmy.run_timmy(timmy_path=timmy_path, dest_file=remote_logs_path)
...
class Timmy():
    def __init__(self):
        ...
        if settings.TIMMY_PATH.startswith('http'):
            ...
            self.timmy_path = '/root/timmy/timmy.py'
        self.ready = False
    ...
    def get_timmy_version(self, timmy_path=None):
        ...
        res = self.run_timmy(timmy_path=timmy_path, version=True, raise_on_err=False)
        if res['exit_code']!=0:
            return False
        return res['stdout_str']

    def run_timmy(self, timmy_path=None, raise_on_err=True, dest_file=None, env_id=None, node_ids=None, roles=None,
