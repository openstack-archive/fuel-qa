...
class Timmy:
    ...
    def setup_timmy(self):
        ...
        if self.pip_install:
            ...
            assert_equal(0, exit_code, 'Cannot install package {0} '
                                       'on admin node.'.format(' '.join(pkgs)))
        ...
        if self.get_timmy_version():
            logger.info("Timmy log collector found and ready")
            self.ready = True
            return True
    ...
    def get_timmy_version(self):
        return self.run_timmy(params = '--version')
    ...
    def run_timmy(self, dest_file=None, env_id=None, node_ids=None, roles=None,
                  ...
                  fuel_ip=None, fuel_user=None, fuel_password=None):
        """Collect logs
        ...
        self.ssh_manager.execute_on_remote(self.ip, cmd=cmd)