...
def check_hiera_resources(remote, file_name=None):
    ...
    if not file_name:
        ...
        output_f = ''.join(remote.execute(
            'if [ -r /etc/{0} ] ; then echo "passed" ;  fi'.format(file))[
            'stdout'])
        assert_true('passed' in output_f, output_f)
...
def ping_remote_net(remote, ip, timeout=100):
...
def check_firewall(remote, ):
    # TODO tleontovich : Add check for rabbit and remote puppet rule
    pass
