...
def get_package_version(remote_admin, package, income=None):
    ...
    cmd_version_income = ('rpm '
                          '-qp {0} --queryformat '
                          '"%{{VERSION}} %{{RELEASE}}" |sort'
                          ' -n'.format(package))

    cmd_version_current = ('rpm '
                           '-q {0} --queryformat '
                           '"%{{VERSION}} %{{RELEASE}}" |sort'
                           ' -n'.format(package))
    if income:
        result = remote_admin.execute(cmd_version_income)
    else:
        result = remote_admin.execute(cmd_version_current)
    asserts.assert_equal(
        0, result['exit_code'],
        'Failed to get package version'
        'with result {0}'.format(result))
    logger.debug('command execution result {0}'.format(result))
...
def compare_packages_version(remote, package_name, income_package_name):
    ...
    if version.LooseVersion(income_release) < version.LooseVersion(
        ...
        raise exceptions.PackageVersionError(
            package_name=income_package_name, version=income_release)
    if version.LooseVersion(installed_version) >= version.LooseVersion(
            income_version):
        raise exceptions.PackageVersionError(
            package_name=income_package_name, version=income_version)
    else:
        return True
