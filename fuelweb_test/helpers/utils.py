...
def get_ini_config(data):
    ...
    :param data: a string of file content
...
def get_process_uptime(remote, process_name):
    ...
    parent_pid = remote.execute(cmd)['stdout'][0].replace('\n', '')
    asserts.assert_not_equal(parent_pid,
                             '',
                             "No such process "
                             "with name {}".format(process_name))
    cmd = "ps -p {0} -o etime= | awk '{print $2}'".format(parent_pid)
