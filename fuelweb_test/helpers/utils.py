...
class SettingsChanger(object):
    ...
    SKIPPED_FIELDS_LIST = [
        ...
        'access.user', 'workloads_collector.user']
        ...
        if tp == 'checkbox':
            ...
            for val in values:
                if val['data'] != current_val:
                    ...
                    self.options[key][2] = opt['value']
                    logger.info(
                        '`{0}` was changed to `{1}`'.format(key, val['data']))
                    break
            else:
                logger.debug(
                    ...
                    try:
                        if not eval(condition) and self.options[key][5]:
                            ...
                            logger.info(
                                ...
                                "to restrictions".format(
                                    key, not self.options[key][2]))
                ...
                if tp == 'checkbox':
                    try:
                        if eval(condition) and self.options[key][2]:
                            ...
                            self.options[key][2] = opt['value']
                            logger.info(
                                '`{0}` was changed to `{1}` due to randomly '
