...
class FuelAccessParams(object):
    ...
    def from_yaml_params(yaml_content):
        ...
        """
        access_params = FuelAccessParams()  # type:FuelAccessParams
...
def install_configdb():
    ...
    if not settings.PERESTROIKA_REPO:
        raise exceptions.FuelQAVariableNotSet(
            'PERESTROIKA_REPO',
            'http://perestroika-repo-tst.infra.mirantis.net/mos-repos/centos/')
    ssh_manager = SSHManager()
    admin_ip = ssh_manager.admin_ip
    fuel_config_file = get_access_config_file(ssh_manager)
    openrc_content = ''
    with YamlEditor(fuel_config_file, ip=admin_ip) as yaml_editor:
        openrc_content = FuelAccessParams.from_yaml_params(
            yaml_editor.content).to_openrc_content

    openrc_path = '/root/.openrc'
    ssh_manager.check_call(
        admin_ip, 'touch {openrc_path}'.format(openrc_path=openrc_path))
