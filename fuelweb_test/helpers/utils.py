...
@logwrap
def kill_process_on_remote_node(ip, namepattern, prms=None, **kwargs):
    """Kill process on remote node

    :param ip: str, ip address of remote node
    :param namepattern: str, name pattern
    ...
    if prms:
        str_prms = ' '.join(
            ['{} {}'.format(k, v) for k, v in prms.iteritems()])
    cmd = 'pkill {str_prms} "{namepattern}"'.format(
