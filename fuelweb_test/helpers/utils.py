...
def get_net_settings(remote):
    ...
    for interface in node_interfaces:
        ...
        if interface in node_vlans:
            ...
            bond_slaves = [l.strip() for l in
                           run_on_remote(remote,
                                         bond_slaves_cmd.format(interface))]
        elif interface in node_bridges:
            if_type = 'bridge'
            bridge_slaves = [l.strip() for l in
                             run_on_remote(
                                 remote, bridge_slaves_cmd.format(interface))]
        else:
            if_type = 'common'
        if_ips = [l.strip()
