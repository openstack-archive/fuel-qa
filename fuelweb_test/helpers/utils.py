...
def erase_data_from_hdd(remote, device=None, source="/dev/zero",
                        block_size=512,
    ...
    :param device: str, block device which should be corrupted. If none -
       device with /boot partition will be choosen
    :param source: str, block device or file that will be used as source for
       "dd", default - /dev/zero
    :param block_size: int, size of block which will be pass to "dd"
    ...
    if not device:
        device = remote.execute(
            "mount | awk '/boot / {print($1)}'")['stdout'][0]
