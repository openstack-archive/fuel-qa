...
def prepare_steps():
    cloud_image_dir_path = "{}/cloudimage".format(os.getcwd())
    if not os.path.exists(cloud_image_dir_path):
        os.makedirs(cloud_image_dir_path)
    meta_data_path = "{}/meta_data".format(cloud_image_dir_path)
    user_data_path = "{}/user_data".format(cloud_image_dir_path)
...
def generate_meta_data(d_env, meta_data_path):
    ...
    meta_data_content = (""
                         ...
                         "local-hostname: pxeserver.local")
...
def generate_cloudimage_iso(cloud_image_dir_path,
                            user_data_path,
                            meta_data_path):
    cmd = "genisoimage -output {}/cloud_centos.iso -volid cidata " \
          "-joliet -rock {} {}".format(cloud_image_dir_path,
                                       user_data_path,
                                       meta_data_path)
    os.system(cmd)
