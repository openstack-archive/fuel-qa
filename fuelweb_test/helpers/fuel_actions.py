...
class FuelPluginBuilder(BaseActions):
    ...
    def fpb_install(self):
        ...
        :return: nothing
        """
        fpb_cmd = ("bash -c 'yum -y install tar createrepo rpm dpkg-devel "
                   "dpkg-dev rpm-build python-pip {packages}; {cmd}'")
        if FUEL_PLUGIN_BUILDER_FROM_GIT:
            fpb_cmd = fpb_cmd.format(
                packages=" git",
                cmd="pip install git+{}".format(FUEL_PLUGIN_BUILDER_REPO))
        else:
            fpb_cmd = fpb_cmd.format(
                packages="",
                cmd="pip install fuel-plugin-builder")

        self.ssh_manager.check_call(self.admin_ip, fpb_cmd)
