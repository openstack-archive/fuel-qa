...
class RallyEngine(object):
    ...
    def create_container(self):
        ...
        cmd = ('docker create --user {user_id} --net="host" --name '
               '"{container_name}" -e "http_proxy={proxy_url}" -t -i -v '
    ...
    def setup_utils(self):
        ...
        assert_equal(result['exit_code'], 0,
                     ...
                     '{0}'.format(result))
        cmd = "alias rally_docker='docker exec -t -i {0} /bin/bash'".format(
            self.container_name)
        result = self.admin_remote.execute('echo "{0}" >> ~/.bashrc'.format(
            cmd))
        assert_equal(result['exit_code'], 0,
                     "Alias creation for attaching Rally container failed: "
                     "{0}.".format(result))
    ...
    def list_tasks(self):
        cmd = (r"rally task list | awk -F '[[:space:]]*\\\\|[[:space:]]*' "
               r"'$2 ~ /\S{36}/{print $2}'")
        result = self.run_container_command(cmd)
