...
def regenerate_ubuntu_repo(ssh_manager, ip, path, port=22):
    # Ubuntu
    cr = CustomRepo(ssh_manager, ip, port)
    cr.install_tools(['dpkg', 'dpkg-devel', 'dpkg-dev'])
    cr.regenerate_repo('regenerate_ubuntu_repo', path)


def regenerate_centos_repo(ssh_manager, ip, path, port=22):
    # CentOS
    cr = CustomRepo(ssh_manager, ip, port)
...
class CustomRepo(object):
    ...
    def __init__(self, ssh_manager, ip, port=22):
