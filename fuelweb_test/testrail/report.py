...
def main():
    ...
    if not project.get_plan_by_name(test_plan_name):
        logger.debug('Creating new TestPlan "{0}".'.format(test_plan_name))
    ...
    plan_entries = [project.test_run_struct(
        name='{tests_suite_name}'.format(tests_suite_name=tests_suite['name']),
        suite_id=tests_suite['id'],
        milestone_id=milestone['id'],
        description='Results of system tests ({tests_suite}) on iso # '
                    '"{iso_number}"'.format(tests_suite=tests_suite['name'],
                                            iso_number=iso_number),
        config_ids=[os['id']],
    ...
    if not any(entry['suite_id'] == tests_suite['id']
        ...
        test_plan = project.add_plan_entry(plan_id=test_plan['id'],
                                           suite_id=tests_suite['id'],
