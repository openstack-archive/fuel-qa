...
TEST_GROUP_PATTERN = re.compile(r'run_tests.py\s+.*--group=(\S+)\b')
...
def _is_case_processable(case, tests):
    ...
    if issubclass(parent_home, ActionsBase) and \
...
def _get_test_case_name(case):
    ...
    """
    parent_home = case.entry.parent.home
    return parent_home.__name__ if issubclass(parent_home, ActionsBase) \
