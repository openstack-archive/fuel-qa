        ...
        if test_case[group_name] in existing_cases:
            testrail_case = \
                (case for case in cases
                 if case[group_name] == test_case[group_name]).next()
            ...
            for field in ('title', 'estimate', 'custom_test_case_description',
                ...
                if test_case[field] and \
                    ...
                    if field == 'estimate':
                        testcase_estimate = int(test_case[field][:-1])
                        testcase_estimate = \
                            _duration_to_testrail_estimate(testcase_estimate)
                        if testrail_case[field] == testcase_estimate:
                            continue
                    elif field == 'custom_test_case_description' and \
                        test_case[field] == testrail_case[field].replace(
                                '\r', ''):
                            continue
            ...
            if fields_to_update:
                ...
                logger.debug('Skipping uploading "{0}" test case because '
