...
def main():
    ...
    testrail_bug_filename = "-".join(['testrail_bugs', args.job_name,
                                      str(args.build_number), '.json'])
    ...
    update_subbuilds_failuregroup(subbuilds, failure_gd,
                                  testrail_testdata,
                                  testrail_bugs)
    json.dump(subbuilds, open(subbuildsfilename + ".update", 'w'))
    json.dump(failure_gd, open(failure_gdfilename + ".update", 'w'))
    json.dump(testrail_testdata, open(testrail_testdata_filename + ".update",
                                      'w'))
    json.dump(testrail_bugs, open(testrail_bug_filename + ".update", 'w'))
    ...
    statistics_filename = "-".join(['statistics', args.job_name,
                                    str(args.build_number), '.json'])
    if not statistics:
        logger.error('Necessary statistics info are absent. Exit')
        return 7
    json.dump(statistics, open(statistics_filename, 'w'))
