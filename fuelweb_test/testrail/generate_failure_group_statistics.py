...
def distance(astr, bstr):
    ...
    if alen > blen:
        astr, bstr = bstr, astr
        alen, blen = blen, alen
    current_row = range(alen + 1)  # Keep current and previous row
    ...
    if buildinfo:
        ...
        if artifact_paths:
            ...
            if xml_path:
                ...
                if req.status_code in xrange(200, 299):
    ...
    for failure, tests in failure_group_dict.items():
        ...
        if format_out == 'html':
            ...
            if testrail_tests:
                ...
                for i in tests:
                    item = {'test': i.get('test')}
                    item.update(get_testrail_test_urls(testrail_tests,
                                                       i.get('test')))
                    statistics[ftype][failure].append(item)
            else:
                statistics[ftype][failure] = [i.get('test') for i in tests]
        else:
            for i in tests:
                item = {'failure_type': ftype,
...
def dump_statistics(statistics, format_output=None, file_output=None):
    ...
    if format_output != 'html':
        for i in statistics:
            item = {'FailureType': i.get('failure_type'),
