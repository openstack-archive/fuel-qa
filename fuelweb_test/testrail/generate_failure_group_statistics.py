...
# Usage: python -m fuelweb_test.testrail.generate_failure_group_statistics
...
def get_testrail_data(plan_id, run_id):
    ...
    try:
        ...
        logger.error("TestRail request(s) was(were) failed due to - {}".format(e))
    ...
    if buildinfo:
        ...
        if artifact_paths:
            xml_path = [path for path in artifact_paths
                        if not path.find(
                        JENKINS.get('xml_testresult_file_name'))][0]
            if xml_path:
    ...
    if sub_builds:
        ...
        for failure in failure_reasons:
            if failure.get('failure') in failure_group_list:
                if not failure_group_dict.get(failure.get('failure')):
                    failure_group_dict[failure.get('failure')] = []
                failure_group_dict[failure.get('failure')].append(failure)
        # let's find Levenshtein distance and update failure_group_dict
        for n, k in enumerate(failure_group_dict.keys()[:-1]):
...
def dump_statistics (statistics, format_output=None, file_output=None):
    ...
    # if we would like to get html then let's ass links
    else:
        rows=[]
        data = tablib.Dataset(headers=headers)
        for i in statistics:
            if file_output and format_output in ['xls','xlsx','csv', 'yaml']:
                rows.append((i.get('failure_group'),
                             i.get('test'),
                             "/".join([i.get('job_name'),
                                       i.get('build_number')])))
            elif file_output and format_output in ['html']:
                rows.append((i.get('failure_group'),
                             '<a href={}>{}</a>'.
...
def main(argv=sys.argv[1:]):
    ...
    logger.info('Getting subbuilds for {} {}'.format(args.job_name, args.build_number))
    #subbuilds = get_sub_builds(args.build_number)
    #json.dump(subbuilds, open('/home/krozin/Documents/subbuild.json','w'))
    subbuilds = json.load(open('/home/krozin/Documents/subbuild.json'))
    logger.info('{} Subbuilds have been found'.format(len(subbuilds)))

    logger.info('Calculating failure groups')
    failure_group_dict, failure_reasons = get_global_failure_group_list(subbuilds)
    logger.info('{} Failure groups have been found'.format(len(failure_group_dict)))
    ...
    stat = get_statistics(failure_group_dict, tests)
