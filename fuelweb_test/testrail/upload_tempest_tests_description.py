...
def generate_groups(line):
    ...
    if 'compute' in line:
        groups.append('nova')
    if 'image' in line or 'glance' in line:
    ...
    if 'identity' in line or 'tenant' in line or 'auth' in line \
            or 'account' in line or 'credentials' in line:
    ...
    if 'rest_client' in line:
        groups.append('rest_client')
    if 'remote_client' in line:
        groups.append('remote_client')
    if 'linux' in line:
        groups.append('linux')
    if 'ssh' in line:
        groups.append('ssh')
