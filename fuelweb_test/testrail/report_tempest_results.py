...
def main():
    ...
    parser.add_option('-m', '--multithreading', dest='threads_count',
                      default=100, help='The count of threads '
                                        'for uploading the tests results')
    ...
    s.logger.info('Initializing TestRail project client...')
    client = trc.TestRailProject(url=s.TestRailSettings.url,
                                 user=s.TestRailSettings.user,
                                 password=s.TestRailSettings.password,
                                 project=s.TestRailSettings.project)
    s.logger.info('TestRail project client has been initialized.')
    tests_suite = client.get_suite_by_name(s.TestRailSettings.tests_suite)
    s.logger.info('Tests suite is "{0}".'.format(tests_suite['name']))

    # STEP #2
    # Parse the test results
    s.logger.info('Parsing the test results...')
    test_results = parse_xml_report(options.path)
    s.logger.info('The test results have been parsed.')

    # STEP #3
    # Create new test plan (or find existing)
    milestone = client.get_milestone_by_name(s.TestRailSettings.milestone)
    test_plan_name = '{0} iso #{1}'.format(milestone['name'],
                                           options.iso_number)
    s.logger.info('Test plan name is "{0}".'.format(test_plan_name))
    s.logger.info('Trying to find test plan "{0}"...'.format(test_plan_name))
    test_plan = client.get_plan_by_name(test_plan_name)
    if not test_plan:
        s.logger.info('The test plan not found. Creating one...')
        description = (s.JENKINS['url'] + '/job/' +
                       milestone['name'] + '.all/' + str(options.isp_number))
        test_plan = client.add_plan(test_plan_name,
                                    description=description,
                                    milestone_id=milestone['id'],
                                    entries=[])
        s.logger.info('The test plan has been created.'.format(test_plan_name))
    else:
        s.logger.info('The test plan found.'.format(test_plan_name))
    ...
    if not run:
        s.logger.info('Adding a test plan entry with test run '
        ...
        entry = client.add_plan_entry(plan_id=test_plan['id'],
                                      ...
                                      runs=[run_structure],
                                      name=run_name)
        s.logger.info('The test plan entry has been added.')
        run = entry['runs'][0]

    # STEP #4
    # Upload the tests results to TestRail
    s.logger.info('Uploading the tests results to TestRail...')
    joblib.Parallel(n_jobs=options.threads_count)(joblib.delayed(
        upload_test_result)(client, run, r) for r in test_results)
    s.logger.info('The results of Tempest tests have been uploaded.')
    s.logger.info('Report URL: {0}'.format(test_plan['url']))
