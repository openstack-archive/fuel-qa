...
MULTIPLE_NETWORKS = env_as_bool('MULTIPLE_NETWORKS', False)
...
BONDING = env_as_bool("BONDING", False)
...
USE_ALL_DISKS = env_as_bool('USE_ALL_DISKS', True)

UPLOAD_MANIFESTS = env_as_bool('UPLOAD_MANIFESTS', False)
SYNC_DEPL_TASKS = env_as_bool('SYNC_DEPL_TASKS', False)
...
UPLOAD_PATCHSET = env_as_bool('UPLOAD_PATCHSET', False)
GERRIT_REFSPEC = os.environ.get('GERRIT_REFSPEC')
PATCH_PATH = os.environ.get(
    'PATCH_PATH', '/tmp/fuel-ostf')

KVM_USE = env_as_bool('KVM_USE', False)
VCENTER_USE = env_as_bool('VCENTER_USE', False)
DEBUG_MODE = env_as_bool('DEBUG_MODE', True)
...
MAKE_SNAPSHOT = env_as_bool('MAKE_SNAPSHOT', False)

NEUTRON_ENABLE = env_as_bool('NEUTRON_ENABLE', False)
...
CLASSIC_PROVISIONING = env_as_bool('CLASSIC_PROVISIONING', False)
...
FUEL_STATS_CHECK = env_as_bool('FUEL_STATS_CHECK', False)
FUEL_STATS_ENABLED = env_as_bool('FUEL_STATS_ENABLED', True)
FUEL_STATS_SSL = env_as_bool('FUEL_STATS_SSL', False)
...
CUSTOM_ENV = env_as_bool('CUSTOM_ENV', False)
BUILD_IMAGES = env_as_bool('BUILD_IMAGES', False)

STORE_ASTUTE_YAML = env_as_bool('STORE_ASTUTE_YAML', False)

EXTERNAL_DNS = os.environ.get('EXTERNAL_DNS', '208.67.220.220')
EXTERNAL_NTP = os.environ.get('EXTERNAL_NTP', 'ua.pool.ntp.org')
FUEL_USE_LOCAL_NTPD = env_as_bool('FUEL_USE_LOCAL_NTPD', True)
FUEL_USE_LOCAL_DNS = env_as_bool('FUEL_USE_LOCAL_DNS', True)
...
REPLACE_DEFAULT_REPOS = env_as_bool('REPLACE_DEFAULT_REPOS', True)
...
PATCHING_DISABLE_UPDATES = env_as_bool('PATCHING_DISABLE_UPDATES', False)

DOWNLOAD_LINK = os.environ.get(
    'DOWNLOAD_LINK', 'http://releases.ubuntu.com/14.04.2/'
                     'ubuntu-14.04.2-desktop-amd64.iso')

UPDATE_FUEL = env_as_bool('UPDATE_FUEL', False)
...
ALWAYS_CREATE_DIAGNOSTIC_SNAPSHOT = env_as_bool(
