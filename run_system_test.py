...
def cli():
    ...
    cli_run.add_argument("groups", nargs='*', default=None)
...
def run(**kwargs):
    ...
    old_groups = kwargs.get('old_groups', None)
    ...
    for g in groups:
        if config_name:
            register_system_test_cases(
                    groups=[g],
                    configs=[config_name])
            run_groups.append("{0}({1})".format(g, config_name))
        else:
    ...
    if explain:
        ...
        register(groups=["run_system_test"], depends_on_groups=run_groups)
