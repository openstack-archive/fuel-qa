...
#  TODO: Remove
#  Temporary solution, I shuld test runner with click which absent on
#  CI servers
basedir = os.path.dirname(__file__)
sys.path.insert(0, os.path.join(basedir, 'vendor'))
...
@cli.command('run')
@click.option('--with-config', type=click.STRING, required=False, default=None)
...
@click.option('--with-xunit', default=False, is_flag=True,
              help="Gen xunit xml by Proboscis.")
@click.option('--nologcapture', default=False, is_flag=True,
              help="Disable log capture for Proboscis.")
@click.option('-q', default=False, is_flag=True,
              help="Run Proboscis in quite mode.")
@click.option('-a', "nose_attr", default=False, is_flag=True,
              help="Provide Nose attr to Proboscis.")
@click.option('-A', "eval_nose", default=False, is_flag=True,
              help="Eval Nose attr to Proboscis.")
@click.option('--group', type=click.STRING, required=False,
              default=None, multiple=True,
              help="Test group for testing. (backward compatibility)")
@click.argument('groups', nargs=-1, required=False, default=None)
def run_test(groups, with_config, explain, show_plan,
             with_xunit, nose_attr, eval_nose, q, nologcapture, group):
    """Run Proboscis with specified groups"""
    if with_config:
        groups = ["{0}.{1}".format(g, with_config)
                  if g.startswith('system_test.') else g for
                  g in groups or group]
