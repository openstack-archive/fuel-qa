...
def read_mirror_flags():
    return {
        k: get_env_value(k, v)
        for k, v
        in DEFAULT_MIRROR_FLAGS.iteritems()}


def combine_deb_url(
        snapshot_id,
        mirror_host=MIRROR_HOST):
    return ("http://{mirror_host}/mos-repos/"
            "ubuntu/snapshots/{snapshot_id}"
            .format(**locals()))
...
def combine_rpm_url(
        ...
        mirror_host=MIRROR_HOST):
    return ("http://{mirror_host}/mos-repos/"
            "centos/mos9.0-centos7/snapshots/{snapshot_id}/x86_64"
            .format(**locals()))
...
def g_build_extra_deb_repos(
    ...
    for dn in (
        ...
        if mirror_flags['ENABLE_MOS_UBUNTU_{}'.format(dn.upper())]:
            yield ("mos-{dn},deb {repo_url} "
                   "mos9.0-{dn} main restricted".format(**locals()))
