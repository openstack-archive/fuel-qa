...
It you are running and debugging many deployments on a single Fuel Master node
you may want to truncate the logs from the previous deployments. Using -l
option is also recomended for interactive use.
"""
...
class AstuteLog(object):
    ...
    def rpc_cast(self, record):
        ...
        if 'Casting message to Nailgun' in record:
            if 'deploying' in record or 'provisioning' in record:
                return
            self.add_line(record)
...
class FuelSnapshot(object):
    ...
    def parse_log(self, log_file, parser):
        ...
        :type parser PuppetLog
        :type parser AstuteLog
        """
