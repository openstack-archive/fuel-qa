...
def puppet_modules_mapping(modules):
    ...
    logger.debug("List of puppet modules covered by system_tests {}".format(
        all_modules))
    logger.info("List of modules edited in review {}".format(modules.keys()))

    # checking that module from review covered by system_test
    for module in modules.keys():
        if module not in all_modules:
            logger.warning("{} module not exist or not covered by system_test"
    ...
    if not ("ceph" in modules and set(
            ["roles/cinder.pp", "cinder", "openstack-cinder"]) & set(modules)):
        for test in mapping:
            ...
            if test_intersection > max_intersection:
                ...
                system_test = test
    # To completely check ceph module we can't mix storage settings with cinder
    else:
        logger.warning(
