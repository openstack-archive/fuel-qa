...
def fuel_library_modules_mapping(modules):
    ...
    if modules and isinstance(modules, dict):
        ...
        if not ("ceph" in modules and
            ...
            if devops_template != "ironic_template.yaml":
                path_to_template = devops_template
            else:
                import gates_tests
                path_to_template = os.path.join(
                    os.path.dirname(os.path.absname(gates_tests.__file__)),
                    "devops_templates/ironic_template.yaml")
...
def openstack_puppet_project_mapping(project):
    ...
    with open(
        ...
        if devops_template != "ironic_template.yaml":
            ...
            import gates_tests
            path_to_template = os.path.join(
                os.path.dirname(os.path.dirname(gates_tests.__file__)),
                "devops_templates/ironic_template.yaml")
