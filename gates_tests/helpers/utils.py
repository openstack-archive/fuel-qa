...
def openstack_puppet_project_mapping(project):
    ...
    "review_in_openstack_puppet_module" name
...
def map_test_review_in_openstack_puppet_modules(**kwargs):
    groups = kwargs.get('run_groups', [])
    old_groups = kwargs.get('groups', None)
    groups.extend(old_groups or [])
    if 'review_in_openstack_puppet_modules' in groups:
        if settings.GERRIT_PROJECT:
            module = settings.GERRIT_PROJECT
        else:
            module = str()
        fuel_library_modules_mapping(module)
