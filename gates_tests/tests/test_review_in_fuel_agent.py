...
class Gate(TestIronicDeploy):
    ...
    def check_package_version_injected_in_bootstraps(package):
        ...
        cmd = "ls |grep {} |grep deb |cut -f 2 -d \"_\"\'".format(package)
        package_from_review = ssh.execute_on_remote(ip=ssh.admin_ip, cmd=cmd)[
            'stdout_str']
        logger.info("package from review is {}".format(package_from_review))

        cmd = "chroot {}/ /bin/bash -c \"dpkg -s {}\" |grep Version |awk '{print $2}'".format(
                bootstrap_var, package)
        installed_package = ssh.execute_on_remote(ip=ssh.admin_ip, cmd=cmd)[
            'stdout_str']
        logger.info("injected package is {}".format(installed_package))

        if not package_from_review == installed_package:
            logger.error(
                    "package {} in bootstrap doesn't equal package from review".format(
                            package))
            raise
