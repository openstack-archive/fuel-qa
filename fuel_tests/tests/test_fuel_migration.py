...
def fuel_master_migration(request):
    ...
    intermediate_snapshot = "fuel_master_migration"

    instance = request.instance
    cluster_id = instance._storage['cluster_id']

    if not instance.env.d_env.has_snapshot(intermediate_snapshot):
        ...
        # make snapshot
        instance.env.make_snapshot(intermediate_snapshot, is_make=True)

    instance.env.revert_snapshot(intermediate_snapshot)
