...
class TestFuelMasterMigrate(object):
    ...
    def check_migration_status(self):
        ...
        self.node_rebooted(self.env.get_admin_node_ip(), timeout= 60 * 45)

        logger.info('Waiting for appearance of /tmp/migration-done file..')
        with get_admin_remote(self.env.d_env) as remote:
            assert_true(remote.exists("/tmp/migration-done"),
                        "File /tmp/migration-done wasn't appeared")

    @staticmethod
    def node_rebooted(ip, timeout=60 * 15):
        wait(lambda: not icmp_ping(ip), timeout=timeout,
             timeout_msg=("Node with ip: {} has not become offline after "
                          "starting reboot").format(ip))
        wait(lambda: icmp_ping(ip), timeout=timeout,
    ...
    def compute_warm_restart(self):
        ...
        self.manager.fuel_web.warm_reboot_ips([self.compute['ip']])
...
def fuel_master_migration(request):
    ...
    instance.wait_nailgun_available()
