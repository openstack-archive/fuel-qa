...
from fuelweb_test.helpers.fuel_release_hacks import (
    install_mos_repos_via_fuel_release)
    ...
    def get_ready_setup(self, centos=settings.CENTOS_MASTER_NODE):
        ...
        if self.check_run("empty"):
            ...
            if centos:
                ...
                if not (settings.CENTOS_CLOUD_IMAGE_PATH and
                        settings.EXTRA_DEB_REPOS):
                    raise exceptions.FuelQAVariableNotSet(
                        (settings.CENTOS_CLOUD_IMAGE_PATH,
                         settings.EXTRA_DEB_REPOS),
                        ("_path_to_CENTOS_CLOUD_IMAGE_PATH",
                         "EXTRA_DEB_REPOS"))
